...
const routes = ['settings', 'settings?tab=visibility', 'settings?tab=members', 'settings?tab=advanced', 'resources?section=api', 'resources?section=events', 'resources?section=data', 'resources?section=project', 'resources?section=guide'];
...
describe.skipIf(!session?.project)('项目设置与资源的真实布局', () => {
  test.each([
    ...
    for (const route of routes) {
      ...
      const measured = await page.eval<{ overflow: number; forms: number; reviewTools: boolean; nativeGroup: boolean; colorScheme: string; locale: string }>(`(() => ({
        ...
        nativeGroup: [...document.querySelectorAll('main label select')].some(node => node.getBoundingClientRect().height > 0),
      ...
      if (width < 900) expect(measured.nativeGroup).toBe(true);
