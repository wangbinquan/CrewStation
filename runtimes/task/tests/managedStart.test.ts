...
const profile = (steps: Parameters<typeof material>[0]) => material(steps, { profile: 'managed', revision: 2, vars: { GATEWAY: 'https://gw' }, secrets: { API_KEY: 'sk-managed' } });
...
describe('启动前 Hook 经真实 Runner 协议（RFC-004；RFC-006 起每次启动都经它）', () => {
  test('hello 宣告解释器；startAgent 先跑 Hook 再起 Agent，档位的变量、凭据与脚本输出进 CLI 环境，HOME 是私有家目录', async () => {
    ...
    await session.call({ id: 'm1', type: 'startAgent', agentId: 'managed-1', ...profileFields('managed-1', { compute: 'managed', profileRevision: 2, launch: launchSpec('opencode', { model: 'anthropic/echo' }), mcp,
      beforeStart: profile([{ kind: 'script', stepId: 'probe', name: '记录 HOME', language: 'shell', argv: [], timeoutMs: 10000, source: 'printf \'{"HOME_SEEN":"%s","MCP_SEEN":"%s"}\' "$HOME" "${CS_MCP_OPERATIONS_URL:-absent}" > "$CS_HOOK_ENV_OUT"' }]) }), mode: 'oneshot', initialPrompt: 'hello' });
    ...
    expect(started.event.event.spec).toEqual({ compute: 'managed', profileRevision: 2, protocol: 'opencode', model: 'anthropic/echo', permission: 'edit' });
    ...
    expect(hookDone.event.execution.profile).toEqual({ profile: 'managed', revision: 2 });
  ...
  test('Hook 失败：只发 before_start_failed 的 error 事件，不创建 CLI；准备中不能发消息；取消终止准备', async () => {
    const { session, drivers } = await boot();
    await session.call({ id: 'f1', type: 'startAgent', agentId: 'failing', ...profileFields('failing', { beforeStart: profile([{ kind: 'script', stepId: 'bad', name: '坏脚本', language: 'shell', argv: [], timeoutMs: 10000, source: 'exit 3' }]) }), mode: 'interactive', initialPrompt: 'hi' });
    ...
    await session.call({ id: 's1', type: 'startAgent', agentId: 'slow', ...profileFields('slow', { beforeStart: profile([{ kind: 'script', stepId: 'wait', name: '等待', language: 'shell', argv: [], timeoutMs: 30000, source: 'sleep 30' }]) }), mode: 'interactive', initialPrompt: 'hi' });
