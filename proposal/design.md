...
> 修订日期：2026-09-22（v0.3.7：RFC-018 回填——出站 FQDN 白名单下线，D47 作废并新增 D54，Q23 关闭，接入容器服务槽直连上游）  
...
> 修订日期：2026-09-23（v0.3.14：调用链按项目列出与回放——运行与诊断「调用链」页签列出本应用全部调用链、分层回放与执行事件，读取一律按项目，修正回放按 trace_id 跨项目取任务的越权；新增 §14.7 与 D62，§4.3 接口表、§14.4 相应补充）
> 修订日期：2026-09-24（v0.3.15：应用可见范围在网关拦截——正式地址按可见范围放行，拦下给平台统一的「没有项目权限」页，负责人可开放「申请访问权限」；项目关系新增「用户」角色，可见范围收为两档；新增 §7.5 与 D63，§1.1 第 6 条、§4.3 接口表、§7.1、§7.3 相应补充）
> 修订日期：2026-09-24（v0.3.16：数字人服务槽放开出站——项目命名空间默认网络策略的出向不再限制，数字人服务槽与迁移 Job 也直连公网与公司内网，入向隔离不变；新增 D64，D54 的「数字人服务槽不放行」作废；改 §2.4 流量表、§11.1、§13.1、D60 的措辞，§13.4 补一条接受的风险）
...
需求编号 R01–R58 以 Proposal v0.3.16 §6 为准。本设计以公司 Kubernetes 集群为唯一部署目标，本机验证使用 kind 集群，不再有 Docker Compose 路径。
...
- 新增配置与 Secret 对象、日志聚合与日志页、部署健康态与告警订阅、套餐与副本声明、每项目命名空间、TaskRunner 独立 UID、空闲提醒、管理员代建项目、preview 测试者、参考 APIProxy 与内置 EventProducer 走项目流程、CLI 入首版。
...
6. **网关统一鉴权，业务不带凭据。** 用户身份由网关认证后以可信身份头与绑定目标服务的签名令牌注入；业务服务不做登录鉴权。正式地址先按应用可见范围放行（v0.3.15，D63），更细的授权规则归业务。服务身份由网关按源 Pod IP 反查，业务代码不携带、不能自报任何身份。
...
| `Project` | 开发与管理空间，首版承载一个业务服务；由开发者自建为负责人或管理员代建并指定负责人；对应一个命名空间 | 项目级 |
...
| 实体 | 建议映射 | 谁创建／释放 |
|---|---|---|
| 项目命名空间 | 每项目一个 Namespace，含 ResourceQuota、默认 NetworkPolicy、项目 Secret | cs-controller 在项目创建后异步开通 |
...
flowchart TB
    ...
    Dev --> Model["模型 / 依赖源 / GitLab"]
```

运行中的部署关系由工作台从受管集群快照实时画出：每个项目的概览与运行诊断有自己的形态图，管理员在集群管理看系统层与项目层；见 §14.6（RFC-019）。
...
| 项目命名空间出站 | 任务容器、构建、服务槽与迁移 Job → 模型、依赖源、GitLab、平台 MCP、公网与公司系统（默认 NetworkPolicy 出向不限制，不按域名；v0.3.16 起数字人服务槽也放开，D64） |
...
| `GET /v1/projects/:id/traces` ；`GET …/traces/:traceId` ；`GET …/traces/:traceId/executions/:taskId/events` | 本项目的调用链列表（来源、状态、有活动的时间范围筛选，游标翻页）、一条链的分层回放、一个 Agent 执行的事件分页；只含本项目的记录，本项目没有的链回 404（v0.3.14，D62） |
| `GET /v1/apps/:projectId/access` ；`POST /v1/apps/:projectId/access-requests` ；`GET /v1/app-access-requests` ；`POST /v1/app-access-requests/:id/decision` | 申请人读自己对某应用的使用权与最近一条申请并提交使用申请（任何登录用户）；负责人与管理员按项目、管理员按全局分页读申请，同意即加为「用户」角色，拒绝可附理由（v0.3.15，D63） |
...
| `POST /v1/admin/projects/:id/archive` / `delete` ；`GET /v1/admin/audit` | 项目归档、删除；审计查看。`pause` 已作废（v0.3.9，D57：由下线待验证版本与正式版本维护替代） |
| `POST /v1/services/:id/slots/preview/offline`、`…/postpone`；`POST /v1/releases/:id/redeploy`；`GET /v1/services/:id/slot-events`；`GET`／`PUT /v1/services/:id/maintenance`、`POST …/maintenance/exit`；`GET`／`PUT /v1/admin/settings/auto-offline` | 待命槽下线、推迟、重新部署与记录；正式版本维护；自动下线时长（v0.3.9，§6.9） |
...
**要求：开发者可自建数字人项目，创建者自动为负责人；管理员可自建或代建并指定具有平台开发资格的负责人。每创建一个逻辑业务服务，平台就在配置的公司 GitLab 兼容 Group／Subgroup 下自动创建一个 Project，并创建项目命名空间。** 两个部署槽、副本、发布版本和开发会话都复用该仓库。接入容器同样如此。

自动建仓与初始化流程：

1. 开发者提交名称、标识与可用数字人模板，服务端固定本人为负责人和默认资源；管理员可额外指定合格负责人和资源。cs-api 事务性创建项目、服务、owner 成员、配额与开通事件；创建与平台角色降级使用同一用户锁，避免用户被降级后仍成为新项目负责人。
...
一个 Pod 已运行不代表仓库、数据库、模型和预览均可用。开发会话与每个 CLI 的执行环境另有按段的启动进度，见 §5.10。
...
### 5.10 启动进度（v0.3.10，RFC-022）

开发会话、每个 CLI 的执行环境和档位测试都按段报告启动过程。段的种类共八种：排队分配容器、替换旧容器、容器启动中（调度、拉取镜像）、检出代码（分支）、容器已启动等待连接、准备环境（启动前步骤 x/y）、Agent 启动中、已就绪。各对象用其中一部分：

- 开发会话：五段，排队、容器、检出、连接、就绪；重建时以「替换旧容器」代替「检出代码」。
- CLI：六段，排队、容器、连接、准备环境、Agent 启动中、就绪；没有启动前步骤时，准备环境记为跳过。
- 档位测试：沿用同一契约，另有自己的步骤、模型轮次与测试命令段。

规则：

- **只进不退，首尾相接。** 每段的起止时间取自事实来源：Pod 与 Events（调度、容器起止、镜像拉取）、Runner 事件（启动前步骤、进程拉起）、平台的受理时刻。
- **只有平台判定才记失败。** Kubernetes 自己还在重试的问题（调度不上、拉取退避）只记为警告。
- **完成的段只留结果说明。** 失败的段保留说明、警告和日志尾部，作为现场。

保存与读取：

- task-runtime 把环境的启动进度存在 `environments.startup`，由每秒一次的观测更新，只观测启动中的环境。
- CLI 的六段由 dev-session 用环境的前三段加上执行任务的 Runner 事件组合出来，就绪、失败或取消后冻结在受理记录里。
- 会话与名册接口带 `startup` 和读出时刻 `observedAt`。工作台在有对象启动时每秒读一次，失败后日志还没补上的 20 秒内也每秒读，并按读出时刻校正本机时钟。

显示：工作台、CLI 与档位测试共用一个步骤条组件。

- CLI 的步骤条盖在终端区域中间，状态条显示当前段。
- 开发会话的步骤条替换整个 CLI 区，页头芯片显示当前段。
- 失败时停在出错的那一段，给出重试和日志：开发会话失败在检出代码及之前，就按原分支重新开始，并回收失败会话的容器与工作卷（2026-09-23 修订）；失败在之后，就打开恢复。CLI 的重试是原位替换原标签。

谁点的「＋ 创建开发Agent会话」，就在那个窗口替谁取得这个 CLI 的输入控制：Runner 在启动中接受取得，但仍拒绝输入与改尺寸，协议号不变；旧 Runner 在进程拉起后再取，不报错。收到 PTY 输出即视为进程已拉起。细节见 [RFC-022 设计](./rfc/RFC-022-startup-progress/design.md)，实机证据见其验收记录。
...
固定远端 Project ID、标签、SHA、构建 profile、依赖锁与镜像基础摘要。构建任务使用干净隔离环境，只取得所需源码、私有依赖和镜像写入权限。全局与项目级构建并发上限入表队列。记录镜像摘要、确切源码提交、构建环境、依赖检查、测试和日志；构建日志可在日志页查看。秘密不进入镜像层、构建参数、源码或日志。

发布关联图必须可查询：`开发会话 → 分支 → 推送提交 → v 标签 → Build → 镜像摘要 → 配置版本 → 迁移记录 → Release → 待命槽 ServiceDeployment → TrafficSwitch → prod 槽`。

### 6.5 待命槽部署与切流

构建成功后：解析生产组配置与 Secret 为配置版本；按 `release.migration` 声明对生产库执行迁移，迁移必须与在线槽正在运行的 Release 兼容（默认 expand-only），`destructive: true` 的发布只允许在项目维护期间（三个开关都拦，D57）部署与切流；启动待命槽副本、健康检查；preview 域路由指向待命槽，项目成员与指定测试者可访问。待命槽与在线槽共用生产数据库、文件、身份与授权。后发布的标签替换待命槽上的 Release，前者标为 Superseded。

切流由项目负责人发起，带 `expectedActiveRelease`：核对在线槽 Release、待命槽健康、必要业务检查，然后网关把 prod 域路由切到待命槽；原在线槽保留运行成为待命槽（回退目标，到期自动下线见 §6.9），记录 TrafficSwitch。事件订阅、后台消费者的有效目标随切流一起改为新 prod 槽。回退是负责人再次切换回原槽的独立动作，不回退数据库、不恢复已撤销授权；若期间已执行不兼容迁移，回退被阻塞并说明。
...
### 6.9 待命槽下线与正式版本维护（v0.3.9，RFC-021）

待命槽（preview 角色）上的版本可以**下线**：删掉它的 Deployment，保留 Service 与路由，槽记为已下线（原因、时间、操作人），发布记录标为 `offline`；试用地址由网关给出平台的「未部署」说明页，接口请求得 503。下线有四个来源：负责人或管理员手动下线；切流后被换下的**回退目标**保留期满（默认 72 小时，从切流起算，访问不延长）；待验证版本**连续无人访问**（默认 14 天，从就绪或最近一次访问起算）；管理员在集群管理删除非正式槽。自动下线前 24 小时提醒负责人（记录、时间线、发布页与概览可见），负责人可推迟一个周期、不限次数；没提醒过不下线；有发布进行中或集群运维操作时不动。三个时长在管理空间「平台设置」统一调整（D56）。已下线或被替代的版本可从发布记录**重新部署**到待命槽：不重新构建、不重跑迁移，按当前生产配置部署，迁移兼容性与回退同一套检查。

正式版本（prod 角色）可以**进入维护**：Pod 照常运行，网关按三个独立开关拦截——用户流量（维护页展示原因与预计恢复时间，接口 503 与 `Retry-After`；项目成员、平台管理员与临时指定的人照常进入）、服务域调用（503；项目自己的负载与平台组件不拦）、事件推送（投递标为「维护暂存」，不算尝试，退出维护或关掉开关后按接收顺序重新入队）。维护跟着正式入口走，切流后仍在，负责人或管理员手动退出。**项目维护中且三个开关都拦即破坏性迁移窗口**：含 `destructive: true` 迁移的版本只在此时部署，切流到这类版本也一样（D57）。接口、状态与失败模式见 [RFC-021 设计](./rfc/RFC-021-slot-offline-maintenance/design.md)。
...
网关按 Host 分两个域。用户域承载工作台、业务页面与 API、preview 与开发预览，做登录跳转与身份注入；会话 Cookie 按 Host 下发，应用域与控制台使用不同的注册域，网关对非安全方法校验请求来源。服务域承载内部 API 调用、平台 API 调用、事件推送、EventProducer 入口与数字人互调，不做登录跳转；网关按源 Pod IP 反查调用方身份并注入平台签名的来源令牌，业务服务凭来源令牌区分平台推送与用户请求。业务服务的网络策略只接受来自网关的流量。令牌格式、有效期、签名密钥轮换与验签方式为待决项 Q16。平台保证身份可信，并按应用可见范围放行正式地址（v0.3.15，D63，见 §7.5）；页面与数据更细的权限规则由业务实现，常见于可见范围是「全部登录用户」的应用。
...
平台身份与项目关系分开（RFC-011，作者 2026-09-20 批准）。新账号默认 `user`；既有管理员保留，原项目负责人／开发成员一次性回填为 `developer`，其他账号为 `user`，试用成员关系保留。角色初始化有独立标记，迁移中断重跑不重新提权。

| 平台角色 | 首页与能力 |
|---|---|
| 用户 `user` | 默认能力市场，按应用可见范围使用；被指定为试用成员时可从 Beta 卡片试用未发布应用，无开发和管理入口 |
| 开发者 `developer` | 默认能力市场；可进入本人为负责人／开发成员的数字人项目，自建项目自动为负责人；不能访问平台管理 |
| 管理员 `admin` | 默认能力市场，包含开发者能力；可开发全部数字人项目，并在平台管理中处理用户角色、接入容器、套餐、配额、开放策略、项目生命周期、安装升级与全局审计 |
...
| 用户 `user` | 可以是普通用户；只能打开正式地址、在市场里看到应用，不进项目、不试用待命版与开发预览；正式版本维护中与其他人一样看维护页（v0.3.15，D63） |

市场使用业务卡片，点击直接打开应用主页，移除应用详情页，旧详情地址重定向市场。卡片对所有角色都不提供项目编辑入口，不显示 SHA、部署槽或负责人；未发布且允许试用的应用显示 Beta，正式应用卡片为成员保留独立的新版 Beta 链接，不把正式入口误标 Beta。全局 Agent 动态入口与其跨空间查询移除，开发会话内部状态保留。显式且仍授权的开发／管理深链接继续有效，无目标登录回市场。

对象级操作检查最新平台角色、项目关系与状态；平台开发身份不自动加入已有项目。管理员变更角色使用旧角色比较更新（冲突返回 409），不能移除最后管理员；在任负责人须先转交才能降为用户。非负责人成员降级时保留关系但停止开发权限；已有 HTTP／CLI／WebSocket 也重新授权，不能靠旧令牌或连接继续读写，恢复角色后按存量关系生效。创建者与角色编辑的竞态受共同锁保护。完整接口、失败恢复与验收见 `rfc/RFC-011-role-based-home/`。
...
### 7.5 正式地址的使用权与使用申请（v0.3.15）

应用可见范围（D63）同时决定市场里谁看得到、网关放谁打开正式地址，只有两档：「项目成员（含用户）」与「全部登录用户」。原「项目成员与指定用户」一档取消，名单迁为「用户」角色成员；新建应用默认「项目成员」，已有应用照原设置生效。管理员、负责人与任一成员角色（含「用户」）始终放行。接入容器不进市场、没有可见范围，正式地址只给管理员与成员。

用户域 ForwardAuth 对正式地址逐请求判定，放行才注入身份。拦下时浏览器导航得到 403 与平台统一的「没有项目权限」页，写明应用名称，地址栏不变，会话不清；程序调用得到 403 JSON（`details.reason = app-access`）。负责人在应用展示页另设「允许申请／只能授权」，默认允许。允许时页上给「申请访问权限」，新开工作台申请页，填写可选理由（最多 500 字）提交；只能授权或接入容器时，页上写「请联系项目负责人」和负责人的名字，由负责人在成员页直接添加。

同一人对同一应用同时只有一条待处理的申请。负责人与管理员在三处处理：应用展示页的「使用申请」卡、项目概览的提醒条、管理空间的「申请审批」。同意即加为「用户」，已是成员的保留原角色；拒绝可附理由，申请人在申请页看得到，之后可以重新申请。一次裁决后不可更改。申请页在待处理时自动重读，批准后给「打开应用」，原页面刷新即可进入。

待命版与开发预览的规则不变：只给负责人、开发成员与试用成员（「用户」不算），拒绝时仍是原来的 403 页，没有申请入口。
...
| SQLite 或本地单写目录 | 单副本＋独占写入策略 | `releaseMode: maintenance`：停写、切流、重挂，在项目维护期间（§6.9） |
...
迁移脚本来源于用户项目，使用独立 Job 与迁移身份，对生产库加迁移锁和版本条件；在标签部署到待命槽时执行，因此迁移期间在线槽仍在运行，迁移必须与在线槽的 Release 兼容：默认采用扩展、回填、切流、后续收缩的分阶段方式，`release.migration.compatibility` 声明兼容范围。`destructive: true` 的发布只能在项目维护期间（三个开关都拦，D57）部署与切流，切流后不可回退到不兼容的旧槽、也不能重新部署更早的版本，界面明示。迁移失败进入 Failed，保留日志与修复指引，不切流。Agent 不获得生产迁移身份；生产变更由负责人审批的 production-change 或新标签完成。
...
| 暂停项目（已作废，v0.3.9，D57） | 由下线待验证版本（删待命槽工作负载）与正式版本维护（网关拦流量，Pod 不停）替代 | 保留数据库与文件 |
...
TaskDataBinding 记录任务、资源、主体、模式、范围、到期时间、审批人与凭据引用。获批绑定进入任务容器后，容器内的 Agent 与终端都可使用，这是接受并记录的残余风险（§13.4）。任务启动时实际验证连接；缺授权或资源标为未就绪。暂停、关闭、撤销与到期都要处理已有连接，不只阻止发新凭据；撤权时效需实测。开发会话释放时，平台直接收回它名下还没结束的绑定：终止连接、删掉临时角色、记为已收回，不另作确认，因为释放本身已有弹窗确认；到期的绑定由后台每分钟收一次（2026-09-23 作者裁定）。开发会话默认不订阅生产事件、不执行生产定时任务、不发送真实通知，环境标识以约定环境变量提供。
...
“空集群”指节点、网络、DNS 与容器运行条件可用但未安装 CrewStation。管理员还需提供公司源码托管地址、目标 Group 与建仓、推送、创建标签资格，身份体系与模型访问，以及集群可达的依赖源。安装预检必须验证所选 CNI 在 Pod 到网关路径上保留源 IP，并实测它确实执行 NetworkPolicy（D60）：项目之间的入向隔离全靠 NetworkPolicy（v0.3.16 起出向不再限制，D64），而不执行它的网络插件照样建得出策略、只是不生效。
...
5. **初始化**：最小样例模板、任务容器与服务套餐、数据与配额套餐、管理员、源码托管连接、上游连接与开放策略；为内置 GitLab EventProducer 与参考 APIProxy 自动建平台项目、建仓并发布首个标签。
...
| 更新对象 | 管理者 | 是否影响业务数据 |
|---|---|---|
| CrewStation 常驻服务、MCP、日志采集 | 安装器／Helm／平台发布流程 | 仅受控元数据迁移 |
...
| 开放策略、目录、上游连接、套餐 | 管理员流程 | 独立生效，不随代码回退 |
...
开放策略、目录、APIGrant 变化生成新版本下发网关，定义最大延迟与失联时策略。生产组配置与 Secret 变化生成新配置版本，需新标签或负责人触发的重启生效，界面明示。
...
| 风险源 | 必须控制 |
|---|---|
| 开发会话与业务任务中的 Agent 生成代码、命令 | 项目命名空间内独立任务容器；CPU、内存、进程、文件大小限制；出站不按域名限制（RFC-018）；Agent 以普通用户运行，TaskRunner 独立用户 |
| 同一容器内多个 Agent 与终端 | 平台不做写入协调；只保证容器边界、配额与发布前检查 |
| 数字人业务服务 | 命名空间隔离；不读取平台数据库与其他项目 Secret；只接受来自网关的流量；不能自报身份 |
| 内部 API 调用 | 网关按源 Pod IP 反查身份与本地放行表；proxy 无长期凭据。v0.3.16 起网络策略不再阻止绕过 proxy 直连公司系统（D64）：放行表只约束经网关的调用，直连时拿不到只给 proxy 的上游凭据 |
...
设计目标为数百数字人服务并发、数百节点、单集群。按项目、服务和槽统计正式服务资源、任务容器数与时长、Agent 进程数、构建消耗、数据库连接、对象容量、事件吞吐、网关请求与长连接、日志量。配额只限并发任务数；其他资源以套餐与监控管理。目标档位的负载参数在 T5.7 定义，网关、cs-auth、cs-session、cs-events、数据库分别在该档位压测；未实测前不写具体数字。
...
| 项目命名空间出向不限制，数字人服务可以不经接口目录与网关放行表直接调用公司系统 | 接受（D64，作者 2026-09-24 裁定） | 公司系统自身的认证；上游凭据只给接入容器；经网关的调用仍按放行表判定 |
...
用户能看到：数字人地址、仓库与两槽部署标签及 SHA、开发会话状态与空闲提醒、Agent 会话与终端、预览、数据访问模式、发布与切流进度、可用接口与配额、配置与 Secret 键、日志、健康态与告警、本应用的调用链列表与回放（§14.7）。管理员能看到：配额占用与拒绝、放行表与身份索引版本、事件投递与死信、构建部署失败、孤儿容器与卷、备份恢复、安装升级步骤与版本、审计。

### 14.5 受管集群目录与管理员运维（RFC-010）

管理空间 `/admin/cluster` 覆盖项目（包括归档残留）与显式登记的内置组件；不认领整个系统命名空间，也不覆盖 kube-system。资源归属由项目目录与有效 owner UID/资源引用确定；Pod 用途结合 task-runtime 的实例 UID、父任务及 native purpose，区分工作区、CLI、Agent、业务子任务和档位测试。物理部署槽与当前正式/试用角色来自 release。

L6 `cluster-management` 拥有持久快照、来源完整性、5分钟操作检查与幂等队列/结果，独立于观测日志模块；cs-api 提供查询/检查/受理，cs-controller 执行采集和操作。来源失败保留旧值并明确缺失，不能显示为真实零。列表按同一快照分页，过期返回410。

Kubernetes 原生动作使用 UID/resourceVersion 条件；开发工作区经保卷恢复、CLI/Agent 经新执行、业务任务经暂停/恢复/取消/重试、档位测试经终态流程，避免直接删 Pod 留下错误平台状态。release 拥有物理槽副本覆盖（跨后续发布保留，可恢复 Manifest）、删除试用槽后的槽状态和历史。成功业务不会被集群入口重放，正式槽和核心安装对象有明确操作限制。

操作结果显示阶段、原始原因、操作者、目标 UID、operationId、领域操作 ID、HTTP/trace与耗时；丢失回执和“继续核对”复用原幂等意图。详细语义与实机证据见 [RFC-010](./rfc/RFC-010-cluster-management/proposal.md)。

### 14.6 部署与运行形态图（RFC-019）

工作台把 §2.3 的部署关系按项目实时画出来：项目概览一行横带汇总卡（线上槽、待命槽、开发会话、业务任务、构建与迁移各一张，按实际存在的对象出现），运行与诊断「部署与运行形态」页签的全图（入口 → 工作负载 → Pod → 数据与存储四条泳道），集群管理「拓扑」页签的三层（`crewstation-system` 系统层、项目层、单项目 Pod 层）。数据只有一份：§14.5 的受管集群快照。成员通过项目范围的只读盘点接口 `GET /v1/projects/{projectId}/cluster-resources` 拿到同一快照按项目过滤的投影（须有该项目的 `develop` 授权，`availableActions` 恒为空，超过 500 项标 `truncated`，快照过期 410）；管理员的摘要 `summary.projects[]` 带每项目的工作负载、Pod、就绪、异常与开发会话计数。来源读取失败时计数不填、受影响节点标待核对，不显示零。

图由工作台自绘 SVG（`apps/console/src/shared/ui/topology/`），视觉借鉴 Archify：九种语义色进 `tokens.css` 明暗两套；节点为语义色描边＋半透明填充，状态另有非颜色提示；观测到的关系实线、静态架构标注虚线并在图例注明不是实测；排布是确定性的分层布局，节点位置只由 UID 与显式行号决定，换快照不重排；铺满容器宽度，≤640px 降级为分组列表；项目层异常项目置顶、超过 60 个折叠。成员侧没有重启、扩缩、删除入口，Pod 层点节点走 §14.5 的资源详情。范围、裁定与实机证据见 [RFC-019](./rfc/RFC-019-deployment-topology/proposal.md)。

### 14.7 调用链列表与项目隔离（v0.3.14）

运行与诊断的「调用链」页签（D62）列出本项目的全部调用链：
- **来源**：开发会话、业务任务与推送给本应用的事件三类都算。同一条链跨类时合成一条，例如事件触发的业务任务与该事件是同一条，起点是事件。
- **排序与筛选**：按链上最早记录的时间倒序。可按来源、状态、时间范围筛选；时间范围有最近 1 小时、24 小时、7 天与全部，按「这段时间里有活动或仍在进行」算。也可以粘贴完整 trace_id 直接打开。
- **状态三档**：
  - 进行中：任何一部分还在进行。
  - 失败：死信、业务任务失败、子任务最后一次尝试失败、会话或任务容器失败。
  - 已结束：其余情况，即业务任务关闭、会话正常释放、事件已送达。
  - 单个 CLI 或 Agent 执行失败只记在它自己那一行，不让整条链变成失败。

选中一条，右侧（窄屏在下方）分层回放：
- 起点事件：投递结果、尝试次数、最近一次错误。
- 各个任务：开发会话或业务任务，写明起止时间、创建人或调用方、分支。
- 各个 Agent 执行：CLI、headless Agent、业务子任务的每次尝试，写明档位、协议、原生会话 ID 与失败原因。
- 每个执行的事件逐条分页查看：Agent 事件、启动前步骤、CLI 活动信号、终端关闭。平台自己执行的命令不列，这类命令由工作台读分支、读文件时产生，一个会话可达数万条。思考事件不带文字（§14.2 不保留推理内容）。

**按项目隔离。** 一个事件投给多个订阅项目时，各项目的投递共用同一个 trace_id。v0.3.14 之前的回放按 trace_id 跨项目取任务，别的订阅项目的成员能看到本项目延续该 trace_id 的任务与 Agent 输出摘要。

现在列表、回放与事件三个接口（§4.3）的每次读取都带项目，本项目没有这条链时回 404：
- 数据由 task-runtime（任务环境）、events（投递）、business-task（业务任务与子任务）、session（Runner 事件）各自按项目给出，observability 只负责合并。
- 两个来源合并翻页时，只收「每个给满了一页的来源都已扫描到」的下界以内的链，保证翻页不丢不重。

事实数据仍取自各模块自己的表与 cs-session 保存的 Runner 事件；§14.1 所说独立的 execution_events 表与 `otel_trace_id` 关联尚未实现，本节不改变这一点。
...
| D03 | 五个自研常驻服务，内部模块化 | 基线；cs-events；另有两个平台 MCP 与日志采集组件（出站代理一项已随 RFC-018 作废） |
...
| D17 | 每逻辑业务服务自动创建独立托管 Project | 要求；RFC-011 支持开发者自建或管理员代建 |
...
| D35 | 三类平台角色与独立项目关系 | 要求（S7）；RFC-011 将平台角色确定为用户／开发者／管理员，项目负责人／开发成员／试用成员独立；默认市场、开发者自建为负责人 |
...
| D53 | 受管集群目录与持久运维操作独立为 L6 cluster-management，原模块保留生命周期和期望配置所有权 | 作者 2026-09-20 批准 RFC-010、ADR-0006；具体本机权限执行/验收状态单独记录 |
| D54 | 平台不按域名限制出站。项目命名空间的出向由 NetworkPolicy 按负载标签决定：默认只到 DNS 与系统命名空间；开发会话、业务任务、构建全放行；接入容器（`APIProxy`／`EventProducer`）的服务槽全放行，数字人服务槽不放行。**「默认只到 DNS 与系统命名空间」「数字人服务槽不放行」两句已作废**（v0.3.16，D64：默认出向不限制，按标签放行的几条策略随之不再起作用） | 要求（作者 2026-09-22 裁定，RFC-018）；取代 D47 的出站白名单部分；部分由 D64 取代 |
| D55 | 部署与运行形态图由工作台自绘 SVG 从 RFC-010 的受管集群快照画出，不引入 Archify 运行时；项目成员经项目范围只读盘点接口看到与管理员相同的 Pod 投影（不含环境变量值、Secret、注解、YAML），管理动作只在集群管理 | 要求（作者 2026-09-22 批准 RFC-019 并裁定其提案 §7） |
...
| D60 | 安装与升级预检实测网络插件确实执行 NetworkPolicy：临时命名空间里放一个应答端和两个探针 Pod，套了禁止出站策略的必须连不上、没套的必须连得上，测完删除；不通过就停止，不继续安装、不迁移也不滚动。项目之间的入向隔离全靠 NetworkPolicy（v0.3.16 起出向不再限制，D64；此前还有 D54 的出站隔离），而不执行它的网络插件照样建得出策略、只是不生效 | 要求（作者 2026-09-23 裁定；起因是本机 kind 集群自带的 kindnet 执行策略有缺陷，导致 Runner 反复掉线，见 `docs/engineering/dev-gotchas.md`「本机集群的网络插件是 Calico」） |
...
| D62 | 调用链按项目列出与回放：运行与诊断「调用链」页签列出本项目全部调用链——开发会话、业务任务、推送给本应用的事件三类，同一条链跨类时合成一条；状态分进行中／已结束／失败，时间范围按有活动算，可按 trace_id 打开；右侧分层回放（起点事件 → 任务 → Agent 执行），执行事件逐条分页，平台自己执行的命令不列。列表、回放与事件的每次读取都按项目取数，本项目没有的链回 404；此前的回放按 trace_id 跨项目取任务，一个事件扇出给多个订阅项目后，别的项目的成员能看到本项目的任务与 Agent 输出摘要 | 要求（作者 2026-09-23 裁定：「能不能显示该应用的所有调用链列表」；两轮问答定下三类来源、左列表右详情、分层时间线、来源／状态／时间／按 ID 打开四种筛选与查找、状态三档、时间按有活动算，流程直接改＋回填；页签名按 RFC-020 原批准改为「调用链」） |
| D63 | 应用可见范围在网关拦截：正式地址按可见范围逐请求放行，可见范围收为「项目成员（含用户）」与「全部登录用户」两档，已有应用照原设置立即生效；拦下时给平台统一的「没有项目权限」页，负责人另设「允许申请／只能授权」（默认允许），允许时页上「申请访问权限」新开工作台申请页，只能授权时写负责人名字；项目关系新增「用户」角色，只用正式版、不进项目、不试用待命版，维护中不放行；负责人与管理员在应用展示页、项目概览提醒与管理空间「申请审批」处理，同意即加为「用户」；原指定用户名单迁为「用户」；接入容器正式地址只给管理员与成员、不接受申请；待命版与开发预览规则不变；RFC-003 市场可见性「不把市场隐藏宣称为禁止通过已有链接访问」一句作废 | 要求（作者 2026-09-24 裁定：「应用的可见范围还是在网关拦截一层吧，如果没应用权限要显示平台统一的没有项目权限，然后有个申请权限按钮。待命版和开发预览保持现状。都是要做到平台按授权范围拦截的。业务可以自己再拦截一层，这种一般会出现在业务开了全员可见的情况下」；两轮问答定下存量照现有设置生效、批准即加为「用户」、成员页加「用户」而可见范围收为两档、无权限页在原地址显示且按钮新开工作台申请页、负责人可选允许申请或只能授权（默认允许）、三处审批入口、接入容器只给管理员与成员；流程直接改＋回填） |
| D64 | 项目命名空间出向不限制：默认网络策略 `crewstation-default` 的出向改为全放行（显式写成 `egress: [{}]`，服务端 apply 整体替换这个列表），数字人服务槽、迁移 Job 与其他负载一样直连公网与公司内网；入向不变，项目命名空间只接平台系统命名空间的入向，项目之间照旧互不可达。任务、构建、接入容器三条按标签放行的策略照旧下发、不再起作用（资源中心对网络策略只建、只改回、从不删）；存量命名空间由调和器在换版后按新形状改回。接口目录与网关放行表不再是访问公司系统的唯一网络路径，只约束经网关的调用；R15 改写，Plan AT-07 第一句作废 | 要求（作者 2026-09-24 裁定：「都放开外网」；问答中选定只改默认策略、保留三条多余策略，接受数字人服务可以不经接口目录直连公司系统；流程直接改＋回填，RFC-018 记修订）；部分取代 D54 |
...
| Q07 | 切流的检查项与破坏性迁移的维护窗口规则 | 产品与组织规则评审；T2.11 关闭。维护窗口规则已由 D57 定（v0.3.9）：项目维护且三个开关都拦；切流检查项仍待评审 |
