...
用户能看到：数字人地址、仓库与两槽部署标签及 SHA、开发会话状态与空闲提醒、Agent 会话与终端、预览、数据访问模式、发布与切流进度、可用接口与配额、配置与 Secret 键、日志、健康态与告警。管理员能看到：配额占用与拒绝、放行表与身份索引版本、事件投递与死信、构建部署失败、孤儿容器与卷、备份恢复、安装升级步骤与版本、审计。
