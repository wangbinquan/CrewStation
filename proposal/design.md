...
需求编号 R01–R58 以 Proposal v0.3.15 §6 为准。本设计以公司 Kubernetes 集群为唯一部署目标，本机验证使用 kind 集群，不再有 Docker Compose 路径。
...
| 任务容器出站 | 任务容器 → 模型、依赖源、GitLab、平台 MCP（命名空间 NetworkPolicy 按负载标签放行，不按域名） |
...
| D54 | 平台不按域名限制出站。项目命名空间的出向由 NetworkPolicy 按负载标签决定：默认只到 DNS 与系统命名空间；开发会话、业务任务、构建全放行；接入容器（`APIProxy`／`EventProducer`）的服务槽全放行，数字人服务槽不放行 | 要求（作者 2026-09-22 裁定，RFC-018）；取代 D47 的出站白名单部分 |
