...
2026-09-23 修订说明（作者实机反馈「图是不是没有自动更新，必须刷新页面才会刷新」；裁定等 RFC-025 T7 上线后直接改＋回填）：

- **实测**：轮询确实在跑，但有两层滞后。
  - 服务端每 30 秒采集一次快照，页面每 15 秒读一次，变化最长约 45 秒才上图。
  - 缺陷：全局 30 秒的新鲜期挡掉了「回到前台补读」。从别的标签页切回、从概览进来都不立即重读，后台期间跳过的重读也不补（切走 18 秒再切回，又等了 10 秒才重读），于是像是「刷新才有」。
- **修订**：
  - `useApiQuery` 的定时重读查询，新鲜期缩到自己的重读周期（只缩不延）；进入页面或切回标签页时，数据旧过一个周期就立即重读。工作台同类查询一并生效，与 `usePollingRefetch` 的「后台暂停、回前台补查」一致。
  - 图框右上角的观测时间精确到秒。
  - 悬停提示改为「集群快照每 30 秒采集一次，页面每 15 秒读取」。
- **现状**：开发会话、CLI 与业务任务两带（含各自的工作卷）自 RFC-025 T7 起随推送流即时更新；服务槽、构建与迁移仍读快照，按 RFC-025 计划在后续各期（T8、T9）进台账。
