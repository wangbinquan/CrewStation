...
> 状态：In Progress · 2026-09-22 · 本机 kind 集群、dev-admin 真实 Chrome；未实机复核的项写明原因
...
| TP-11 | 通过 | 集群管理「拓扑」系统层：`crewstation-system` 内 21 个节点、20 条静态线默认显示，图例注明「虚线为静态架构标注，不是实测」；`cs-api` 的 aria-label「cs-api，副本 1／1，REST API · 集群盘点 · MCP 后端」，每个组件状态来自同一快照的资源表 |
...
| TP-16 | 部分通过 | 1728 视口：集群拓扑 SVG 1467px 铺满 1470px 容器；运行诊断页签开详情时 SVG 1072px 铺满 1074px 容器，`scrollWidth 1728 = innerWidth`；1280／1024 铺满与 390／320 列表在 `tests/e2e/topology.test.ts` 断言，本机因 §4 未跑，浏览器会话过期后无法再登录复核 |
...
| TP-18 | 部分通过 | 完整本地 gate 通过（§1）；`test:cover`：unit／module／console 三层全绿，e2e 层 28 项全部因登不进（§4，密码登录关闭且 dev-auth 未就绪）超时失败，其余 27 项 skip；`test:patch --base origin/main --worktree`：改动的可执行行 **522／522（100%）** 被用例执行到；精确 SHA CI 待推送后记录 |
...
收尾清单：作者授权后恢复 dev-auth；重建 console 镜像（含 `factText`）复核 TP-16 的 1280／1024／390、TP-17 的键盘与浅色主题、§3 的显示；跑 e2e 层；把上表「待复核」改成实测；记录精确 SHA 的 CI 六项结果；README 收口。
