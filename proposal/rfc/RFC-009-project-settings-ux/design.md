...
> **2026-09-23 修订（作者裁定，直接修改＋回填，不另立 RFC；表单与确认改弹窗）。** 本节的「编辑区」都改为弹窗，列表与摘要常驻：环境变量的「新增变量」与行内「修改／更新密钥」、成员的添加与改角色（负责人转移的确认叠在上面）、应用展示与可见范围的两个编辑器。开发、生产两组各留一份草稿；每个弹窗取消、✕、Esc 只关窗，再点同一项恢复，「清空」回到打开时的值；有未保存输入时改编辑另一项先弹确认（「继续编辑」回到原草稿），不再有「取消编辑」的行内确认。切环境时藏在页签后面的那一组连同它的弹窗一起藏起，草稿仍在；离开确认照旧列出会丢的草稿。保存失败的原因留在发出保存的那份草稿的弹窗里。行内「修改」「删除」按同日按钮统一改为紧凑档。
...
> **2026-09-23 修订（作者当面裁定，直接修改，不另立 RFC）。** 归档与删除配置项的确认改为页面内的模态弹窗 `shared/ui/dialog/ConfirmDialog`，写清对象与后果（归档：服务路由随后移除，正式与待验证版本的地址都将无法访问；源码仓库、数据库与文件保留；已有开发会话与运行中的容器不自动清理；当前没有恢复入口）。输入英文确认词才能确认，归档输 `archive`，删除输 `delete`。归档请求进行中弹窗锁住，结束后关闭，结果仍显示在卡片上；删除配置项确认后弹窗即关，进行中的反馈在该行的删除键上。归档的行为、权限与状态限制不变。见 RFC-003 design §6 同日一条。
