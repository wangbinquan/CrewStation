// 只构建设计附件，不连接真实 API，不进入 console 的生产路由或构建。
import { mkdir, copyFile } from 'node:fs/promises';
import { resolve } from 'node:path';

const output = '/private/tmp/crewstation-project-settings-rfc009';
...
if (process.argv.includes('--serve')) {
  const server = Bun.serve({ hostname: '127.0.0.1', port: 5199, fetch(request) {
    const file = new URL(request.url).pathname;
    if (!['/', '/index.html', '/entry.js', '/entry.css'].includes(file)) return new Response('Not found', { status: 404 });
