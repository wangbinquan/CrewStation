...
| [RFC-024](./RFC-024-cli-interface-ready/proposal.md) | CLI 界面就绪：新开 CLI 的步骤条等到 CLI 画出界面再撤；Runner 在无头终端上按「可见文字＋静止 500 ms」判定（45 秒超时放行），启动进度增加「CLI 初始化」段，旧底座行为不变 | Done · 2026-09-23 · 作者批准，Q1–Q3 取推荐（静止 500 ms、超时 45 秒、代答查询另立 RFC）；实现 652677fa，[CI 35855602907](https://github.com/wangbinquan/CrewStation/actions/runs/35855602907) 六项成功；本机部署 `iface-20260923`、默认档位修订 6，实机 OpenCode 43.2 秒画出界面后步骤条才撤，见 [验收记录](./RFC-024-cli-interface-ready/acceptance.md)；待作者定：超时是否调长 |
