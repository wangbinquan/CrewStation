...
| [RFC-019](./RFC-019-deployment-topology/proposal.md) | 部署与运行形态图：项目概览缩略与运行诊断全图、集群管理拓扑页签（系统层／项目层／Pod 层），视觉借鉴 Archify、工作台自绘 SVG | Draft · 2026-09-22 三件套已写，待作者批准；两轮原型评审的裁定（详情右侧、静态线默认显示、语义色进 tokens、Archify 图不入库、全页去掉 1200px 上限已实施 `6a69195`）已落入提案 §4，提案 §7 一项待确认；见 [prototype-review.md](./RFC-019-deployment-topology/prototype-review.md) |
