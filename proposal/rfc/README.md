...
| [RFC-013](./RFC-013-resource-uuid/proposal.md) | 平台资源统一 UUIDv7：完整 36 字符 ID、名称与身份分离，覆盖套餐／档位／模板／API／事件／配置／步骤及旧数据迁移 | In Progress · 2026-09-21 本地门禁 1937 pass／5 skip／0 fail，旧库副本升级和 97.4% 改动行防护通过；已授权与 RFC-014 联合发布，实际切换另行验收 |
| [RFC-014](./RFC-014-identity-admin-ux/proposal.md) | 用户与权限、认证 UX：紧凑用户目录与按需角色编辑，登录方式与身份字段分组，接入方配置和诊断重排 | In Progress · 2026-09-21 T1–T7 完成，44 项定向与 1937 项共享候选门禁通过；最新样式已实机复查，联合发布和精确 SHA CI 进行中 |
