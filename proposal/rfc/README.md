...
| [RFC-010](./RFC-010-cluster-management/proposal.md) | 集群资源管理：项目与平台内置资源的数量、状态、归属和 Pod 用途，受控重启／扩缩／删除与操作记录 | In Progress · 实现已提交、基线 v0.3.6 已回填；完整检查 1864 pass / 5 skip / 0 fail，构建与最终页面只读实机验证通过；远端发布核对中，本机 RBAC 审批阻塞完整清单和动作验收，见 [证据](./RFC-010-cluster-management/acceptance.md) |
| [RFC-011](./RFC-011-role-based-home/proposal.md) | 三类平台角色与使用者首页：能力市场只展示应用，成员试用卡片标 Beta，开发者自建项目，管理员独立管理，删除全局 Agent 动态 | In Progress · 作者 2026-09-20 批准实施并提交上库；角色／授权／自建与首页已实现，真实四测试视角、创建开通和 Beta 试用通过；完整门禁 1864 pass／5 skip／0 fail，发布收口中，具体测试账号改权待授权；见 [验收记录](./RFC-011-role-based-home/acceptance.md) |
| [RFC-012](./RFC-012-project-compute-access/proposal.md) | 项目算力授权：允许的 Agent 档位、项目默认档位、开发容器套餐及全链路校验 | In Progress · 2026-09-20 本地实现项目授权与默认可见性（平台默认强制可见）、开发套餐及运行链校验；96 条相关回归、三尺寸构建预览和部署后的真实页面/API 验收通过，共享门禁与提交依赖见实施计划 |
