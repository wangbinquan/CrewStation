...
| [RFC-015](./RFC-015-cluster-resource-observability/proposal.md) | 集群容量与用量：节点总览、Pod／容器资源、PVC 申请与实际使用及最近七天趋势 | In Progress · 2026-09-21 · 作者已批准完整实施、部署与提交上库，含七天趋势、采集器与指标存储；正在实现 |
| [RFC-016](./RFC-016-preview-process-control/proposal.md) | 开发会话预览进程：Agent 经操作 MCP 自主启动／停止／重启，富状态与预览输出缓冲，控制统一走 cs-api | In Progress · 2026-09-21 · T1–T9 落地并推送：Runner 三条新命令经 `previewControl` 能力位协商、输出环形缓冲、三条 cs-api 路由、三个 MCP 工具、工作台迁 REST；PV-01…PV-18 实机验收未执行 |
