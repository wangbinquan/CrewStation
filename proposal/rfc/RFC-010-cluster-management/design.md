...
> 状态：In Progress；作者已批准实现，范围裁定见 [proposal.md](./proposal.md)
> 日期：2026-09-20；当前实现事实及源码行号见 [audit.md](./audit.md)
...
下面是待实现契约，统一管理员校验与现有错误信封。路径属于当前单集群，不引入多集群选择器。
...
默认观察期限拟 5 分钟；超时显示未收敛与当前原因，不自动回滚或取消 Kubernetes 已受理的动作。
...
- 本机服务账号新增读取、受控 patch 与 Ingress 删除权限已写入安装清单，但自动审批阻止应用到共享本机集群；具体授权问题等待作者答复。无需扩大权限即可完成的 UI/数据库测试继续执行。

实机范围、对应 CM 编号与未验收项以 [acceptance.md](./acceptance.md) 为准。
