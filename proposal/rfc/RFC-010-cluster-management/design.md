...
> **2026-09-21 修订（作者当面裁定，直接修改，不另立 RFC）。** 采集每 30 秒换一份快照，快照 id 进查询键，原实现因此每 30 秒把列表和详情
> 整块卸载成「载入中」，滚动容器塌掉后位置回到顶部。裁定：新快照自动采纳，但**除 snapshotId 外条件一致时保留上一份数据、回执到达再原地替换**
> （`useApiQuery` 的 `keepPrevious`／`features/cluster/model/clusterReads.ts`），筛选、分页与目标变化仍按新查询载入；「下一页」的游标改为跟随
> 该页回执自己的 snapshotId，不与摘要新读到的快照错配。摘要与列表仍取同一份快照（§7 的一致性不变）。
...
> **2026-09-23 修订（作者当面裁定，直接修改，不另立 RFC）。** 「删除／结束」改为页面内的模态弹窗 `shared/ui/dialog/ConfirmDialog`：点下去就弹窗并自动检查影响，影响清单、UID、关联资源和确认有效期都在弹窗里，输入 `delete` 才能确认。受理后弹窗关闭，操作进度与结果照旧在页内显示。检查结果只认当前动作最近一次的那份，取消或换了动作之后，路上的旧结果不会挂到新动作上。重启与调整副本仍是先检查影响、再在页内确认。

> **2026-09-23 修订二（作者裁定，直接修改＋回填，不另立 RFC；表单与确认改弹窗）。** 重启、调整副本、恢复发布配置整条流程也移进弹窗：先「检查影响」，结果出来后在同一个弹窗里确认（此时 role 换成 alertdialog）；调整副本的目标数是草稿，关窗保留、「清空」回到当前副本数，改了数目要重新检查；回车只触发检查，不会直接确认执行。
