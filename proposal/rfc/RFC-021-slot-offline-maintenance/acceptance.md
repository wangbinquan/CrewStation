...
## 2026-09-23 修订（T14）的实机核对

作者当面裁定两处修订（见提案 §4 末）：提醒发出之后才能推迟；待验证版本空着时可以选版本部署。代码 cd80eac（推迟）、8e51443（选版本）。本机 08:40Z 把 cs-api 滚到 `cs-control-plane:postpone-20260923`、console 滚到 `cs-console:postpone-20260923`，两者都由 `git archive 8e51443` 构建；其余控制面服务未动。

| 项 | 结果 | 证据 |
|---|---|---|
| 提醒之前没有「推迟」、接口拒绝 | 通过 | `rfc021-verify`（dev-developer，负责人）：待命槽 v0.1.1 回退目标，到期 09-26 06:14Z，`retention.postponable = false`；待验证卡只有「回退到 v0.1.1」「打开试用」「下线」；按页面上的到期时间调推迟接口得 412「还没到可以推迟的时候：到期前 24 小时提醒负责人之后才能推迟」，槽没有变化 |
| 提醒之后出现「推迟 72 小时下线」，点一次后消失 | 通过 | 只改这一行计时 JSON 的 `since`（`06:14:09.053Z` → `2026-09-21T07:41:20.546Z`，到期落进 24 小时窗口），由真实巡检在 08:41:58Z 发出提醒（槽事件 `reminder`，cs-controller 日志 `slot offline notice` 发给负责人）；页面「… · 已于 16:41 提醒负责人」并出现「推迟 72 小时下线」，`postponable = true`。点击后到期改为 09-27 07:41:20Z、`postponements = 1`、`postponable = false`，按钮消失，时间线记「dev-developer 把 v0.1.1 的自动下线推迟到 2026年9月27日 15:41」；接着再调接口得 412 |
| 待验证卡空着时「部署版本…」、弹窗里选版本 | 通过 | `demo`（dev-admin，M9 同权）：待命槽已下线 v0.1.2，卡片主按钮「部署版本…」；弹窗标题「部署到待验证版本」，版本下拉默认 v0.1.2，列出 v0.1.3（已被替代）与 v0.1.2（已下线），新到旧；改选 v0.1.3 后问句与确认键随之改变。确认发出的 `POST /v1/releases/<v0.1.3>/redeploy {"expectedStandbyReleaseId":null}` 用 CDP Fetch 在浏览器里拦下（服务端没有收到，demo 状态不变）；1440 与 390 宽没有横向溢出，控制台无错误 |
| 只读 e2e | 通过 | `slotLifecycle`（dev-admin，`CS_TEST_REQUIRE=e2e`）4／4：槽的计时带 `postponable`；离到期远于提醒提前量时推迟必然被拒 |
| 门禁与 CI | 见说明 | 两笔提交前都在「HEAD＋本批」的干净导出树上跑 `bun run check`（2343／2364 pass，0 fail）。cd80eac 的 [CI 35836810951](https://github.com/wangbinquan/CrewStation/actions/runs/35836810951) 五项成功，e2e 只红 `layoutSpacing` 两条（c5e44b8 带入，767aca6 已修，与本批无关）；8e51443 的 [CI 35838303881](https://github.com/wangbinquan/CrewStation/actions/runs/35838303881)：六项全部成功（含 gate 新增代码防护与 e2e） |

## 遗留

- 升级起算的 8 个待命槽（SM-07）会按规则在 09-25 起陆续提醒、09-26 起下线；提醒发出之后，各负责人可在发布页推迟（2026-09-23 修订）。
- `rfc021-verify` 保留作证据：正式版本是声明了破坏性迁移的 v0.2.1，待命槽是回退目标 v0.1.1，`/api/hello` 为默认开放，可见范围为「所有登录用户」。T14 核对后 v0.1.1 的到期时间是 09-27 07:41Z（推迟过一次），09-26 07:41Z 前后会再次提醒。
