...
> **2026-09-23 修订（作者裁定，直接修改＋回填，不另立 RFC；表单与确认改弹窗）。** 角色编辑不再在目录右侧展开：「管理权限」打开弹窗（标题带姓名与邮箱，焦点落在当前选中的角色上），「检查变更」的确认叠在上面；保存失败时确认收起、原因留在角色弹窗，409 的「重新读取当前角色」在弹窗操作条里；编辑时不再禁用筛选。关窗保留选择，再点同一位用户恢复，改了没保存时去管理另一位先确认。§4 的身份提供方新增／编辑同日改为大弹窗（四个分组页签，编辑时「删除此接入方」在操作条里、删除确认叠在上面），§5 的项目规则（按项目覆盖）添加／编辑改为弹窗，取消只关窗、原来的「放弃输入」行内确认去掉。
