...
> Done · 2026-09-18 作者设定会话目标「完整实现RFC并提交上库」，据此实施完成。按下表小批推进；每批自带测试（开发规则 §4），门禁绿了再按精确路径提交（§2、§3）。
>
> 2026-09-18 进度：T1–T13 全部完成（见文末「实施说明」）：CP-01…CP-22 与 design.md §13 实机核对，基线三件套回填到 v0.3.4。待作者复核：§13 的 P1–P8、ADR-0005、implementation-open-questions I17–I19。
...
| T4 | 完成（7cbe22f） | `modules/task-runtime/tests/agentExecutions.test.ts`（三种用途的准入与额度、档位镜像按摘要、显式 Runner 命令与 `runAsUser: 0`、握手被拒回收并释放额度、镜像拉取／容器起不来立即失败、暂停结束子执行环境且恢复不重起）；`taskRuntimeModule.test.ts`（无 agent-env） | 每个 Agent 一个 Pod：「＋ CLI」、headless Agent 与业务 Agent 子任务各自一个 Pod（档位摘要、显式 Runner 命令、Runner 以 root 起、CLI 为 uid 10001），占额度并在结束后释放（CP-02／08／09／10／17）；业务任务暂停结束子执行环境、恢复不重起（CP-19） |
...
| T6 | 完成（7cbe22f） | `modules/business-task/tests/businessTaskModule.test.ts`（子任务各自执行环境、额度满失败文案、子 Runner 未连上时等待、消息路由、执行环境丢失时失败带原因、未开始即取消回收、修订固定与 `default` 解析） | CP-10／17／19 实机通过：`/chat` 的 Agent 子任务在自己的 Pod 里经默认档位得到真实模型输出；额度满时立即 failed 且不建 Pod；命令子任务不占额度；暂停结束 Agent Pod。实机发现构建 Pod 写死 1 CPU 导致首次发布排不进节点，b0a526c 按它实际跑的 `git clone`＋`buildctl` 客户端改为 250m／512Mi（I18 更新）；子任务失败原因只剩「运行时报告错误」，67c343d 让 opencode 的 error 行带上厂商原文（任务镜像未重建，实机仍是旧文案）；「当前可用」实为现有档位，文案改为「现有档位」 |
...
| T9 | 完成（2635e27） | release 三种拒绝；capabilities 改源；CLI 安装不播种（`installInitialize.test.ts`、`installConfig.test.ts`）；模板 `compute: default`；settings 删除 `CS_AGENT_ENV_SECRET`／`CS_DEFAULT_COMPUTE_PROFILE` | CP-07：Manifest 引用通用终端档位的发布在部署阶段被拒；CP-17：模板 `compute: default` 在设了默认档位后首次发布成功；「没有默认档位」时的拒绝本机无法复现（默认设过就不能停用或删除），由 release 用例覆盖 |
...
| T12 | 完成（2026-09-18） | — | 全部 CP 与 design.md §13 四项已实机核对，逐项结果与证据见 `/private/tmp/crewstation-rfc006-20260918/README.md`。本机做不到的两处以自动化用例为准：CP-03 的 Claude Code 真实轮次（本机无登录态，只验证了鉴权失败分类）、CP-11 的「没有默认档位」。CP-17 的 `/chat` 真实轮次要求 `chat-v1` 用 full 档（本机只有拒绝无 bash 请求的免费模型，I19） |
| T13 | 每批执行 | 2635e27：本地门禁全绿，并在隔离目录单独跑过提交树的门禁；CI run 35308530121 成功。7cbe22f（每个 Agent 一个 Pod）：本地门禁全绿（1546 pass）；CI run 35310598962 成功。b2cf79a：CI run 35311620432 成功；4a1b274：CI run 35312141019 成功。2e743a6＋8083552（实机修复）：本地门禁全绿（1578 pass／5 skip）；CI run 35322681836 成功。b0a526c（构建 Pod 右尺寸）、67c343d（opencode 错误原文）与文案修复：本地门禁全绿，CI 见 STATE.md | — |
