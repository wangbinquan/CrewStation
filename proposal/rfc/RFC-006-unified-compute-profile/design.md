...
2026-09-23 修订说明（[RFC-022](../RFC-022-startup-progress/design.md) D8）：测试的阶段迁到公共启动进度契约。

- **段的来源。** 排队分配容器、容器启动中、容器已启动等待连接三段取自测试环境的启动进度；测试结论仍按 Pod 阶段归类。
- **段的改名与用时。** `launch` 段改为 `agent`（Agent 启动中），它和之后的模型轮次、测试命令都带起止与用时。
- **管理页。** 「测试结果」换成公共步骤条，定位到步骤、退出码与输出尾部照旧；升级前的测试记录按旧的段种类照常显示。
- **Runner。** 执行 CLI 的 Runner 在启动中接受「取得输入控制」，协议号不变，供创建者窗口提前取得；要用这一点，档位镜像需按新底座重建并另存修订。
