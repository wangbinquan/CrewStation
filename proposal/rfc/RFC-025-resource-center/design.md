...
>
> - **渲染**：调和器与原来同一组构造函数、同样的标签（不加资源 ID 标签，按名字归记录，线上对象一个字段都不用改）；网络策略按名字取模板，平台换版改了策略形状时调和器发现不一致即按新形状改回，不再靠启动重下发。缺了或不一致才 apply；先命名空间后额度，网络策略等所在命名空间观测到了再建；命名空间删除中不动（记录是降级，原因写明删完后补回），消失后按「缺了」再建；期望不完整的不渲染，只告警。命名空间、额度与网络策略调和器只建、只改回，从不删（§6.4）；项目归档时两条记录不释放；2026-09-27 作者已裁定保留，工作卷和其他资源处理完后由管理员删除，入口待实现（[I27](../../../docs/engineering/implementation-open-questions.md#i27-项目归档后命名空间记录与命名空间怎样收尾)）。
...
> **实施补记（2026-09-24，T16：租约接进调和器）**：持有者是副本自己（`<进程名>-<主机名>.cluster-control`／`.data-control`）。
>
> - `cluster-control` 逐条调和在记录的租约下（30 秒，处理中每 10 秒续约，结束即释放）。抢不到不直接丢掉：持有者那一轮读到的可能是这次变化之前的版本，所以隔 5 秒再排一次。孤儿回收整轮持一把作业租约（`cluster-control:orphan-sweep`，5 分钟），另一副本这一轮跳过。
> - `data-control` 的全量一轮持作业租约（`data-control:resync`）；尾随逐条持记录的租约，抢不到跳过——漏掉的由下一轮全量（至多 30 秒）补上。
> - 不分工的：补投影（任务环境、服务槽、路由、限流、数据资源）、放行表核对、墓碑清理、台账维护、启动时的命名空间重下发——都是「按期望再写一遍」，同样的内容台账不写库，两个副本各跑一份只是多几次读。身份索引两个副本都按各自的观测写（内容相同）；新副本首次全量同步那一刻，它列出之后才建的 Pod 会被短暂标成已删，随它的 watch 事件补回。
...
>
> **实施补记（2026-09-24，第五期第二步：集群管理的清单叠加台账，I29 裁定）**：清单与历史仍以集群管理 30 秒一次的采集快照为底（台账之外的系统组件、ConfigMap 等照列）；读清单、详情与项目盘点时按页向资源中心要认领叠加（`claimsOf`，按种类＋命名空间＋名字成批查子对象），台账认领的行带上所属标准记录的阶段、原因、看的人能做的操作，以及 `maintained`——期望在、种类由调和器渲染（路由、限流策略、命名空间、网络策略；种类注册表的 `rendered`，与调和器按种类应用的清单一致）。这些对象的「删除」禁用，写明「由资源中心按期望维护，删除后会被补回；要移除请从所属功能下线或释放」；工作卷的 PVC 也不给直接删，指向「待回收的工作卷」。删除前的影响检查按实时对象再叠加一次：台账读不到就不给做（清单读不到则照快照给，不挡住清单）。项目盘点只叠加阶段与原因，不给记录操作。工作台：清单的状态格照记录写八个标准阶段，集群观测到的状态短语退为小字；详情加「资源中心记录」一节（记录操作带版本号经资源中心受理，不可撤销的要输入 `delete`）；「待回收的工作卷」排在「存储与配置」之后，读全平台视图里「待回收」为真、PVC 还在的工作卷，每行「删除工作卷」。标准输出同时修正：没有执行者的操作（所属模块没登记、资源中心自己也不做）如实写「这类资源暂不支持在这里操作」——此前工作区的释放、重试在视图里显示可做、受理时才 412；所属模块登记执行者之前，集群管理里实际可做的记录操作只有删除工作卷。拓扑读全平台视图、`capabilities` 摘要改读视图留在 T13 后续；重启、扩缩仍走 RFC-010 的领域操作（经所属模块，结果随投影回到记录）。
...
>
> **别名入账（第六期第二步）**：台账的别名表此前是空的。task-runtime 把 RFC-013 之前的旧身份（`tsk_…`）写成工作负载记录的别名（来源 `tsk`），按旧 ID 可以找回记录；投影与补投影都带上，同样的别名不重复写。本机有 15 个没释放的环境带旧身份。旧标签（对象上的 `crewstation.io/task: tsk_…`）不改写：认领按名字，孤儿判定先经身份目录换成现 ID，改写只是改标签、没有行为差别，其中多数对象（09-21 失败的三个旧会话）还会随保留期满被回收。
...
>
> **实施补记（2026-09-24，I25 裁定后第一步：工作区的创建移交）**：按 [I25](../../../docs/engineering/implementation-open-questions.md#i25-调和器接手建任务容器时凭据放在哪) 的 (b)，开发会话与业务任务的工作区改由调和器建出。task-runtime 受理时只登记：建出容器要用的期望随记录进台账——Pod 的镜像、运行 UID、资源、工作负载标签、检出（仓库、分支与 Git 凭据 Secret 的名字，凭据本身不在）、开发预览的端口、主机与中间件，工作卷的大小与标签；子对象加上这一次启动的 Runner Secret（`<Pod 名>-runner-<第几次启动>`）；领域条件 `Provisioning`（创建中、还没绑定 Pod 实例）为真时调和器才建。调和器先建工作卷的 PVC，卷进了观测缓存再建工作区：Runner Secret 不在就向 task-runtime 要内容（`runnerValues`：平台约定变量、配置、数据连接串与这一刻签发的 Runner 令牌，令牌只存哈希；环境已不需要建出时拒绝）后建，不可变；Pod 的环境只从这个 Secret 引用，Pod 规格里没有凭据；有预览的再应用预览 Service 与路由；最后把 Pod 的 UID 交回（`bindWorkload`：「排队分配容器」这一段结束，`Provisioning` 随之变假）。已在的对象不动；观测到过又没了的不补建——丢了的 Pod 仍由 task-runtime 判「容器不存在」、不复活，丢了的卷换成空卷会让数据悄悄消失。建不成的按退避重试，原因写成只归资源中心的条件 `Created` 为假，阶段是「分配中」并带这条原因；过了建出宽限（2 分钟）仍没有 Pod，task-runtime 照旧判失败。恢复暂停的业务任务即再启动一次：换一个 Runner Secret（第几次启动加一），Pod 同样由调和器建。执行环境（CLI、headless、子任务）、失败后的重建与档位测试这一步仍由 task-runtime 建，是 I25 的后续步骤；服务槽与 Job 随 T8。运维开关 `CS_WORKLOAD_CREATION=owner` 回退为 task-runtime 受理时自己建（迁移期的回退手段；已由资源中心建出的环境恢复时仍走资源中心）。

> **实施补记（2026-09-24，I25 第二步：执行环境的创建移交）**：CLI、headless Agent 与业务子任务的执行环境同样由调和器建出。受理照旧（额度、不可变执行身份，受理时读一次集群核对父工作区——这一处读仍在 task-runtime），由资源中心建出时只登记：期望带镜像（档位按摘要固定）、资源、父工作区的节点与工作卷、所属工作区标签与受理意图注解，以及父工作区受理那一刻的 Pod 名与 Pod、卷的 UID；不进准备队列。调和器建之前照观测缓存核对父工作区的 Pod 与卷还是那两个实例、Pod 在运行且在那个节点、卷已绑定——父工作区在受理之前就在、调和只在缓存同步之后进行，所以缓存里没有就是没了（例如工作区重建换了 Pod），与换了实例一样经端口 `workloadUnavailable` 交 task-runtime 判准备失败（「启动期间原工作区实例或工作卷已变化」），不建也不等。核对通过就建不可变的 Runner Secret（`<Pod 名>-runner`：执行环境不会再启动，沿用原名；内容向 task-runtime 要，这时父工作区得还在运行、连着，不在就判「原工作区已断开」）与钉在那个节点的 Pod，最后交回 Pod 与 Secret 的实例，执行环境进入「启动中」。过了建出宽限（2 分钟）仍在排队的由 task-runtime 的对账判准备失败；排队时资源中心写了建不成的原因，阶段照原因显示「分配中」，不再是「排队中」。清理照旧：task-runtime 的作业按受理意图注解与所属工作区标签认领后删，调和器也照「不要了」删。受理意图的摘要加了一个与键先后无关的写法——投影可能用受理时内存里的对象，清理时比对的是库里读回的（jsonb 会重排档位对象的键），旧摘要两边算不出同一个值，清理会拒绝删资源中心建的 Pod；比对时新旧两种都认。工作卷观测到了随即唤醒等它的工作区（第一步里要等 2 秒重试）。

> **实施补记（2026-09-24，I25 第三步：检出用的 Git 凭据）**：开发会话检出源码用的只读 Git 令牌，此前在受理时就签好，由组合根直接写进项目命名空间按服务共用的 `git-checkout-<服务 ID>`，没有记录认领。由资源中心建出的会话改为每次启动一个凭据 Secret（`<Pod 名>-checkout-<第几次启动>`，键 `token`，不可变，只挂给 checkout init 容器），作为工作区记录的子对象：受理只要仓库地址，调和器在 Runner Secret 之后、Pod 之前建它，令牌此刻经端口 `checkoutValues` 向 task-runtime 要（task-runtime 再经检出端口向 scm 签，30 分钟），值不落库；会话释放时随记录删掉。已受理的旧形状环境照旧用按服务的 Secret；以后不再写它，留下的由收编（T14）处理。

> **实施补记（2026-09-24，I25 第四步：档位测试）**：管理员的档位测试（系统命名空间里的一次性 Pod）同样由调和器建出：受理只登记，期望带档位镜像与资源、Pod 内的临时目录（`emptyDir`，没有工作卷，所以不等卷），Runner Secret 归这一次启动；要值时档位测试用平台自己的服务（不属于任何项目，照旧不带租户配置与数据）。此前 Runner 令牌与平台变量以明文环境变量写在系统命名空间的 Pod 规格里，现在只在不可变的 Secret 里。等 Runner 的轮询里，记下 Pod 实例之前、建出宽限之内的「Pod 不存在」是还没建出来，不再立即判「测试容器没有起来」。并发上限照旧用平台的档位测试计数。失败会话的重建（按 UID 与 resourceVersion 删掉旧 Pod、等它退出、等子执行回收、失败时补偿）与逐条调和的模型对不上，仍由 task-runtime 建，单独设计。
...
>
> **实施补记（2026-09-24，第三期第八步：服务槽由资源中心建出）**：按 [I25](../../../docs/engineering/implementation-open-questions.md#i25-调和器接手建任务容器时凭据放在哪) 同一裁定（渲染时向所属模块要），服务槽的 Deployment、Service 与这一次部署的环境 Secret 改由调和器建出。部署与重新部署只把期望随槽状态写进台账：槽记录的 `spec.slot` 带镜像、启动命令、端口、健康检查路径、副本（已含运维覆盖）、资源、环境 Secret 名（`<服务>-<物理槽>-env-<第几次部署>`）与运维重启标记，不含配置与密钥；子对象加上这个环境 Secret。
>
> - **建**：调和器先建不可变的环境 Secret——内容此刻经端口 `slotEnvValues` 向 release 要（平台约定变量、生产组配置与数据连接串，值不落台账；槽上已不是这一次部署时拒绝），这一次实际用上的配置版本记回发布（只改这一列）；再按期望应用 Service 与 Deployment（与 release 自己部署时同一个构造函数，挪到 `@crewstation/k8s`，选择器建了就不能改，两边必须一样）。Deployment 只从这个 Secret 引用环境，注解 `crewstation.io/resource-generation` 写渲染它的期望版本（记录的 generation）；被人改了（副本、镜像、标签）观测到就改回。新版本铺完、副本都就绪之后，删掉换下来的旧环境 Secret。
> - **判铺开**：Deployment 的观测带上注解里的期望版本（子对象的 `appliedGeneration`，契约只增）。流水线不再读 Deployment，照槽记录判：观测到的 Deployment 是渲染最新期望的那个、记录运行中才算就绪，推进超时判失败，其余（启动中、崩溃重启、副本没全就绪）等部署时限——期望刚改、还没应用时旧版本的「都就绪」不再被当成新版本的（直接读 Deployment 时也有这个窗口）。
> - **建不成**：调和器写只归资源中心的条件 `Created` 为假，并经端口 `slotFailed` 报给 release：槽记为失败、记下原因，流水线下一步据此判发布失败（发布记录只由流水线写，不与它并发改写），原因进发布记录与槽记录的 `Failed` 条件；与自己部署时 apply 失败即判失败一致。流水线已判失败的槽调和器不再改，留着诊断。
> - **下线**（手动、到期、集群管理删除）：`Serving` 为假时调和器先按 UID 删 Deployment，它消失之后再删环境 Secret（下线的槽不留生产配置与密钥），Service 保留；release 不再自己删工作负载，期望留着——调和器据此知道删哪些，下次部署接着数第几次。
> - **集群管理的槽运维**（C6「执行改为写期望」）：扩缩与恢复写副本数，重启写重启标记（Pod 模板注解，一变就重新铺开），删除即下线，都在受理的同一事务里改期望；完成与否照槽记录判（新期望铺完、副本数对上；删除看 Deployment 没了）。直接改 Deployment 的旧做法对这些槽不再适用——调和器会把它改回。清单里槽的 Service 与环境 Secret 是「维护中」、禁用直接删除（种类注册表的 `renderedChildren`）；Deployment 的删除仍是 release 的下线。
> - **预检**：重新部署的集群 dry-run 改经 cluster-control 的端口 `dryRunSlot`（上文「渲染移交资源中心后改经它的端口」）：按要写进台账的期望（下一次部署）渲染出与调和器相同的三个对象，以服务端 dry-run 提交。
> - **旧形状**：上线之前部署的槽（期望里没有 `slot`）照旧由 release 读 Deployment、自己删，集群管理的运维照旧直接改 Deployment；下一次部署或重新部署时换成新形状，不在上线时批量重铺。运维开关 `CS_RELEASE_CREATION=owner` 回退为 release 自己建（迁移期的回退手段，同时管构建与迁移 Job）。
> - 构建与迁移 Job 见下一条补记。
>
> **实施补记（2026-09-24，第三期第九步：构建、迁移 Job 由资源中心建出）**：同一裁定。流水线先把发布推进到构建中／迁移中，再在 Job 记录里写期望（`spec.job`：镜像、命令、不含凭据的明文变量——构建的仓库地址、标签与镜像名——资源、截止时间与 TTL、这一次的凭据 Secret 名 `<Job 名>-env`），子对象加上这个 Secret；发布记录里记下这次发布的 Job 由资源中心建（`pipeline.jobs`）。调和器先建不可变的凭据 Secret——内容此刻经端口 `jobEnvValues` 向 release 要：构建是这一次签的只读 Git 令牌（不再写按服务共用的 `git-cred-<服务>`），迁移是槽的环境（与部署同一套渲染）；发布已不在这一步时拒绝——再建只从它引用凭据的 Job（标签与原来相同）。Job 建过就不再建：观测到它就补记 `Created`，之后被 TTL 或人删掉也不补建。Job 结束（`Finished`）或流水线已放弃（领域条件 `Failed`）就删掉凭据 Secret，Job 交给 TTL，构建日志在那之前照常可读。流水线照 Job 记录判结果：`Finished` 定成败，没建成（`Created` 为假，Job 记录每次发布一条，没有上一次留下的条件）即失败，说法与原来相同（「构建失败：…」「迁移失败，未切流：…」）；Job 一直没建出来（调和器停着）时按截止时间加 5 分钟兜底判超时。没建成与超时给 Job 记录报 `Failed`。期望没写进台账（那一次投影失败只告警）时下一轮再写一次。已按旧形状启动的发布照旧直接读 Job。运维开关 `CS_RELEASE_CREATION=owner` 同时回退 Job。
