...
| RFC-025-T10 | 三期·限流：平台设置与项目覆盖（`gateway` 校验并写 `rate-limit-policy`）；中间件渲染；工作台与命令行的 429 处理；能力说明 MCP 文案 | T2、T9 | 进行中：策略的存取与管理接口（平台默认与项目覆盖，按版本号乐观并发，只给管理员，cef5b880）；策略写成 `rate-limit-policy` 记录（平台一条、每个项目一条），调和器照记录渲染 Traefik Middleware。路由挂中间件、界面与 429 处理在后 |
