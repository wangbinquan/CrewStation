...
| RFC-025-T8 | 二期·服务槽与构建：`service-slot`、`build-job`、`migration-job` 的期望与调和；统一预检接入发布、重新部署与切流；`observability` 健康、发布页槽卡改读记录 | T5 | 进行中：服务槽投影进台账（两条稳定记录，Serving 条件表达下线与尚未部署），调和器观测 Deployment（4aeaa892，已部署）；槽的旧接口状态就绪后照台账、铺完后副本没全就绪判降级（6d54a25f）；形态图槽带的入口与 Deployment 状态照槽记录。健康卡改读记录要先把槽的 Pod 作为观测到的子对象入账（崩溃重启判定要重启次数），与构建、迁移 Job 与统一预检在后 |
