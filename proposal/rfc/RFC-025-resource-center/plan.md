...
| RFC-025-T8 | 二期·服务槽与构建：`service-slot`、`build-job`、`migration-job` 的期望与调和；统一预检接入发布、重新部署与切流；`observability` 健康、发布页槽卡改读记录 | T5 | 进行中：第一步服务槽投影进台账（两条稳定记录，Serving 条件表达下线与尚未部署），调和器观测 Deployment |
