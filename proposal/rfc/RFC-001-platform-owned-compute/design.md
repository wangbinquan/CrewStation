...
本机端到端复跑（2026-09-12，`39c8e36`）暴露了三处本文没预料到的连带后果，都已修：

1. **一次性改掉契约，老仓库就全失效，而修它的路也被堵住了。** 开发会话开的时候硬解析 Manifest，解析不过就不给开——偏偏开发容器正是改 `crewstation.yaml` 的地方。现在会话照开，只是没有预览，并把原因与改法写在页面上；发布仍然硬拒。**下次再做「无兼容期」的契约变更，先问一句：改它的那条路径自己会不会被这次变更挡住。**
2. **只说错在哪不够，要说改成什么。** zod 的 `Unrecognized keys: "driver", "model"` 定位准确但没有出路。`describeManifestFailure` 让发布与开会话共用一套说法，直接给出 `compute: <档位名>` 与去哪儿看可用档位。
3. **改了 `runtimes/task` 但集群里跑的还是旧镜像。** `deploy/local/install-platform.sh` 原本「镜像存在就不重建」，于是 `started` 事件里少了 `compute` 字段而代码看着完全正确，查了一轮才定位。已改为默认重建。

另有一件本 RFC 不解决、需另议的：开发容器里 worker 的 `HOME` 就是 `/work`（`runtimes/task/Dockerfile` 的有意设计，两个 CLI 都要可写 HOME），于是 CLI 缓存落进 git 工作区，发布前置检查把平台自己产生的文件当成「未提交的更改」而拒绝发布。模板的 `.gitignore` 先挡住，`HOME` 该不该是 `/work` 留待专门处理。
