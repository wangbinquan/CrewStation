...
2026-09-23 再修订（作者当面裁定：连接就绪前整页 loading，见 [RFC-003 开发工作区 §2](../RFC-003-workbench-ux-redesign/development-workspace.md)）：上面两条在工作区第一次揭开之前改为整页中间显示，页头芯片随工作区一起在就绪后才出现；启动失败时保卷恢复卡紧跟在步骤条下面，等待连接失败不再单给打开恢复的「重试」。进来时就失败、协议拒绝或正在释放，整页是状态卡而不是工作区加提示条。就绪之后的断线、失败与重建仍按本节，工作区与草稿都留在原处。
