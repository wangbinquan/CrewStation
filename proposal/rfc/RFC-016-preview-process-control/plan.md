...
按仓库规则，实现中发现的设计缺口记入 `docs/engineering/implementation-open-questions.md`，不就地裁定。

- **I21（实现中发现）：Agent 能观测预览，却调不到它。** RFC-006 之后 Agent 跑在自己的执行 Pod 里，`127.0.0.1:<预览端口>` 到不了预览进程，而 `url` 指向的用户域主机有 ForwardAuth 登录。于是 Agent 能把预览救活、能读它的输出，却没法验证「改完之后页面真的对了」——闭环缺最后一步。工具描述已按事实写明，不让 Agent 去撞回环地址。三个选项见 I21。
