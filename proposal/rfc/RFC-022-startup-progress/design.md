...
### 14.4 修订：终端查询改由 Runner 应答（RFC-026，2026-09-23）

上文 §14.1 第 12 条「首帧由 CLI 自己决定……创建者窗口已持有控制，查询在同一刻得到应答」与 D1（创建者提前取得控制）的前提——只有持有控制的窗口应答终端查询——
由 [RFC-026](../RFC-026-runner-terminal-query-replies/proposal.md) 改变：新 Runner 的无头终端统一应答，浏览器拦下查询；创建者提前取得仍保留，但不再是 CLI 画出界面的前提。
