...
状态：In Progress · 2026-09-22。作者先裁定「这个能力可以下掉，不需要有这个约束」，在看过 §5 能力影响清单与 §4 两个问题后答复：「批准，并且本次就把功能全部下掉，历史的数据也清理掉，不要残留」。据此：§5 八项全部确认，Q1 取方案 C，Q2 改取方案 b（本次即清理 `egress` schema 与迁移记录）。本 RFC 属于开发规则 §5.5 的能力收缩型 RFC。
