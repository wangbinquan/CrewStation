...
状态：Draft · 2026-09-22。作者在了解「出站白名单」页面的用途后裁定「这个能力可以下掉，不需要有这个约束」。本 RFC 属于开发规则 §5.5 的能力收缩型 RFC：§5 逐项列出被关闭的能力，§4 列出仍需作者裁定的两个问题；作者逐项确认前不改代码。
...
| E2 | 待裁定（§4 Q1） | 服务槽 Pod 的出向网络范围 |
| E3 | 待裁定（§4 Q2） | 遗留 `egress` schema 与迁移记录的处理 |
...
**Q1 服务槽 Pod 的出向范围。** 今天数字人与接入容器的服务槽 Pod 只能到平台系统命名空间与 DNS。

| 选项 | 内容 | 影响 |
|---|---|---|
| C（建议） | 只给接入容器（项目 `kind` 为 `APIProxy`／`EventProducer`）的服务槽放开出向；数字人服务槽维持现状 | 接入容器本来就是「代公司系统转发」的平台项目，放开与其职责一致；业务服务访问公司系统仍须经目录与放行表，R15「不能绕过网关放行」对内部 API 继续成立 |
| A | 项目命名空间内所有 Pod 出向全放行 | 最简单；但业务服务可绕过 API 目录直连公司系统，R15 对内部 API 只剩约定 |
| B | 保留 `/internal/egress/http` 通道，只去掉白名单判定 | 不建议：约束没了，却留着为它而生的通道、限额与模块 |

**Q2 遗留数据。** `egress` schema 里有条目、申请、被阻记录三张表，`platform_infra.migrations` 里有该模块的四行记录。

| 选项 | 内容 |
|---|---|
| a（建议） | 保留 schema 与四行记录，本 RFC 与 dev-gotchas 记为已知残留；日后由运维在维护窗口手工 `DROP SCHEMA egress CASCADE` 并删除四行记录 |
| b | 本次本机部署时就执行上述两条语句（先做停写备份），生产升级说明写明同一步骤 |

## 5. 能力影响清单

每一项都是 breaking change，请逐项确认。
...
| 7 | `egress` 模块（L3）及其 PostgreSQL schema | `modules/egress` 全部 38 个文件（含 README、package.json 与 `.gitkeep`）；`task-runtime`／`platform` 的依赖声明；`tools/arch/migrations.lock.json` 四条；`platform_infra.migrations` 四行；仓库结构 §5 清单与分层图 | 删模块；锁条目按 testing.md §7 手工退出并在提交说明写明原因；schema 按 Q2；ADR-0008 记录模块退役 |
| 8 | 服务槽网络范围 | 每个项目命名空间的 NetworkPolicy；现有项目的策略要重新下发 | 按 Q1 改策略；cs-controller 启动时对全部未归档项目重跑命名空间步骤 |
...
| EG-03 | 新开通项目的 NetworkPolicy 符合 Q1 裁定；控制面启动重下发后，本机全部现有项目命名空间一致（`kubectl` 逐个核对） |
...
| EG-07 | 基线三件套 v0.3.7、tech-evaluation、仓库结构 v0.5＋ADR-0008、I9 关闭、集成 README／CONTRIBUTING、e2e 页面清单全部更新，`dev-gotchas` 的网络策略条目按 Q1 改写 |
