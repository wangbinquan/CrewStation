...
2026-09-24 修订：作者裁定「都放开外网」，数字人服务槽也放开出站——Q1 改取方案 A 的出向部分，任务、构建、接入三条策略保留不删（基线 Design D64，见 §4 末与 EG-09）。
...
| E5 | 作者 2026-09-24 裁定「都放开外网」 | §4 Q1 改取方案 A 的出向部分：默认策略出向全放行，数字人服务槽与迁移 Job 也能直连外部；任务、构建、接入三条策略保留不删（§4 末修订） |
...
> **2026-09-24 修订（作者裁定，直接修改＋回填，基线 Design D64）：Q1 改取方案 A 的出向部分。** 作者问「现在业务服务、业务执行容器都🈲外网访问吗」，得知只有数字人服务槽（以及发布时的迁移 Job）出不去、业务任务与它的 Agent 执行容器全放行之后，裁定「都放开外网」。问答中作者在「只改默认策略」「并删掉多余策略」「只放公网（私网段仍挡）」三种里选了只改默认策略：`crewstation-default` 的出向改为 `[{}]`，项目命名空间里所有 Pod 出向不限制，入向不变；任务、构建、接入三条按标签放行的策略保留、不再起作用，因为资源中心的调和器对网络策略只建、只改回、从不删，删它们要另给调和器补删除。上表方案 A 所写的代价随之成立并由作者接受：业务服务可以绕过接口目录直连公司系统，R15 对内部 API 只剩经网关调用这一段的约束（基线 R15 同步改写，AT-07 第一句作废）。
...
| EG-09 | 2026-09-24 修订（D64）：换版后本机全部项目命名空间的 `crewstation-default` 出向为 `[{}]`、入向不变；数字人服务槽按 IP 与按域名都能出公网；别的项目命名空间的 Pod 仍连不进来 |
