...
1. **`gitNameClaim`／`emailClaim` 的消费方**。agent-workflow 用它们定 Git 提交身份。本仓开发容器今天**不设** per-user git 身份（任务镜像只装了 git，`runtimes/task/Dockerfile`），容器内 `git commit` 要靠人自己配。建议：登录时解析出的 Git 名与邮箱存进用户档案，开发会话容器启动时注入 `GIT_AUTHOR_*`／`GIT_COMMITTER_*`，让这两个配置项有真实消费方。若不做，字段仍按 §6.3 保留并落库，但不驱动任何行为。
2. **`x-cs-auth-method` 头的登记位置**。为了让 cs-api 知道「当前会话是密码还是 OIDC 建立的」（§6.1 的关闭前置条件），ForwardAuth 需要在**仅工作台目标**的请求上注入这个头。它要进 `drop-identity-headers` 与 `authResponseHeaders`。建议登记为平台内部头（不进 `IDENTITY_HEADERS` 这张业务接入约定表，业务服务也收不到）。
