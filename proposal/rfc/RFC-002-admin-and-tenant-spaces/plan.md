...
- [ ] 非管理员：顶栏无空间切换控件，左栏无任何管理入口
- [ ] 非管理员访问 `/admin/*` → 拒绝页（非 404、非空白、非静默跳转），含回工作台链接
- [ ] `/v1/me` pending 时不闪现内容也不闪现拒绝；请求失败时显示错误而非拒绝
- [ ] 管理左栏含用户与权限、算力档位（占位）、服务套餐、任务容器套餐、接入容器、出站白名单、网关
- [ ] 租户项目列表不含接入容器；管理空间的接入容器页列出它们
- [ ] 普通成员带 `kind=APIProxy` 请求也拿不到别人的接入容器
- [ ] 空间往返保持项目上下文
- [ ] 十个既有页面的功能与路径不变
- [ ] `bun run check` 全绿，CI 绿
- [ ] `proposal/rfc/README.md` 状态改 Done 并附 commit
- [ ] `STATE.md` 更新
