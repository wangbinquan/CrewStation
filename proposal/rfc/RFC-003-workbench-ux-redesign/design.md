...
管理有界读取契约：API 申请和出站申请各增加 `/page` 查询，兼容原全量端点；按项目与裁定状态先筛选，再按创建时间及 ID 倒序取默认 20、最多 50 条。游标绑定账号、管理员身份、项目与状态，翻页不依赖锚点仍处于原状态。当前页关联项目通过 project 公开批量基础查询取得名称／类型，最多 50 个 ID，一次本模块 SQL；名称读取失败保留申请与真实项目 ID，不假造对象。总览每类待办只读取前五条，说明当前范围并导航到实际管理页，数量不是全局总数。审批页和项目管理页以 URL 条件及服务端游标分页；管理项目列表保持管理员上下文，接入容器仍只包含 APIProxy／EventProducer。
