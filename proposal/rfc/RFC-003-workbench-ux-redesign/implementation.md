...
> **2026-09-21 修订（作者当面裁定，直接修改，不另立 RFC）。** 上一条的「刷新期间不保留旧授权卡片」实机表现是每 15 秒把整片卡片网格换成一行「载入中」：
> 内容塌掉后浏览器把页面滚动位置夹回顶部（视口装不下时滚动的是文档本身：`AppShell` 用 `min-height: 100vh`，`main` 会长到内容高度），看起来就是页面自己刷新了一遍。作者要求「就算刷新也是部分更新」，裁定改为：
> 例行刷新在途保留当前卡片、回执到达后原地替换（同一批 DOM 节点，不卸载）；**读取失败或响应表明撤权时仍立即撤下旧卡片与入口**，该性质不变。
> 实现去掉 `useMarketQuery` 的 `!query.isFetching` 判据，`MarketPage` 的载入态只看 `isPending`；回归见 `appMarket.test.tsx`「例行刷新在途保留当前卡片」。
...
`bun run check` 于 2026-09-16T07:42:52Z 通过，**1342 pass／4 skip／0 fail**（1346 tests／232 files，131.52s），arch、lint、两处 typecheck 无告警。本批提交 **ed65e23d2005bba1b0188a38ad7b9def4066a02f**（22 files，+128／−29），[CI 35069981665](https://github.com/wangbinquan/CrewStation/actions/runs/35069981665) 于 2026-09-16T07:46:06Z 成功。

## 第八十七批：数据访问申请与审批的浏览器旅程

开发者与负责人两个真实上下文同看 `rfc003-verify-workbench` 的开发页，全部经页面控件操作（副本在 `/private/tmp/crewstation-rfc003-batch84/`）：

- 开发者展开“数据访问”，选择“生产数据只读”、有效时长 5 分钟并提交，页面立即写入“已受理”并把该条记为“待批准”；开发者这一侧没有批准／拒绝／撤销按钮（`canManage` 为假）。
- 负责人展开“数据访问 · 1 项待审批”，打开该记录的“详情与处理”，点“批准”后出现 `role="alertdialog"`“批准「生产数据只读」申请（tdb_…134c1834）？”，焦点在面板内（第八十四批的确认面板修复生效），附审批意见字段；“确认批准”后记录变为“访问凭据已供给 · 接入待确认”，摘要为“已批准：生产数据只读 · 接入待确认”，到期时间为批准后 5 分钟（15:53），与申请时长一致。
- 开发者刷新后看到同样的状态与摘要；负责人再“撤销绑定”→“确认撤销”（问题句写明模式与绑定 ID），三条记录都为“已撤销”，开发者刷新后一致。两侧全程 0 console error。
- 顺带：开发者打开“应用可见性”只看到当前范围与“只有负责人或管理员可以修改”，没有保存按钮（UX-AT-48 的只读部分）。

UX-AT-38 记为通过，累计 **38／52**。旅程之外：申请理由字段是 `textarea`，i18n 里的 `devSession.data.reasonPlaceholder` 未被使用，属可整理项，不影响验收。本批无生产代码改动。
...
`bun run check` 于 2026-09-16T08:04:20Z 通过，**1344 pass／4 skip／0 fail**（1348 tests／233 files，132.86s），arch、lint、两处 typecheck 无告警。本批提交 **b70a92f0fcbcbb59c9213aef8bf77cb3b3cb4b38**，[CI 35071877706](https://github.com/wangbinquan/CrewStation/actions/runs/35071877706) 于 2026-09-16T08:08:39Z 成功。

## 第八十九批：定向开放申请的完整浏览器往返与管理员撤销

开发者（`rfc003-developer`）与管理员两个真实上下文，对象是 `rfc003-verify-workbench` 的开发资源目录与 `/admin/requests`、`/admin/capabilities`（副本在 `/private/tmp/crewstation-rfc003-batch84/`）：

- 开发者在目录里看到 `test-gitlab:GET:/v4/projects/{id}/repository/tags`“定向开放 · 未授权 · 申请定向开放”，填写理由提交后该行变为“待审批”（按钮收起），“本服务的定向开放申请”列出申请人、时间与理由。
- 管理员在“申请审批 · API 申请 · 待审批”看到带项目名、slug、服务 ID 与“查看此项目申请”入口的记录，写下意见“验收拒绝分支：请写明只读范围与调用频率后重新申请。”并拒绝，记录从待审批列表消失。
- 开发者刷新后该行恢复“申请定向开放”，申请列表显示“已拒绝”、审批人、审批时间与管理员意见原话；补写范围与频率后重新申请，再次“待审批”。
- 管理员批准并写意见，“已批准”列表可见；开发者刷新后该行为“定向开放 · 已可调 · 试调”，申请列表显示“已批准”与批准意见。
- 管理员在“能力接入 · API”选择调用方“RFC-003 新工作台验收”（URL 带 `projectId`），该行显示“已可调 · 改为默认开放 · 撤销授权”；点“撤销授权”出现行内确认“撤销「RFC-003 新工作台验收」对 … 的定向授权？”，确认后提示“… 的定向授权已撤销；默认开放的接口仍可调用。”，行变“未授权”；开发者刷新后回到“未授权 · 申请定向开放”，两条申请记录保留为历史。四个页面全程 0 console error。QA 服务的授权状态已恢复到旅程前。

UX-AT-16（拒绝→读理由→重申→批准的浏览器前后切换与原位置状态）与 UX-AT-50（消费与管理页面的完整浏览器交互；协议与运行链路见第五十三批）记为通过，累计 **41／52**。观察：申请列表与审批页把申请人／审批人显示为 `usr_…` 原始 ID，而成员页显示可辨识名字，属后续可整理项；开发资源目录页的 `h1` 是“项目设置”，与左栏“开发资源”入口不完全对应，一并记下。本批无生产代码改动。

### 门禁

`bun run check` 于 2026-09-16T08:13:17Z 通过，**1344 pass／4 skip／0 fail**（1348 tests／233 files，131.34s）。本批提交 **cb0df4e6f721c2747482fee248a68fc0b987e1d6**，[CI 35072636758](https://github.com/wangbinquan/CrewStation/actions/runs/35072636758) 于 2026-09-16T08:16:25Z 成功。

## 第九十批：负责人切流时另一使用者的页面与过期确认

owner 与 admin 三个真实页面（副本在 `/private/tmp/crewstation-rfc003-batch84/`），对象仍是 `rfc003-verify-workbench`（正式 v0.1.0、待验证 v0.1.1，两个版本源码 SHA 相同）：

- admin 先在发布页点“检查上线／回退至 v0.1.1”并停在确认面板（问题句“正式版本 v0.1.0 → v0.1.1”，按钮“确认上线 v0.1.1／取消切换”）。
- owner 在自己的发布页检查并“确认上线 v0.1.1”，页面写入“已登记正式版本切换到 v0.1.1。网关异步应用，待实际访问确认生效。”，两张版本卡当即互换（正式 v0.1.1／待验证 v0.1.0），`/slots` 中 `active` 随之翻转；正式主机仍返回样例页。
- UX-AT-12：admin 那张停留的面板在 owner 上线后自行显示“确认后的部署已变化或无法读取，请重新核对两个版本。”并禁用确认按钮（发布页每 5 秒核对部署记录），没有发出第二次切流；“重新核对两个版本”后面板变为“正式版本 v0.1.1 → v0.1.0 · 确认回退至 v0.1.0”，admin 取消。页面展示新状态、保留说明并可重新确认的恢复路径成立。
- UX-AT-11 正常回退：owner“检查上线／回退至 v0.1.0”→“确认回退至 v0.1.0”，写入“已登记正式版本切换到 v0.1.0”，`active` 回到 v0.1.0；两次切流记录进入切流历史。破坏性迁移版本在浏览器里的拒绝分支仍未做。
- UX-AT-31：第一轮 admin 开发页页头 90 秒仍是“生产部署: v0.1.0”，查明是无头浏览器里非前台目标 `visibilityState` 为 hidden、比较查询按设计“隐藏页面不轮询”；把 admin 页面置前台重跑一轮（再上线并回退）：上线后 7.6 秒页头变为“生产部署: v0.1.1”，回退后 7.0 秒回到 v0.1.0（查询每 10 秒核验一次）。另一使用者操作后可见页面及时刷新成立。

UX-AT-12、31 记为通过，累计 **43／52**；UX-AT-11 的正常回退在浏览器完成，破坏性版本的拒绝分支保留。四次切流全部由页面控件完成，正式版本最终仍为 v0.1.0。本批无生产代码改动。
...
开发者（`rfc003-developer`）在 `rfc003-verify-workbench` 的开发资源目录里，对已获定向授权的 `rfc003-integration-qa:GET:/v4/projects`（第五十三批批准）点“试调”：操作试调卡片出现并自动绑定当前运行中的开发会话（“试调会话 tsk_01a09eb4f03f7000ba011a517772cc09”），路径参数为空，查询参数填 `{"per_page":"2","simple":"true"}`，请求头与请求体留空（GET 不发送请求体），“发送请求”后“最近一次试调响应”显示 **HTTP 200、容器内实测 142 ms**、操作键、“本次响应所属会话 tsk_…”、响应正文 `[]`（本机 GitLab 对匿名令牌返回空项目列表，与第五十三批接口结论一致）以及响应头与“本次发送的请求”两栏。请求经原开发容器以当前服务身份发出，页面全程 0 console error。

UX-AT-15 记为通过，累计 **44／52**。本批无生产代码改动。
...
## 第九十四批：失败分支的完整浏览器实机（破坏性回退拒绝、死信重放、局部来源故障、浅／无共同历史、查询失败恢复、状态未知）

第九十三批之后剩 7 项验收需要故障注入或专门历史构造。本批用无头 Chrome（一浏览器、每身份独立上下文）在当前部署上把其中 6 项走完整旅程，均以真实故障、真实恢复、真实数据为证；没有任何生产代码改动（部署上的现有行为都正确，无需修复）。

- **UX-AT-11 破坏性迁移的浏览器回退拒绝**：admin 在 `rfc003-verify-delivery` 发布页对 v0.1.2（含破坏性迁移）点“检查上线／回退至 v0.1.1”，确认面板显示 `v0.1.2 → v0.1.1` 与两个 SHA；确认后被准确拒绝——页面弹出“当前版本 v0.1.2 含破坏性迁移，不能切回旧版本 v0.1.1……系统不会自动重发……请发布修复版本”，面板关闭、正式仍 v0.1.2、待验证仍 v0.1.1、槽位不变；再次打开确认面板并取消、刷新后卡片一致，无控制台错误。（此前正常回退已在 E63 浏览器完成。）
- **UX-AT-14 死信投递、追查与重放**：向内置 GitLab producer（本批先由 admin 在管理空间把其正式从“尚未部署”上线到 v0.1.3）POST 一条真实 `Push Hook`（带 secret token 与 event-uuid），受理 202、扇出 7 条投递；同时把 `rfc003-verify-workbench` 生产 Deployment 缩到 0，使本服务投递持续 `HTTP 404`。owner 的运行与诊断→事件投递页实时看到该投递 `重试中`，attempts 由 4→5、下次重试时间推进，无需刷新；退避重试至第 8 次落 `死信`（后台 SQL 观测 attempts 2→8、退避 next_attempt 递增）。owner 从死信行点 `trace_id` 跳到“调用链回放”页（按该 trace 查询，如实显示“没有可用的关联记录”，因该事件未派生平台任务）；把生产槽恢复到 1／1 后，owner 点“重新入队”，该投递离开死信过滤、在“已投递”过滤下显示 attempts 归 1、送达时间 18:05。事后核对：本服务及 demo／rfc003-ux／delivery／files 五条均 `delivered`；producer 与 reference-api-proxy 两条 `dead`（这两个接入容器订阅了 gitlab.push 但其应用类型没有 `/events/gitlab` 消费处理路径，属既有配置，非本批引入）。
- **UX-AT-27 局部来源读取故障不掩盖其他条目**：把 `rfc003-verify-files` 的服务槽记录改成缺字段的 JSON，使该服务的槽位读取失败。项目列表里该项目正式／待验证均显示“状态暂不可用”，其余四个项目仍完整正常；市场列表该卡片为“正式状态未知 · 正式入口暂不可用”、其余四个仍“已上线”；市场详情同样“正式状态未知 · 正式入口暂不可用”且不给 openLink（不跳 preview、不冒充地址）；项目概览两槽“状态暂不可用”。把记录按快照原样恢复后，全部回到“已上线 · v0.1.8”、openLink 复原；`files-slots` 与快照逐字节相同，无控制台错误。
- **UX-AT-30 浅历史与无共同历史分开呈现**：在开发容器里给 `/work` 真实写入 `.git/shallow` 造出浅克隆，版本比较条显示“暂不可比较 / 浅克隆历史不足，请补齐历史后重算”，未提交如实、未推送与上游“未知”，并给出“补齐历史并重算”入口；`git checkout --orphan` 造出无共同历史分支后显示“无共同历史”。三态（浅历史、无共同历史、未部署）分开，不用远端分支冒充 HEAD。补齐历史的实际拉取需要验收容器不具备的远端凭据（`could not read Username for host.docker.internal:8929`），故只验证到检测、如实文案与恢复入口；手工移除 `.git/shallow`、删除孤儿分支后比较恢复“工作树领先 待上线 1”，工作树回到 `main`、`1aa2db9`、非浅、仅两个既有未跟踪文件。
- **UX-AT-48 可见性查询失败的恢复**：用 CDP `Fetch` 拦截使 `GET …/app-visibility` 连接失败，可见性设置页显示“读取失败：Failed to fetch”，不出现表单、不谎报“无应用”；仍在失败时点“读取最新设置”只显示“载入中…”后继续失败，不谎报成功；停止拦截后经窗口聚焦重查，恢复出完整表单（可见范围、展示资料、保存／取消、“已保存第 8 版”），无控制台错误。（真实并发冲突与开发者只读此前已完成，见 E60。）
- **UX-AT-49 市场“状态未知”分支**：承 UX-AT-27 的注入，市场详情对该应用显示“正式状态未知 · 正式入口暂不可用”、openLink 为空、不跳 preview；恢复后为“已上线 · v0.1.8”、真实正式主机 `rfc003-verify-files.cs.localhost`。加上此前已实看的“已上线”“尚未上线”，市场三类齐备。

累计 **51／52**。唯一未做完整独立实机的是 **UX-AT-42 的“乱序补发”**分支：其去重、断线补发、状态源不可用已实证（见 E42／E57），乱序缺口的对账为纯领域规则并有精确单测（`nativeActivityProjection.test.ts`：源序号跳号 → `source:'unavailable', sourceReason:'channel-gap'`，且其后 `source-ready` 不恢复），其可观察结果与已实证的“状态源不可用”完全一致（活动源变为 unavailable、不显示成功、工具完成不等同整轮完成）。要在浏览器再造一次真实乱序需向当前活跃的验收会话（13 个 agent、4 个在跑的 CLI）注入并回滚整段动态投影，风险与既有证据不成比例，故不做。

### 环境处置

本批为验证批，无生产代码改动。处置：`rfc003-verify-workbench` 生产 Deployment 恢复 1／1、两槽 ready；工作树恢复 `main`／`1aa2db9`／非浅、无孤儿分支；`rfc003-verify-files` 服务槽按快照原样恢复（逐字节一致）。内置 GitLab producer 的正式版本本批由“尚未部署”上线到 v0.1.3（为把 webhook 打进 cs-events；这是其首次正式上线，属正常操作，保留）。发出的真实 `gitlab.push` 事件按订阅扇出，五个数字人 `delivered`、两个接入容器 `dead`（既有订阅配置）。
...
`bun run check` 于 2026-09-16T15:12:30Z 通过,**1344 pass／4 skip／0 fail（1348 tests／233 files）**,arch、lint、两处 typecheck 无告警。本批提交 **b2f147885a022baf343a66e486e1d991bc5ee56e**,[CI 35113904469](https://github.com/wangbinquan/CrewStation/actions/runs/35113904469) 于 2026-09-16T15:16:51Z 成功。
