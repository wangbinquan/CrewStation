...
> 2026-09-15；第七十七批已上库、CI 成功并部署；真实四个独立 OpenCode 运行，两个模型轮次并行，单窗 OOM 只回收自身。第七十八批紧凑界面与运行失败文案已通过门禁并更新本机 console（generation=53）。累计仍 27／52；四窗最终量测、新增失败恢复及版本比较转发缺陷继续，RFC-004 按顺序等待。

## 1. 判据和环境

本表区分源码／自动验证、隔离原生进程、历史浏览器记录和当前完整实机旅程。自动测试覆盖一条接口或路由，不等于使用者在当前部署完成整条旅程。当前 UX-AT-01／02／03／04／05／06／07／08／10／13／17／18／19／20／21／23／24／25／29／32／33／34／36／39／40／41／52 二十七项已取得本次实机通过证明；其余 25 项仍有未执行的角色、失败分支或完整旅程。不把有测试文件或一个绿门禁计算为 52 项验收通过。
...
| UX-AT-28 | E2、E19、E49，单次启动和幂等；真实四窗 OOM 与 I15 隔离方案 | I15 已部署，真实四窗与单窗 OOM 隔离通过；其余窗／原卷保持；新增失败重试和幂等的完整浏览器条件继续 |
