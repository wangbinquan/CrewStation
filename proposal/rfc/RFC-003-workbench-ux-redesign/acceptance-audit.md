...
> 2026-09-15；共享 console 保持 `cbe28250607aa4084d74556c32aff650a9347783`，cs-api／cs-controller 已更新为 `e26515eb6dbb67fc5cf894bb9996b1885144e788`，精确 SHA CI、实际 imageID 和源码核对通过。第五十二批进一步修复同一发布事件的旧计划迟到覆盖，完整本地门禁通过、上库部署继续。参考代理 v0.1.2 已正常上线，已授权只读请求实际到达正确代理，但访问 GitLab 被现有出站规则阻断，I9 待作者裁定。CUA 已恢复、原管理员已登录回 files 工作区，发布计数实际显示未提交 0／未推送 0；页面旅程继续。四窗 OOM 与保卷恢复仍未解决，节点容量仍有限。任务和标准沿用 [plan.md](plan.md)，没有删减范围。
...
- E25 发布消费者并发与失败重试：[implementation 第五十二批](implementation.md#第五十二批发布登记投影的并发迟到)。可控暂停旧请求复现迟到覆盖，发布投影统一由目录消费者完成后触发；K8s 失败后同一事件可重试恢复且无重复操作。新增两个组合根回归与完整本地门禁通过，真实部署另行核验。浏览器已经恢复，files 计数界面复验通过，但不代替 UX-AT-32 的全部文件类型分支。
