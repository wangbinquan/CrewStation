...
> 2026-09-14；共享 console 为 `10455cc61faea17a88ca3216a7db27f2e813b5eb`，cs-api／cs-controller 为 `03d15721f3e6682d10480c1e65d26a163fc8191b`，其余服务与任务镜像仍为 `3d1ce5181a11787e3629fea4021f0130e734912d`。第四十七批已完成精确 SHA CI、镜像核对和共享页面复验：失败任务不因历史回放显示已连接，原工作区和取消新建均保留。第四十八批新增实际名称／slug 搜索、预览崩溃后日志／编辑／重启、干净工作区发布至新 preview。发布后未推送计数错误已有回归和本地修复，完整门禁通过；发布／部署继续。此前四窗 OOM 与保卷恢复仍未解决；数据库已恢复但节点容量仍有限。任务和标准沿用 [plan.md](plan.md)，没有删减范围。
...
- E21 搜索、预览与工作树发布：[implementation 第四十八批](implementation.md#第四十八批搜索预览故障恢复与当前工作树发布)，实际名称／slug 搜索继续旧任务；新 files 专用任务只终止预览进程，日志定位、CLI 草稿／真实模型轮次、文件保存、单次重启及 iframe 新内容完整串联。dirty 拦截后精确提交 QA 首页，受保护 main 拒绝保持，再从普通 QA 开发分支发布 a80dbc1／v0.1.1 并实际打开。临时 preview 副本均恢复 1／1；发布后误报未推送已有修复回归，仍待部署验证。
...
| UX-AT-24 | E4、E6、E9，真实路由与历史适配器回归；E17 开发／发布返回；E21 搜索返回与精确文件链接 | 旧链接的全部对象／筛选及两空间往返仍需实机核对 |
...
| UX-AT-32 | E4、E12、E21，真实 untracked patch、首页修改与独立未提交／未推送统计；新发布后误报未推送已复现 | 计数修复已有真实 Git 回归，待部署复验；暂存、未暂存、删除、二进制及截断的完整界面仍需核对 |
