...
> 2026-09-14；共享 console 为 `10455cc61faea17a88ca3216a7db27f2e813b5eb`，cs-api 为 `9642e23fb4ed0e033d32eeb9ae938ffb276380d2`，cs-controller 为 `03d15721f3e6682d10480c1e65d26a163fc8191b`，其余服务与任务镜像仍为 `3d1ce5181a11787e3629fea4021f0130e734912d`。第四十七批已完成精确 SHA CI、镜像核对和共享页面复验。第四十八批新增实际搜索、预览故障恢复和当前工作树发布；发布后计数修复的完整门禁、精确 SHA CI、cs-api 部署和原会话真实 API 复验均通过，未推送 1→0。Mac 再次锁定，界面复验待手动解锁。此前四窗 OOM 与保卷恢复仍未解决；数据库已恢复但节点容量仍有限。任务和标准沿用 [plan.md](plan.md)，没有删减范围。
...
- E22 迁移故障与恢复：[implementation 第四十九批](implementation.md#第四十九批迁移失败恢复发布与日志事实)，专用 files v0.1.3 迁移输出固定标记后 exit 42，真实 Job／发布失败且原预览不变；移除验收命令并发布 v0.1.4 后正常就绪。失败 releaseId 精确查询返回标记，其他版本无该迁移记录；原任务／Agent、文件摘要和正式槽保留。日志时间与 stdout 元数据修复已发布 cbe2825，新增十项回归、完整本地门禁和精确 SHA CI 通过，镜像内容已核对并导入。实际 console／cs-api 更新被自动审批拒绝，具体确认待答；共享部署和解锁后的页面旅程仍未完成，不把 API 取证视为 UX-AT-13 已通过。
- E23 全部部署槽筛选：[implementation 第五十批](implementation.md#第五十批全部部署槽日志筛选)、[模块 API 回归](../../../modules/observability/tests/observabilityModule.test.ts)，修复未传 slot 仍默认正式槽的后端行为。全部槽限定服务部署工作负载，明确槽保持角色映射；真实只读标签查询由同服务三 Pod 缩为蓝绿两个部署 Pod，排除原失败开发容器。先红后绿和完整本地门禁通过；本批尚无共享部署／页面证明，不增加 UX 通过项。
...
| UX-AT-13 | E5、E6、E22，实际 v0.1.3 迁移 exit 42、精确 releaseId 日志与 v0.1.4 恢复发布 | 真实失败和恢复 API／集群证据已取得；日志元数据修复待部署，从失败发布一跳进入精确日志的页面旅程仍待解锁 |
...
| UX-AT-24 | E4、E6、E9，真实路由与历史适配器回归；E17 开发／发布返回；E21 搜索返回与精确文件链接；E23 全部部署槽后端回归 | 旧链接的全部对象／筛选及两空间往返仍需实机核对；全部槽修复尚未部署 |
...
| UX-AT-32 | E4、E12、E21，真实 untracked patch、首页修改；发布后计数修复已有真实 Git 回归、部署和原会话 API 复验 | 计数界面复验待解锁；暂存、未暂存、删除、二进制及截断的完整界面仍需核对 |
...
| UX-AT-52 | E2、E4、E9、E10，独立历史路径、agent 接续、CLI 原对象；E21 实看历史入口与普通终端 | 实际历史 Agent 对话继续后返回、CLI 状态及未保存输入处理仍待核对；普通终端提交不替代该旅程 |
