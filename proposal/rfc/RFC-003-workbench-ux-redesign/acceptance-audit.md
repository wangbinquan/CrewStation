...
> 2026-09-15；第七十五批已实现 I14 保留原工作树重建。原失败工作卷、HEAD 与文件摘要实际保持，新 Runner／开发预览就绪，旧 CLI 明确结束且没有自动重跑；手动启动一个 OpenCode 并得到真实模型输出。controller／console 为本批 final-19272ba002、generation=24／49；API 为归并后的 publish2-229a686b71／40，session 为本批首镜像／15，均 1／1。UX-AT-34 通过，累计 27／52；I15、I9 和其余完整旅程继续。新 CLI 的版本探测降级仍需处理，不冒称完成通知通过。
...
| UX-AT-28 | E2、E19、E49，单次启动和幂等；真实四窗 OOM 与 I15 隔离方案 | I15 的独立执行环境及保卷／失败清理已实现；启动入口、多 Runner 接线与真实故障注入继续；I14 恢复不能替代新增失败保护 |
...
E50：第七十七批管理员资源绑定与逐 CLI 执行已接线，17 条新增回归及完整门禁 1274 pass／4 skip／0 fail，console 644ms。包括实际数据库的多来源序号／旧游标迁移、末屏及跨实例串行，React 验证每窗 WS 与关闭页签。尚未部署验证，UX-AT-28／35 不提前通过。
