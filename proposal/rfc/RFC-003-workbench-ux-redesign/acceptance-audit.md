...
| UX-AT-25 | E11、E33、E37–E40；开发各视图、管理接入主路由与市场详情新增尺寸记录；代码区 82→302px、菜单越界、只读正文滚动、历史终端撑高均已实机修复 | 继续核对创建接入／开通状态、长标签完整阅读及其余关键路径；已有记录不代表全部通过 |
| UX-AT-26 | E8、E11、E33、E37–E40；页签、错误字段、表格、复制、菜单、只读 CLI 焦点与方向键，以及代码尾部可达；语言声明随真实控件同步 | 继续补齐实浏览器系统明暗主题、所有关键控件焦点可见与全程键盘操作 |
...
| UX-AT-51 | E11，原稿一致性、顶栏／favicon／登录历史实看；第五十三批新增当前原稿四尺寸、单色／双主题的 rsvg-convert 实际渲染图 | 资产尺寸已复核；当前浏览器里的顶栏、favicon、登录与明暗主题统一检查仍待完成 |
