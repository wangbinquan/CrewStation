...
- E22 迁移故障与恢复：[implementation 第四十九批](implementation.md#第四十九批迁移失败恢复发布与日志事实)，专用 files v0.1.3 迁移输出固定标记后 exit 42，真实 Job／发布失败且原预览不变；移除验收命令并发布 v0.1.4 后正常就绪。失败 releaseId 精确查询返回标记，其他版本无该迁移记录；原任务／Agent、文件摘要和正式槽保留。实机发现日志时间与 stdout 元数据失真，已补十项回归和实现修复，完整本地门禁 1118 pass／4 skip／0 fail、console build 成功；部署及解锁后的页面旅程仍待完成，不把 API 取证视为 UX-AT-13 已通过。
