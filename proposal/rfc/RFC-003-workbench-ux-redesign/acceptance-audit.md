...
> 2026-09-16；第八十三批按设计附件对齐外壳、概览、发布、诊断与开发页视觉，修复首次打开逐页回放几万条历史导致长时间“连接中”的缺陷，并把本机平台整体部署到当前 main 树（含 RFC-004 契约）。真实四窗页面在 1280×720 量得首窗 205px、四标题与 12 行可见，UX-AT-35 通过，累计 29／52。RFC-004 已提前落地代码（见其 plan.md），不再排队等待。

## 1. 判据和环境

本表区分源码／自动验证、隔离原生进程、历史浏览器记录和当前完整实机旅程。自动测试覆盖一条接口或路由，不等于使用者在当前部署完成整条旅程。当前 UX-AT-01／02／03／04／05／06／07／08／10／13／17／18／19／20／21／23／24／25／28／29／32／33／34／35／36／39／40／41／52 二十九项已取得本次实机通过证明；其余 23 项仍有未执行的角色、失败分支或完整旅程。不把有测试文件或一个绿门禁计算为 52 项验收通过。
...
| UX-AT-26 | E8、E11、E33、E37–E41；页签、错误字段、表格、复制、菜单、只读 CLI、创建／发布／上线确认按钮的焦点与键盘，以及代码尾部可达；语言声明随真实控件同步 | 继续补齐实浏览器系统明暗主题、所有关键控件焦点可见与全程键盘操作 |
...
| UX-AT-42 | E10、E17，自动去重和真实断线重进后的原事件保持；E18 缺少结果时明确未确认；E46 补动态历史请求迟到／关闭后的恢复回归 | 乱序补发与状态通道不可用的实际通知／恢复仍待证明 |
| UX-AT-43 | E2、E10，个人布局／已读隔离和隐藏窗口定位 | 两位实际成员同时查看且互不清除对方未读 |
| UX-AT-44 | E7、E9、E43，消费入口与管理供给分离；管理员接入全旅程已完成 | 其余真实角色入口检查仍待完成，不能以管理员身份替代 |
