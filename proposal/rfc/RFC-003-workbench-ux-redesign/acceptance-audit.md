...
> 2026-09-14；共享 console／cs-api／cs-controller 已更新为 `03d15721f3e6682d10480c1e65d26a163fc8191b`，其余服务与任务镜像仍为 `3d1ce5181a11787e3629fea4021f0130e734912d`。第四十六批四窗验收发生真实 OOMKilled／137，失败工作树已只读确认保留；原 B／Claude 与新增 D／E 不再在线。故障提示修复通过完整本地门禁及精确 SHA CI，部署后失败会话／工作树／生产比较的真实 HTTP 已复验。另行发现的节点磁盘满导致数据库暂不可用，精确清理本任务旧构建产物后原数据库 Pod 自行恢复；数据卷保留。Chrome 当前仅有窗口标题、无页面与截图，待用户恢复可读窗口。任务和标准沿用 [plan.md](plan.md)，没有删减范围。

## 1. 判据和环境

本表区分源码／自动验证、隔离原生进程、历史浏览器记录和当前完整实机旅程。自动测试覆盖一条接口或路由，不等于使用者在当前部署完成整条旅程。当前 UX-AT-02／03／04／06／08／10／19／21／23／29／33／36／39／40／41 十五项已取得本次实机通过证明；其余 37 项仍有未执行的角色、失败分支或完整旅程。不把有测试文件或一个绿门禁计算为 52 项验收通过。
...
第三十九至四十五批中，Chrome 原生 CUA 恢复后已实看并操作新工作台；第四十六批当前不可读取页面，见本表开头。实际 admin 在专用 rfc003-ux 完成首次上线并打开正式应用；新建 rfc003-verify-workbench、开会话和首个真实 Claude TUI 也有证据。共享更新、首次切流和此前 main 推送授权已明确，无需再次索要。新 Pod 缺平台模型配置，Claude 停在登录选择；作者指定改用 OpenCode，已完成真实双进程并行、后台完成／草稿／精确定位、页签关闭恢复和独立中断退出。RFC-004 三件套与 ADR-0004 已获批准，Draft 表示排队，须等 RFC-003 完结后启动；现有 RFC-003 的剩余角色／尺寸／失败恢复继续。
...
| UX-AT-24 | E4、E6、E9，真实路由与浏览器历史适配器回归；E17 实际开发／发布返回 | 旧链接的全部对象／筛选及两空间往返仍需实机核对 |
...
| UX-AT-28 | E2、E19，单次启动和幂等；真实四窗 OOM 暴露资源保护缺口 | 新增 CLI 资源不足影响原进程的故障仍未解决，需补保护与实际失败恢复 |
...
| UX-AT-34 | E2、E17、E19，真实断线附着；OOM 后名册 unknown，失败会话新增回归与部署后 HTTP 均通过 | 本批共享界面与后续容器恢复／新建旅程未完成，不声称旧进程在线 |
