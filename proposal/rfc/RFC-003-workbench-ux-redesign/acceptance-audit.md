...
- E20 会话与真实容器状态：[implementation 第四十七批](implementation.md#第四十七批会话连接状态与真实容器状态)，共享失败页面复现绿色误提示；修复后真实后端候选保持失败、历史回放不覆盖，同时旧正常会话正确显示已连接。定向及完整本地门禁通过，草稿／焦点与连接变化的回归不替代 OOM 恢复或全套故障旅程。
