...
| UX-AT-52 | E2、E4、E9、E10，独立历史路径、agent 接续、CLI 原对象；E21 实看历史入口与普通终端；E26 补消息草稿、发送回执、对象定位与离开保护回归；E27 真实 OpenCode 两轮接续及等待状态修复；E28 补页面即时刷新与读取次数回归 | 真实历史对话已有 API／WS 证明，等待投影已部署复验，页面刷新补丁已部署；从页面继续后返回 CLI、状态及未保存输入处理仍待核对，不能由自动回归替代完整旅程 |
