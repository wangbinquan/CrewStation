...
> 2026-09-16；第八十批已上库、精确 SHA CI 成功并部署。第八十一批补真实负责人切流、旧确认拒绝、版本比较更新和生产数据两模式的审批／撤销；Mac 锁屏与浏览器连接失败阻断余下页面交互。累计 28／52，RFC-004 按顺序等待。
...
| UX-AT-11 | E5、E15、E16、E44，实际正常回退、历史候选排除、显式策略拒绝及非维护窗口发布失败 | 显式禁止回退已实机证明，拒绝后说明、原槽位和记录保持；仍缺含破坏性迁移的已上线版本在浏览器拒绝回退的完整分支，未以自动测试或发布前拒绝替代 |
...
| UX-AT-30 | E4、E12、E15、E45、E47，一致／领先历史页面，真实落后 Git／API及正式 v0.1.8 后页面分叉 1／3 | 正式目标与分叉页已实看；浅历史、无共同历史及其余分支仍需补齐，具体切流已执行 |
