...
> 2026-09-14；共享 console 已更新为 `cbe28250607aa4084d74556c32aff650a9347783`，cs-api 为 `80d1020195f456ca010d43265926f0b1e7403f79`。第五十批只更新这两个服务，精确 SHA CI、实际 imageID 和源码／静态产物核对通过。真实 API 已验证全部部署槽及实际日志时间；原开发会话、工作文件和正式／预览版本保持。Mac 仍锁定，页面复验待手动解锁。此前四窗 OOM 与保卷恢复仍未解决；数据库已恢复但节点容量仍有限。任务和标准沿用 [plan.md](plan.md)，没有删减范围。
...
- E24 活动代理与目录更新顺序：[implementation 第五十一批](implementation.md#第五十一批真实-api-试调与目录路由一致性)、[组合根回归](../../../modules/platform/tests/gatewayCatalogRoutes.test.ts)。files QA 的已默认开放 GET 经真实开发容器返回 404，实查路由仍使用已退役模板代理名。修复活动代理选择及目录提交后的刷新，覆盖改名、撤下与消费者顺序；参考代理正式槽尚空、预览 v0.1.2 就绪，端到端复验继续，不把自动回归计为 UX-AT-15 通过。
...
| UX-AT-13 | E5、E6、E22，实际 v0.1.3 迁移 exit 42、故障时精确日志与 v0.1.4 恢复；元数据修复已部署并验证实际容器日志 | 原失败 Job 已不存在；解锁后需重新产生受控故障，在日志仍可查时完成失败发布到精确日志的一跳页面旅程 |
| UX-AT-14 | E6，订阅／trace 上下文和诊断入口 | 真实死信投递、重试与结果核对 |
| UX-AT-15 | E7、E24，实际 Swagger 控件、结构化通道及响应；真实 404 的目录／路由缺陷已定位并修复 | 修复部署后，浏览器经开发容器和共享网关完成已授权 API 调用 |
