...
export async function liveInspection(deps: ClusterDeps, actor: Actor, target: ClusterResource, request: ClusterInspectRequest): Promise<ClusterInspection> {
  ...
  return { inspectionId: newResourceId(), expiresAt: new Date(deps.clock.now().getTime() + 300_000).toISOString(), target: row, request, capability, related: relatedResources(snapshot, row).sort((a, b) => a.uid.localeCompare(b.uid)).map((r) => ({ uid: r.uid, name: r.name, kind: r.kind })), ...(domain ? { domain } : {}) };
}
...
export function operationUseCases(deps: ClusterDeps) {
  return {
    ...
    accept: async (actor: Actor, request: ClusterOperationRequest) => {
      ...
      return deps.repository.accept({ operationId: newResourceId(), inspectionId: inspection.inspectionId, idempotencyKey: request.idempotencyKey, actorId: actor.userId, action: request.params.action, target: inspection.target, params: request.params, phase: 'queued', createdAt: now, updatedAt: now, durationMs: 0, traceId: randomUUID(), httpStatus: 202, reason: '已受理，等待执行' }, digest(request));
    },
  };
}
