...
async function setup() {
  ...
  const module = createClusterManagementModule({ db: database.db, k8s, metadata: { read: async () => { if (state.metadataError) throw new Error('metadata unavailable'); return data; } }, domains: { inspect: async () => { throw new Error('unexpected domain'); }, execute: async () => { throw new Error('unexpected domain'); }, observe: async () => { throw new Error('unexpected domain'); } }, authorizeProject: async () => { throw new Error('unexpected project authorization'); }, isAdmin: async (id) => id === admin.userId, systemNamespace: 'crewstation-system', catalog, instance: 'recovery', clock: { now: () => new Date(state.now) }, observationMs: 100, wait: async () => { state.now += 200; await state.wait(); } });
