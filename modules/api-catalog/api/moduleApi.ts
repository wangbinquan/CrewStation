...
export interface GrantedOperations {
  readonly operationRoutes: OperationRoute[];
...
export interface ApiCatalogModuleApi {
  ...
  renameProxy(actor: Actor, id: string, name: string): Promise<ApiProxyDto>;
  ...
  setOpenPolicy(actor: Actor, operationId: string, policy: OpenPolicy): Promise<ApiOperationDto>;
  ...
  revokeGrant(actor: Actor, serviceId: ServiceId, operationId: string): Promise<void>;
