...
import { BUILTIN_RESOURCES, ResourceIdSchema, DomainTopic, IDENTITY_HEADERS, ManifestSchema } from '@crewstation/contracts';
...
const issuesManifest: Manifest = ManifestSchema.parse({ apiVersion: 'crewstation/v2', kind: 'APIProxy', spec: { service, proxy: 'issues', upstream: { connection: 'gitlab-main' }, apis: { exposes: { openapi: './openapi.yaml' } } } });
const workerManifest = (exposes: boolean): Manifest => ManifestSchema.parse({ apiVersion: 'crewstation/v2', kind: 'DigitalWorker', spec: { service, apis: exposes ? { exposes: { openapi: './openapi.yaml' } } : {} } });
...
async function grantChangedEvents(): Promise<Array<[string, string]>> {
  const rows = (await tdb.db.execute(`SELECT payload->>'operationId' AS operation_key, payload->>'state' AS state FROM platform_infra.domain_events WHERE topic = 'api-catalog.grant-changed' ORDER BY id`)) as unknown as Array<{ operation_key: string; state: string }>;
...
describe.skipIf(!available)('api-catalog module', () => {
  let detailKey: string, listKey: string, createKey: string, workerKey: string, issuesProxyId: string;
  ...
  test('release.registered：接入容器按 spec.proxy 登记操作（默认 targeted），数字人 exposes 以 slug 为代理名', async () => {
    ...
    detailKey = ops.find((o) => o.path === '/v1/issues/{id}')!.id;
    listKey = ops.find((o) => o.path === '/v1/issues' && o.method === 'GET')!.id;
    createKey = ops.find((o) => o.method === 'POST')!.id; workerKey = ops.find((o) => o.path === '/reports')!.id;
    issuesProxyId = ops.find((o) => o.id === listKey)!.proxyId;
    expect(ops.every((o) => ResourceIdSchema.safeParse(o.id).success)).toBe(true);
    expect(ops.map((o) => o.id)).toEqual([listKey, detailKey, createKey, workerKey].sort());
    expect(ops.every((o) => o.openPolicy === 'targeted' && o.granted === undefined)).toBe(true);
    expect(ops.find((o) => o.id === createKey)).toMatchObject({ summary: '创建', resourceNote: '按项目' });
  ...
  test('setOpenPolicy 只有管理员；listOperations 对服务标注 granted，默认开放视为已授权', async () => {
    ...
    await expect(catalog.api.listOperations(dev, '01a0bf5d-8f4b-7430-8195-be2419fb2d35' as ServiceId)).rejects.toMatchObject({ kind: 'not_found' });
  });
  ...
  test('prunedOpenApi：按服务可调范围裁剪、清理未引用组件、servers 指向服务域', async () => {
    const pruned = await catalog.api.prunedOpenApi(dev, demo.serviceId, issuesProxyId);
    ...
    await expect(catalog.api.prunedOpenApi(dev, workerB.serviceId, issuesProxyId)).rejects.toMatchObject({ kind: 'not_found' });
  });
  ...
  test('新发布少了操作 → removed，Grant 保留但不再匹配；恢复后重新匹配且开放策略保留', async () => {
    ...
    expect((await catalog.api.listOperations(dev)).map((o) => o.id)).toEqual([listKey, createKey, workerKey]);
    expect((await catalog.api.grantedOperations('demo/demo')).operations).toEqual([]);
    await expect(catalog.api.setOpenPolicy(admin, detailKey, 'default')).rejects.toMatchObject({ kind: 'not_found' });
    await registerRelease(issuesProject, issuesManifest, issuesDoc);
    expect((await catalog.api.grantedOperations('demo/demo'))).toMatchObject({ operations: [detailKey], defaultOpen: [listKey] });
    expect((await catalog.api.listOperations(dev)).find((o) => o.id === listKey)?.openPolicy).toBe('default');
  });
  ...
  test('数字人去掉 exposes → 代理与操作标记 removed；EventProducer 发布被忽略', async () => {
    ...
    const producer: Manifest = ManifestSchema.parse({ apiVersion: 'crewstation/v2', kind: 'EventProducer', spec: { service, producer: 'gitlab', ingress: { path: '/hooks' }, produces: [{ eventType: 'gitlab.push' }] } });
  ...
  test('HTTP 路由：UUID 操作索引、查询参数、状态码与权限', async () => {
    ...
    expect(((await list.json()) as { items: Array<{ id: string; granted: boolean }> }).items.find((o) => o.id === listKey)?.granted).toBe(true);
    ...
    expect(await policy.json()).toMatchObject({ id: createKey, openPolicy: 'default' });
    ...
    const openapi = await app.request(`/v1/catalog/proxies/${issuesProxyId}/openapi?serviceId=${demo.serviceId}`, { headers: as(dev) });
    ...
    expect((await catalog.api.grantedOperations('demo/demo')).operations).toEqual([detailKey, createKey].sort());
