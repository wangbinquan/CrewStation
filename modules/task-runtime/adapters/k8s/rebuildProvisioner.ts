...
const fingerprint = rebuildIntent;
...
function validatePod(pod: K8sObject, record: EnvironmentRebuild): string {
  ...
  if (pod.metadata.deletionTimestamp || (pod.metadata.annotations?.['crewstation.io/rebuild-intent'] !== fingerprint(record) && (!record.legacyCluster || pod.metadata.annotations?.['crewstation.io/rebuild-intent'] !== fingerprint(record, true)))
