...
function clusterTask(e: TaskEnvironment) { return ({ taskId: e.id, projectId: e.projectId, namespace: e.namespace, podName: e.podName, pvcName: e.pvcName, ...(e.podUid ? { podUid: e.podUid } : {}), kind: e.kind, state: e.state, profile: e.native?.computeProfile?.name ?? e.labels['crewstation.io/compute-profile'] ?? e.profile, ...(e.kind === 'profile-test' && e.labels['crewstation.io/profile-revision'] ? { profileRevision: Number(e.labels['crewstation.io/profile-revision']) } : {}), ...(e.kind === 'profile-test' && e.labels['crewstation.io/profile-test'] ? { profileTestId: e.labels['crewstation.io/profile-test'] } : {}), revision: e.updatedAt.toISOString(), volumeMode: e.volumeMode, ...(e.native ? { purpose: e.native.purpose ?? 'cli', parentTaskId: e.native.parentTaskId, agentId: e.native.agentId, ...(e.native.terminalId ? { terminalId: e.native.terminalId } : {}), ...(e.native.podUid ? { podUid: e.native.podUid } : {}), pvcUid: e.native.pvcUid, ...(e.native.computeProfile ? { profileRevision: e.native.computeProfile.revision } : {}) } : {}) }); }
