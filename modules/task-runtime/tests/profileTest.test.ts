...
const material: BeforeStartMaterial = { profile: 'gw', revision: 2, contentHash: 'h', steps: [{ kind: 'script', stepId: 'warm', name: '预热', language: 'shell', source: 'true', argv: [], timeoutMs: 2000 }], vars: {}, secrets: { KEY: 'sk-test' }, configFile: { kind: 'none' }, captureOutput: true };
...
const inputFor = (testId: string, patch: Partial<ProfileTestRunInput> = {}): ProfileTestRunInput => ({
  testId, profile: 'gw', revision: 2, launch: LaunchSpecSchema.parse({ protocol: 'opencode', binaryPath: '/usr/local/bin/opencode', model: 'anthropic/claude-x' }), image,
...
const execution = (agentId: string, state: BeforeStartExecution['state'], stepState: 'succeeded' | 'failed'): BeforeStartExecution => ({
  executionId: 'bse_1', agentId, processAttemptId: 'a:1', profile: { profile: 'gw', revision: 2 }, state, queuedAt: new Date().toISOString(),
  steps: [{ stepId: 'warm', name: '预热', kind: 'script', state: stepState, exitCode: stepState === 'succeeded' ? 0 : 7, log: { stdoutTail: 'warm ok', stderrTail: '' } }],
  ...(state === 'failed' ? { error: { code: 'script_failed' as const, message: '退出码 7', stepId: 'warm' } } : {}),
});
...
beforeAll(async () => {
  ...
  runtime = createTaskRuntimeModule({
    ...
    profiles: { listTaskProfiles: async () => [], getTaskProfile: async (name) => (name === 'coding-medium' || name === 'cli-small' ? { name, cpu: name === 'cli-small' ? '500m' : '1', memory: '2Gi', storage: '10Gi' } : undefined) },
...
describe.skipIf(!available)('档位测试执行器（RFC-006 §6）', () => {
  test('已知协议通过：档位镜像与资源、显式 Runner 命令、临时工作目录、上下文与阶段、结束后释放', async () => {
    ...
    const run = runtime.api.runProfileTest(inputFor('pft_1', { taskProfile: 'cli-small' }), async (p) => { progress.push(structuredClone(p)); }, async () => true);
  ...
  test('启动前步骤失败 → before-start-failed；协议轮次到点仍无终态 → 取消并记 timeout', async () => {
    ...
    let run = runtime.api.runProfileTest(inputFor('pft_2'), async () => undefined, async () => true);
    await connectRunner();
    let outcome = await run;
    expect(outcome).toMatchObject({ state: 'failed', outcome: 'before-start-failed', error: '退出码 7' });
    expect(outcome.stages.map((s) => [s.id, s.state])).toEqual([['step:warm', 'failed'], ['launch', 'skipped'], ['model', 'skipped']]);
    await settled();

    onStart = (id, command) => { emit(id, agentEvent(command.agentId, 1, { type: 'started' })); };
    run = runtime.api.runProfileTest(inputFor('pft_3', { beforeStart: { ...material, steps: [] } }), async () => undefined, async () => true);
    await connectRunner();
    outcome = await run;
  ...
  test('镜像拉取失败、容器起不来（不是平台底座）、Runner 协议不一致各自归类，且都回收测试任务', async () => {
    let run = runtime.api.runProfileTest(inputFor('pft_4'), async () => undefined, async () => true);
    ...
    run = runtime.api.runProfileTest(inputFor('pft_5'), async () => undefined, async () => true);
    ...
    run = runtime.api.runProfileTest(inputFor('pft_6'), async () => undefined, async () => true);
    const { taskId, token } = await podCredentials();
  ...
  test('通用终端：probeTerminal 通过与输出不匹配；租约丢失与环境中途失败记 unknown', async () => {
    ...
    let run = runtime.api.runProfileTest(inputFor('pft_7', { launch: terminal, terminalTest }), async () => undefined, async () => true);
    await connectRunner();
    let outcome = await run;
    ...
    outcome = await runtime.api.runProfileTest(inputFor('pft_9'), async () => undefined, async () => false);
    expect(outcome).toMatchObject({ state: 'unknown', outcome: 'environment-lost' });
    await settled();

    onStart = undefined;
    run = runtime.api.runProfileTest(inputFor('pft_10'), async () => undefined, async () => true);
    const taskId = await connectRunner();
    await Bun.sleep(60);
