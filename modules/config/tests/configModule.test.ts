...
const written = new Map<string, ConfigItemDto>();
const definitions = new Map<string, string>();
const cacheKey = (project: ProjectId, env: ConfigEnv, name: string) => `${project}:${env}:${name}`;
async function writeItem(actor: Actor, project: ProjectId, input: { name: string; env: ConfigEnv; value: string; isSecret: boolean }) {
  const key = cacheKey(project, input.env, input.name), previous = written.get(key);
  const body = { ...input, bindingName: input.name, definitionId: definitions.get(input.name), expectedVersion: previous?.version };
  const item = previous ? await config.api.updateItem(actor, project, previous.id, body) : await config.api.createItem(actor, project, body);
  written.set(key, item); definitions.set(input.name, item.definitionId); return item;
}
async function removeItem(actor: Actor, project: ProjectId, env: ConfigEnv, name: string) {
  const item = written.get(cacheKey(project, env, name))!;
  await config.api.deleteItem(actor, project, env, item.id, item.version);
}
const missingDefinition = '01a0bf5d-8f4b-7200-8000-000000000001';
beforeAll(async () => {
  if (!available) return;
  ...
  const a = await identity.api.ensureUser({ externalId: 'demo:admin', name: 'Admin', email: 'admin@example.com' });
    if (a.platformRole === 'user') await identity.api.setPlatformRole(a.id, { platformRole: 'admin', expectedRole: 'user' });
  ...
  const project = createProjectModule({ db: tdb.db, identity: identity.api, hosts, settings: { defaultMaxConcurrentTasks: 3, defaultServicePlan: BUILTIN_RESOURCES.servicePlanSmall } });
...
describe.skipIf(!available)('config module', () => {
  test('负责人维护生产组：版本按取值组递增、Secret 只写不读、密文落库、事件落库', async () => {
    const host = await writeItem(owner, projectId, { name: 'DB_HOST', env: 'production', value: 'db.internal', isSecret: false });
    expect(host).toMatchObject({ name: 'DB_HOST', env: 'production', isSecret: false, value: 'db.internal', version: 1, updatedBy: owner.userId });
    const token = await writeItem(owner, projectId, { name: 'API_TOKEN', env: 'production', value: 's3cret', isSecret: true });
  ...
  test('角色表：开发者不能改生产组但能改开发组；非成员 404；名字与项目存在性校验', async () => {
    await expect(writeItem(dev, projectId, { name: 'DB_HOST', env: 'production', value: 'x', isSecret: false })).rejects.toMatchObject({ kind: 'forbidden' });
    await expect(removeItem(dev, projectId, 'production', 'DB_HOST')).rejects.toMatchObject({ kind: 'forbidden' });
    const debug = await writeItem(dev, projectId, { name: 'DEBUG', env: 'development', value: '1', isSecret: false });
    expect(debug.version).toBe(1);
    expect(await config.api.currentVersion(projectId, 'production')).toBe(2);
    await expect(writeItem(stranger, projectId, { name: 'X', env: 'development', value: '1', isSecret: false })).rejects.toMatchObject({ kind: 'not_found' });
    await expect(config.api.listItems(stranger, projectId, 'development')).rejects.toMatchObject({ kind: 'not_found' });
    await expect(writeItem(owner, projectId, { name: 'bad-name', env: 'production', value: '1', isSecret: false })).rejects.toMatchObject({ kind: 'validation' });
    expect(await writeItem(admin, projectId, { name: 'ADMIN_SET', env: 'production', value: 'a', isSecret: false })).toMatchObject({ version: 3 });
  });
  ...
  test('renderEnv 解密并可按版本回放；删除生成新版本；validateManifestEnv 报缺失键', async () => {
    ...
    expect((await writeItem(owner, projectId, { name: 'DB_HOST', env: 'production', value: 'db2.internal', isSecret: false })).version).toBe(4);
    ...
    await removeItem(owner, projectId, 'production', 'DB_HOST');
    ...
    await expect(removeItem(owner, projectId, 'production', 'DB_HOST')).rejects.toMatchObject({ kind: 'not_found' });
    expect(await config.api.currentVersion(projectId, 'production')).toBe(5);
    ...
    expect(await config.api.validateManifestEnv(projectId, 'production', [
      { name: 'API_TOKEN', from: 'secret', configDefinitionId: definitions.get('API_TOKEN')! }, { name: 'DB_URL', from: 'config', configDefinitionId: missingDefinition }, { name: 'ADMIN_SET', from: 'config', configDefinitionId: definitions.get('ADMIN_SET')! },
    ])).toEqual({ missing: [missingDefinition] });
    expect(await config.api.validateManifestEnv(projectId, 'development', [{ name: 'DEBUG', from: 'config', configDefinitionId: definitions.get('DEBUG')! }])).toEqual({ missing: [] });
  });
  ...
  test('HTTP 路由：身份头进入、env 不一致 400、开发者改生产 403、Secret 不回显、删除 204、未登录 401', async () => {
    ...
    const put = await app.request(base, { method: 'POST', headers: asUser(owner.userId), body: JSON.stringify({ bindingName: 'FEATURE_X', name: 'FEATURE_X', env: 'production', value: 'on' }) });
    expect(put.status).toBe(200);
    const created = await put.json() as ConfigItemDto;
    expect(created).toMatchObject({ name: 'FEATURE_X', value: 'on', version: 6, isSecret: false });
    const mismatch = await app.request(base, { method: 'POST', headers: asUser(owner.userId), body: JSON.stringify({ bindingName: 'FEATURE_Y', name: 'FEATURE_Y', env: 'development', value: 'on' }) });
    expect(mismatch.status).toBe(400);
    const badName = await app.request(base, { method: 'POST', headers: asUser(owner.userId), body: JSON.stringify({ bindingName: 'lower', name: 'lower', env: 'production', value: 'on' }) });
    expect(badName.status).toBe(400);
    const forbidden = await app.request(base, { method: 'POST', headers: asUser(dev.userId), body: JSON.stringify({ bindingName: 'FEATURE_Z', name: 'FEATURE_Z', env: 'production', value: 'on' }) });
    ...
    expect((await app.request(`${base}/${created.id}?expectedVersion=${created.version}`, { method: 'DELETE', headers: asUser(owner.userId) })).status).toBe(204);
    expect((await app.request(`${base}/${created.id}?expectedVersion=${created.version}`, { method: 'DELETE', headers: asUser(owner.userId) })).status).toBe(404);
