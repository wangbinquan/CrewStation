 ...
 */
export function renderEnvUseCase({ uow, cipher }: ConfigUseCaseDeps) {
  return async (projectId: ProjectId, env: ConfigEnv, version?: number): Promise<Record<string, string>> => {
    ...
    for (const entry of entries) rendered[entry.name] = entry.isSecret ? await cipher.decrypt(entry.value) : entry.value;
    return rendered;
  };
