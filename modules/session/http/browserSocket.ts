    ...
    return {
      onOpen: (_evt, ws) => {
        ...
        chain = chain.then(async () => {
          ...
          stream = await streams.open(actor, taskId, { send: (frame) => socket.send(frame), close: (code, reason) => socket.close(code, reason) }, Number.isSafeInteger(sinceSeq) && sinceSeq >= 0 ? sinceSeq : 0, { tail, viewerName: user.name });
        }).catch((error: unknown) => {
