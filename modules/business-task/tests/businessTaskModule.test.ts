...
const fakeCompute: ComputeCatalog = {
  resolve: async (selector, _usage, id) => {
    computeProjects.push(id);
    if (denyCompute) throw forbidden('项目未获授权使用此档位');
    const found = !selector || selector.kind === 'default' ? computeProfiles.find((p) => p.isDefault) : computeProfiles.find((p) => fixtureResource('compute:'+p.name) === selector.profileId);
    if (!found && (!selector || selector.kind === 'default')) throw precondition('平台尚未设置默认算力档位，请管理员在平台管理里设置', { code: 'no_default_profile' });
    if (!found) throw validation(`算力档位 ${selector && selector.kind === 'profile' ? selector.profileId : 'default'} 不存在`, { code: 'profile_not_found', available: computeProfiles.map((p) => p.name) });
    return { id: fixtureResource('compute:'+found.name), name: found.name, revision: found.revision, protocol: 'opencode', image };
  },
  launchMaterial: async (ref) => {
    materialRequests.push(ref);
    return { id: ref.profileId, name: ref.profileId, revision: ref.revision, protocol: 'opencode', image, launch: LaunchSpecSchema.parse({ protocol: 'opencode', binaryPath: '/usr/local/bin/opencode', model: 'opencode/one' }),
      beforeStart: { profile: ref.profileId, revision: ref.revision, contentHash: 'h', steps: [], vars: {}, secrets: { KEY: 'sk-business' }, configFile: { kind: 'none' }, captureOutput: false } };
  },
};
  ...
  bt = createBusinessTaskModule({
    ...
    environments: {
      ...
      createNativeExecution: async (input) => {
        ...
        const env: EnvironmentView = { id: input.id, projectId, profile: BUILTIN_RESOURCES.taskProfileMedium, state: executionControls.connected ? 'running' : 'creating', connected: executionControls.connected, traceId: 't', podName: `sub-${input.id.slice(4)}`, native: { state: executionControls.connected ? 'running' : 'queued' } };
      ...
      releaseEnvironment: async (taskId) => {
        ...
        return { id: taskId, projectId, profile: BUILTIN_RESOURCES.taskProfileMedium, state: 'released', connected: false, traceId: 't', podName: 'task-1' };
      },
      pauseEnvironment: async (taskId) => ({ id: taskId, projectId, profile: BUILTIN_RESOURCES.taskProfileMedium, state: 'paused', connected: false, traceId: 't', podName: 'task-1' }),
      resumeEnvironment: async (taskId) => ({ id: taskId, projectId, profile: BUILTIN_RESOURCES.taskProfileMedium, state: 'creating', connected: false, traceId: 't', podName: 'task-1' }),
      getEnvironment: async (taskId) => executions.get(taskId) ?? ({ id: taskId, projectId, profile: BUILTIN_RESOURCES.taskProfileMedium, state: runnerConnected ? 'running' : 'creating', connected: runnerConnected, traceId: 't', podName: 'task-1' }),
    },
    runner: {
...
describe.skipIf(!available)('business-task module', () => {
  test('创建任务、并发子任务、契约校验、交互消息、取消、重试、输出、关闭', async () => {
    ...
    await expect(bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'x', agentProfileId: fixtureResource('agent:nope'), mode: 'oneshot', prompt: 'hi' })).rejects.toMatchObject({ kind: 'validation' });
    const agent = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'analysis', agentProfileId: fixtureResource('agent:chat-v1'), outputContractId: fixtureResource('output:report-v1'), mode: 'oneshot', prompt: '分析' });
    ...
    const strict = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'strict', agentProfileId: fixtureResource('agent:chat-v1'), outputContractId: fixtureResource('output:strict-v1'), mode: 'oneshot', prompt: 'x' });
    ...
    const chat = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'chat', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'interactive', prompt: '你好' });
  ...
  test('容器未连接时子任务留在 pending，TaskRunner 连上后补发', async () => {
    ...
    const waiting = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'wait', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '等容器' });
  ...
  test('登记的档位已被管理员下线：子任务直接失败并列出现有档位，不下发 startAgent（RFC-001）', async () => {
    ...
    try {
      ...
      const orphan = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'orphan', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '孤儿档位' });
      expect(orphan.state).toBe('failed');
      expect(orphan.error).toContain(`算力档位 ${fixtureResource('compute:sample-opencode')} 不存在`);
  ...
  test('固定修订：子任务在构造时固定档位修订，启动按该修订取材料并带 attempt，DTO 只暴露档位名与修订号（RFC-006）', async () => {
    ...
    try {
      const task = await bt.api.createTask(caller, { labels: {} });
      const sub = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'pinned', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '分析' });
      expect(sub).toMatchObject({ computeProfileId: fixtureResource('compute:sample-opencode'), profileRevision: 7 });
      ...
      expect(materialRequests.at(-1)).toEqual({ profileId: fixtureResource('compute:sample-opencode'), revision: 7 });
  ...
  test('Manifest 写 default：每次受理时解析到当时的默认档位；没有默认档位时子任务失败并写明原因（C17）', async () => {
    const task = await bt.api.createTask(caller, { labels: {} });
    try {
      const first = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'd1', agentProfileId: fixtureResource('agent:chat-default'), mode: 'oneshot', prompt: '默认一' });
      expect(first).toMatchObject({ computeProfileId: fixtureResource('compute:sample-opencode'), profileRevision: 1 });
      computeProfiles = [{ name: 'sample-opencode', revision: 1 }, { name: 'bigger', revision: 3, isDefault: true }];
      const second = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'd2', agentProfileId: fixtureResource('agent:chat-default'), mode: 'oneshot', prompt: '默认二' });
      expect(second).toMatchObject({ computeProfileId: fixtureResource('compute:bigger'), profileRevision: 3 });
      expect(commands.filter((c) => c.type === 'startAgent').at(-1)).toMatchObject({ compute: fixtureResource('compute:bigger'), initialPrompt: '默认二' });
      computeProfiles = [{ name: 'sample-opencode', revision: 1 }];
      const orphan = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'd3', agentProfileId: fixtureResource('agent:chat-default'), mode: 'oneshot', prompt: '默认三' });
  ...
  test('Agent 子任务各自一个执行环境（RFC-006 §5.4）：额度满直接失败并写明原因；子 Runner 未连上时等待，连上后派发', async () => {
    ...
    try {
      ...
      const full = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'full', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '额度满' });
      ...
      const waiting = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'wait-child', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'interactive', prompt: '等子 Runner' });
      expect(waiting.state).toBe('pending');
  ...
  test('执行环境在 Agent 结束前没了（业务任务暂停、Pod 丢失）：子任务失败并带原因；未开始的子任务取消即回收执行环境', async () => {
    const task = await bt.api.createTask(caller, { labels: {} });
    try {
      const running = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'lost', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '会丢' });
      ...
      executionControls.connected = false;
      const pending = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'never', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '不会起' });
...
test.skipIf(!available)('业务子任务按所属项目校验档位；拒绝后不派发，恢复授权的重试重新校验', async () => {
  ...
  try {
    ...
    const sub = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'denied', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: '项目授权' });
...
test.skipIf(!available)('集群入口不重放成功业务；失败子任务的重复操作只受理一次，并沿原取消流程结束', async () => {
  const actor = { userId: '01a0bf5d-8f4b-7793-867c-efd7527b386b' as never, isAdmin: true };
  const task = await bt.api.createTask(caller, { labels: {} });
  ...
  try {
    const done = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'completed-cluster', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: 'already done' });
    ...
    const failed = await bt.api.submitSubtask(caller, task.id, { kind: 'agent', name: 'retry-cluster', agentProfileId: fixtureResource('agent:chat-v1'), mode: 'oneshot', prompt: 'retry me' });
