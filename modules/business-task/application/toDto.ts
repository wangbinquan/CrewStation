...
export function taskToDto(task: BusinessTask, podName?: string): BusinessTaskDto {
  return {
    id: task.id, serviceId: task.serviceId, state: task.state, traceId: task.traceId, volumeMode: task.volumeMode, taskProfileId: task.profile,
...
export function subtaskToDto(run: SubtaskRun): SubtaskDto {
  return {
    ...
    ...(run.agentProfile ? { agentProfileId: run.agentProfile.id, agentProfileName: run.agentProfile.name } : {}), ...(run.outputContract ? { outputContractId: run.outputContract.id, outputContractName: run.outputContract.name } : {}),
    ...(run.businessOutcome ? { businessOutcome: run.businessOutcome } : {}), ...(run.sessionId ? { sessionId: run.sessionId } : {}),
    ...(run.exitCode !== undefined ? { exitCode: run.exitCode } : {}), ...(run.contractResult ? { contractResult: run.contractResult } : {}), ...(run.computeProfile ? { computeProfileId: run.computeProfile.profileId, profileRevision: run.computeProfile.revision } : {}),
