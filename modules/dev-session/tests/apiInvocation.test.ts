...
export const invocationOperation: ApiOperationDto = { key: 'crm:POST:/items/{id}', proxy: 'crm', method: 'POST', path: '/items/{id}', openPolicy: 'default', granted: true };
export const invocationInput = { expectedTaskId: workspaceTask, operationKey: invocationOperation.key, pathParameters: { id: '用户 / 1' }, query: { label: ['one', 'two'] }, headers: { 'content-type': 'application/json' }, body: '{"message":"hello"}' };
...
test('试调使用当前目录、当前服务和固定会话；HTTP 错误也返回真实响应，不执行 shell', async () => {
  ...
  expect(await f.invoke()).toEqual({ taskId: workspaceTask, operationKey: invocationOperation.key, result: invocationResult });
...
test('会话在输入后或目录查询期间被替换均不转向新容器；服务身份不匹配也拒绝', async () => {
  const f = fixture();
  await expect(apiInvocationUseCase(f.deps)(workspaceActor, workspaceProject, { ...invocationInput, expectedTaskId: 'tsk_1123456789abcdef0123456789abcdef' as TaskId })).rejects.toMatchObject({ kind: 'conflict' });
