...
describe.skipIf(!available)('动态资源实际装配与 HTTP', () => {
  test('读写路径、默认分页与 no-store；动态存储失败仍能列出 CLI，权限错误继续拒绝', async () => {
    ...
    await drizzleNativeTerminals(db.db).reserve({ taskId, record, createdBy: workspaceActor.userId, clientRequestId: crypto.randomUUID(), fingerprint: 'http', driver: 'claude-code', model: 'model', input: { clientRequestId: crypto.randomUUID(), permission: 'edit', cols: 80, rows: 24 } });
