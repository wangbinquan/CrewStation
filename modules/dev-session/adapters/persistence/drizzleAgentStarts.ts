...
import { agentStarts as table } from './agentStartTable';

type Row = typeof table.$inferSelect;

const toStart = (row: Row): AgentStart => ({
  agentId: row.agentId, taskId: row.taskId as TaskId, createdBy: row.createdBy as UserId, compute: row.compute, profile: row.profile, permission: row.permission as AgentPermission,
...
const toRow = (start: AgentStart): typeof table.$inferInsert => ({
  agentId: start.agentId, taskId: start.taskId, createdBy: start.createdBy, compute: start.compute, profile: start.profile, permission: start.permission, request: start.request,
