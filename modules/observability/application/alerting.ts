...
export function alertingUseCases(deps: ObservabilityUseCaseDeps) {
  const { alerts, authorizer, services, slots, clock } = deps;
  ...
  return {
    ...
    /** 巡检：对每个项目的两槽取健康态（照服务槽记录），触发或恢复相应告警。 */
    sweepProject: async (projectId: ProjectId): Promise<number> => {
      ...
      let changed = 0;
      for (const { slot, health } of await slotHealth({ projectId, name: svc.name, namespace: svc.namespace, roles })) {
        if (!health || health.replicas === 0) continue;
        for (const candidate of alertsFromHealth(slot, health.state)) if (await fire(projectId, candidate)) changed += 1;
        if (health.state === 'healthy') for (const key of resolvedKeysForHealthy(slot)) changed += (await alerts.resolve(projectId, key, clock.now())).length;
      }
      return changed;
    },
  };
