...
export function eventTypeToDto(type: EventType): EventTypeDto {
  return { eventType: type.eventType, producer: type.producer, producerProject: type.producerProject, ...(type.schemaRef === undefined ? {} : { schemaRef: type.schemaRef }) };
}

export function subscriptionToDto(subscription: Subscription): SubscriptionDto {
  return { id: subscription.id, serviceId: subscription.serviceId, eventType: subscription.eventType, handlerPath: subscription.handlerPath, state: subscription.state };
}
