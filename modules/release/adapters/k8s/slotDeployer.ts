...
import type { SlotDeployer, SlotStatus } from '../../ports/delivery';
...
export function kubernetesSlotDeployer(k8s: K8sClient): SlotDeployer {
  return {
    deploy: async (spec) => {
      ...
      await k8s.apply(serviceObject({ name, namespace: spec.namespace, selector, port: 80, targetPort: spec.manifest.spec.service.port, labels }));
    },
