...
export function assertMigrationAllowed(spec: MigrationSpec, maintenanceWindow: boolean): void {
  if (spec.destructive && !maintenanceWindow) {
    throw precheckFailed(precheckReason('maintenance-window-required', '含破坏性迁移的版本只能在项目维护期间发布', WINDOW_HINT), { compatibility: spec.compatibility });
  }
  if (spec.compatibility === 'destructive' && !spec.destructive) {
    throw precheckFailed(precheckReason('migration-undeclared', 'compatibility 为 destructive 时必须显式声明 destructive: true', '改好 crewstation.yaml 的 release.migration 后重新发布'));
  }
}
