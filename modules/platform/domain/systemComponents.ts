...
export function installedSystemComponents() {
  ...
  const infra = ['traefik', 'registry', 'buildkitd', 'dev-auth'];
  ...
  const fixed = [
    ['Namespace', 'crewstation-system', 'platform'], ['StatefulSet', 'postgres', 'postgres'], ['Service', 'postgres', 'postgres'], ['Service', 'postgres-headless', 'postgres'],
  ...
  return [...resources(core, 'platform-service'), ...resources(infra, 'platform-infrastructure'), ...fixed.map(([kind, name, component]) => ({ kind: kind!, name: name!, component: component!, purpose: 'platform-infrastructure' as const, restart: kind === 'StatefulSet', impact: kind === 'StatefulSet' ? ['数据库连接将中断，操作结果在数据库恢复后继续核对'] : [] }))];
}
