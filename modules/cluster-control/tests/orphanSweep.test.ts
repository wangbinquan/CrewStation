...
describe.skipIf(!available)('孤儿回收（RFC-025 设计 §6.4、D8）', () => {
  ...
  const deps = () => ({
    feed, ledger, cluster: { remove: async (target: { kind: string; name: string }) => { removed.push(`${target.kind}/${target.name}`); }, applyRoute: async () => 'unchanged' as const, applyMiddleware: async () => 'unchanged' as const,
      ...
      ensureSlotSecret: async () => ({ uid: 'x', created: false }), applySlotService: async () => 'unchanged' as const, applySlotDeployment: async () => 'unchanged' as const, dryRunSlot: async () => undefined }, clock: { now: () => now }, logger: noopLogger, stats: newObservationStats(), minAgeMs: 600_000,
