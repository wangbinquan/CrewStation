 ...
 */
export function ledgerReconciler(ledger: LedgerObservations, feed: ManagedObjectFeed, reconcile: (id: string) => Promise<void>, logger: Logger, options: LedgerReconcilerOptions = {}) {
  const queue = createWorkQueue(reconcile, { logger, concurrency: options.concurrency ?? 4 });
  let running = false;
