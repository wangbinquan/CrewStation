import type { BeforeStartMaterial, ComputeProfileSelector, ComputeUsage, ProfileRevisionRef } from '@crewstation/contracts';
...
export function resolveProfileUseCases(deps: AgentRuntimeUseCaseDeps) {
  ...
  const named = async (selector: ComputeProfileSelector | undefined): Promise<ComputeProfile | undefined> =>
    !selector || selector.kind === 'default' ? uow.read.profiles.getDefault() : uow.read.profiles.get(selector.profileId);
  const materialFor = async (profile: ComputeProfile, revision: ProfileRevision, captureOutput: boolean): Promise<BeforeStartMaterial> => {
    const stored = await uow.read.credentials.list(profile.id);
    const secrets: Record<string, string> = {};
    for (const { id, name } of revision.content.secrets) {
      const found = stored.find((c) => c.id === id);
      if (!found?.cipherText) throw precondition(`算力档位 ${profile.id} 的凭据 ${name} 尚未设置，请管理员在平台管理里补齐`, { code: 'profile_secret_missing', profile: profile.id, name });
  ...
  return {
    pinned,
    materialFor,
    resolve: async (selector: ComputeProfileSelector | undefined, usage: ComputeUsage): Promise<ResolvedProfile> => {
      const profile = await named(selector);
      if (!profile) {
        if (!selector || selector.kind === 'default') throw precondition('平台尚未设置默认算力档位，请管理员在平台管理里设置', { code: 'no_default_profile' });
        throw validation(`算力档位 ${selector.profileId} 不存在`, { code: 'profile_not_found', available: (await uow.read.profiles.list()).map((p) => p.id) });
      }
      if (profile.protocol === 'terminal' && usage !== 'cli') throw validation(`算力档位 ${profile.id} 是通用终端协议，只能用于「＋ CLI」`, { code: 'terminal_profile_not_allowed', profile: profile.id });
    ...
    lookupForRelease: async (selector: ComputeProfileSelector): Promise<{ id: string; name: string; terminalOnly: boolean } | undefined> => {
      const profile = await named(selector);
      return profile ? { id: profile.id, name: profile.name, terminalOnly: profile.protocol === 'terminal' } : undefined;
    },
    listNames: async (): Promise<string[]> => (await uow.read.profiles.list()).map((p) => p.id),
  };
}
