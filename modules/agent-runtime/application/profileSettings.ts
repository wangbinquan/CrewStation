  ...
  return {
    ...
    setDefault: async (actor: Actor, name: string): Promise<ComputeProfileDetailDto> => {
      ...
      await uow.run(async (scope) => {
        ...
        await scope.profiles.update({ ...profile, isDefault: true, updatedBy: actor.userId, updatedAt: clock.now() });
      });
      return getProfile(actor, name);
    },
    removeProfile: async (actor: Actor, name: string, confirmReferences: boolean): Promise<void> => {
      ...
      const projects = await references.listReferencingProjects(name);
      if (projects.length > 0 && !confirmReferences) {
        throw conflict(`档位 ${name} 被以下项目的已上线版本引用：${projects.join('、')}；删除后这些项目下一次起 Agent 会报「档位不存在」，确认后再删除`, { code: 'profile_referenced', projects });
      }
      await uow.run(async (scope) => {
