...
interface PreparedContent {
  ...
  credentials: Pick<ProfileCredential, 'name' | 'cipherText'>[];
  replaced: Array<{ name: string; cipherText: string }>;
  cleared: string[];
}
...
async function prepare(deps: AgentRuntimeUseCaseDeps, protocol: AgentProtocol, content: ComputeProfileContent, writes: Record<string, ProfileCredentialWrite>, existing: readonly ProfileCredential[]): Promise<PreparedContent> {
  ...
  const plan = planCredentialWrites(new Set(existing.map((c) => c.name)), content.secretNames, writes);
  const replaced = await Promise.all(plan.replace.map(async ({ name, value }) => ({ name, cipherText: await deps.cipher.encrypt(value) })));
  const credentials = [
    ...existing.filter((c) => !plan.clear.includes(c.name) && !replaced.some((r) => r.name === c.name)).map(({ name, cipherText }) => ({ name, cipherText })),
    ...replaced,
  ];
  return { content: { ...content, image: pullReference(parsed.image, deps.registry.layout) }, imageDigest, credentials, replaced, cleared: plan.clear };
}
...
async function applyCredentials(scope: RepositoryScope, profile: string, prepared: Pick<PreparedContent, 'replaced' | 'cleared'>, actor: UserId, now: Date): Promise<void> {
  for (const credential of prepared.replaced) await scope.credentials.upsert({ profile, name: credential.name, cipherText: credential.cipherText, updatedBy: actor, updatedAt: now });
  for (const name of prepared.cleared) await scope.credentials.remove(profile, name);
}
...
async function insertProfile(deps: AgentRuntimeUseCaseDeps, actor: Actor, input: { name: string; description: string; protocol: AgentProtocol; defaultVisible?: boolean }, prepared: PreparedContent): Promise<void> {
  ...
  await deps.uow.run(async (scope) => {
    if (await scope.profiles.get(input.name)) throw conflict(`算力档位 ${input.name} 已存在`, { name: input.name });
    await scope.profiles.insert(profile);
    await scope.revisions.insert(revision);
    for (const credential of prepared.credentials) await scope.credentials.upsert({ profile: input.name, name: credential.name, cipherText: credential.cipherText, updatedBy: actor.userId, updatedAt: now });
...
export function profileWriteUseCases(deps: AgentRuntimeUseCaseDeps) {
  ...
  const save = async (actor: Actor, name: string, input: SaveComputeProfileRequest): Promise<ComputeProfileDetailDto> => {
    ...
    await uow.run(async (scope) => {
      ...
      const hash = contentHashOf(prepared.content, prepared.imageDigest, credentialStampOf(prepared.content.secretNames, prepared.credentials));
      const description = input.description ?? profile.description;
      if (current?.contentHash === hash) {
        ...
        if (description !== profile.description) await scope.profiles.update({ ...profile, description, updatedBy: actor.userId, updatedAt: now });
        return;
      }
      ...
      await scope.tests.insert(test);
      await scope.testQueue.enqueue(test.testId);
      await scope.profiles.update({ ...profile, description, currentRevision: revision.revision, updatedBy: actor.userId, updatedAt: now });
    });
