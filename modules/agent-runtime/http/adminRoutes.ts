...
import {
  ...
  SlugSchema, StartProfileTestRequestSchema,
...
const nameParams = z.object({ name: SlugSchema });
const testParams = nameParams.extend({ testId: ProfileTestIdSchema });
...
export function computeProfileAdminRoutes(api: AgentRuntimeModuleApi, isAdmin: (userId: UserId) => Promise<boolean>): Hono<AppEnv> {
  ...
  const name = (c: Context<AppEnv>) => parseParams(c, nameParams).name;
  ...
  r.post('/v1/admin/compute-profiles/:name/copy', async (c) => c.json(await api.copyProfile(await actor(c), name(c), await parseBody(c, CopyComputeProfileRequestSchema)), 201));
  r.post('/v1/admin/compute-profiles/:name/tests', async (c) => c.json(await api.startTest(await actor(c), name(c), await parseBody(c, StartProfileTestRequestSchema)), 202));
  r.get('/v1/admin/compute-profiles/:name/tests/:testId', async (c) => { const p = parseParams(c, testParams); return c.json(await api.getTest(await actor(c), p.name, p.testId as ProfileTestId)); });
  return r;
}
