import type { BeforeStartStep, TemplateReference } from '@crewstation/contracts';
import { BEFORE_START_LIMITS, TEMPLATE_CONTEXT_NAMES, scanTemplate } from '@crewstation/contracts';
...
/**
 * 保存时校验一个版本的内容：尺寸、变量声明、路径形态、JSON 模板结构。
 * 只检查能静态判定的部分；实际落点、权限与格式在容器内展开后再校验一次。
 */
export function validateRevisionContent(content: RuntimeRevisionContent): void {
  let total = 0;
  ...
  if (content.defaultModel && content.models.length > 0 && !content.models.includes(content.defaultModel)) throw fail('默认模型必须在可绑定模型名之内', undefined, 'defaultModel');
  if (total > BEFORE_START_LIMITS.maxConfigBytes) throw fail(`运行环境内容 ${total} 字节，超过 ${BEFORE_START_LIMITS.maxConfigBytes} 字节上限`, undefined, 'steps');
}

function validateStep(step: BeforeStartStep, content: RuntimeRevisionContent, priorScripts: Set<string>): number {
...
export function assertPathTemplate(template: string, stepId: string | undefined, field: string): void {
  ...
  const ok = template.startsWith('/') || template.startsWith('~/') || TEMPLATE_CONTEXT_NAMES.some((name) => new RegExp(`^\\{\\{\\s*${name.replace('.', '\\.')}\\s*\\}\\}`).test(template));
...
function assertReferences(template: string, content: RuntimeRevisionContent, stepId: string | undefined, field: string, allowEnv: boolean): void {
...
function isDeclared(ref: TemplateReference, content: RuntimeRevisionContent, allowEnv: boolean): boolean {
