...
import { childKey, isPresent, mergeChildren } from '../domain/record';
import { settlePhase } from '../domain/phase';
import type { LedgerScope, StoredChild } from '../ports/repositories';

/** 比较两条记录的实质内容：版本号与更新时间不算；对象键的顺序不算（jsonb 读回来会重排）。 */
function substance(record: LedgerRecord): string {
  const { version: _version, updatedAt: _updatedAt, ...rest } = record;
  return jsonHash(rest);
}
...
export function settleChildren(record: LedgerRecord): LedgerRecord {
  ...
  return jsonHash(next) === jsonHash(record.children) ? record : { ...record, children: next };
}
...
export async function commitRecord(scope: LedgerScope, previous: LedgerRecord | undefined, draft: LedgerRecord, now: Date): Promise<LedgerRecord> {
  ...
  if (!previous || jsonHash(storedChildren(previous)) !== jsonHash(storedChildren(saved))) await scope.records.replaceChildren(saved.id, storedChildren(saved));
