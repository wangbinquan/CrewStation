...
import { permissionReason, toResourceRecord, UNSUPPORTED_ACTION } from './views';
...
export async function performAction(deps: ActionDeps, actor: Actor, id: string, action: ResourceActionId, request: ResourceActionRequest): Promise<ResourceActionResult> {
  ...
  if (!handler) throw precondition(UNSUPPORTED_ACTION, { code: 'action-unsupported', owner: record.owner.module });
