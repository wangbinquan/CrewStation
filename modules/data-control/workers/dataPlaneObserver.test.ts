...
const reader = (): DataPlaneReader & DataPlaneWriter & { closed: boolean } => {
  const self = {
    ...
    ensureTemporaryRole: async () => undefined,
...
describe('数据面观测的节奏（RFC-025 第四期）', () => {
  ...
  test('尾随台账变更：从启动时的游标起，只核对数据库与访问绑定，一批取一份快照；台账读失败只告警、下一拍再来', async () => {
    ...
    const plane = reader();
    const counting: DataPlaneReader & DataPlaneWriter = { snapshot: async () => { snapshots += 1; return plane.snapshot(); }, dropRole: plane.dropRole, ensureDatabase: plane.ensureDatabase, ensureTemporaryRole: plane.ensureTemporaryRole, close: plane.close };
