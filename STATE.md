...
作者已明确裁定 I27 选 (a)：归档保留命名空间、额度与网络策略，工作卷和其他资源处理完后由管理员删除。口令轮换如何更新已运行使用者尚无设计，已登记 I31 并询问。T15 三类单身份突发限流与恢复、资源 SSE 保持连接已实测，Q4 现值与未登录 IP 默认值（I33）待作者裁定（acceptance §13）；T9「同 Host 唯一」与现有内部 API 前缀分流的冲突已登记 I32 并询问。剩余 T6 失败会话重建、T9 开发预览 route 拆分与同 Host 仲裁、管理员归档命名空间清理入口及最终 RC/T17 尚未完成。未提交的 `tests/e2e/referenceResources.test.ts` 为接手前已有改动，原样保留，不纳入本轮提交。
