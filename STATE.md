...
## RFC-025 统一资源管理中心：第二期完成（工作区、执行环境与档位测试的创建已移交，重建待移交）、第三期服务槽、路由与限流进行中、第四期命名空间与网络策略上线、数据资源进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-i28b-20260924`（784ab558，含并行会话的 3a7f4cb5 与 dcb60115；cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:app-access-20260924`（并行会话 3a7f4cb5 上线），平台路由清单已按 de9dcb5b 应用。
...
- **第三期（T8，服务槽与构建）**：4aeaa892、6d54a25f、7c3e68d0、25bf918e（槽记录、副本认领、崩溃重启、健康照槽记录）；2b61586f（保留计时 `RetentionDeadline`，槽卡随推送流重读）；b2e19fdc、c2be2128（统一预检接入重新部署、流水线部署、切流与发布受理）；3c5315e0（构建与迁移 Job 进台账）；26033e7c（概览摘要随推送流重读）。槽与 Job 的建、改仍由 release 执行（I25 已裁定，排在任务容器之后）。
- **第三期后半（T9，路由）**：
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前四步已做（a30d1846、0cfa6cba、6185e4b4、66c76d8b，只剩失败会话重建），I28 前两步已做（0b0c43db、784ab558，只剩口令轮换）。
...
- **注意**：
  ...
  - 集群管理对项目命名空间里的网络策略、额度与路由仍给「删除」（按 UID 直接删；被路由引用的中间件本来就不给），删完调和器随即按期望补回；运维操作经台账是 T13 的事。
