...
## RFC-025 统一资源管理中心：第二期完成（工作区的创建已移交，执行环境与重建待移交）、第三期服务槽、路由与限流进行中、第四期命名空间与网络策略上线、数据资源进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-i25a-20260924`（a30d1846，cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:cluster-actions-20260924`（01c9e79c），平台路由清单已按 de9dcb5b 应用。
...
- **第二期（T6、T7，任务类容器）**：
  ...
  - 还没做：执行环境（CLI、headless、子任务）、失败后的重建与档位测试的创建移交；检出用的 Git 凭据 Secret 改为渲染时向 scm 要。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 第一步已做（a30d1846），I28 待实施。
...
- **注意**：
  ...
  - 本机节点的 CPU 请求已到 88%（可分配 10 核里 8.855 核），业务任务的子任务执行 Pod 眼下调度不上；要跑通 `/chat` 全程得先空出 CPU，释放哪些会话或缩哪些槽由作者定。
