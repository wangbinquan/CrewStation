...
## RFC-025 统一资源管理中心：第二期大部分、第三期前四步上线（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3、§4。

- **第二期（T6、T7，任务类容器）**：
  - 8df032a0：task-runtime 在环境落库的同一事务里把期望与领域条件投影进台账（保存点隔离，投影失败只告警），每 5 分钟补投影；cluster-control 尾随台账变更、按记录从观测缓存补观测。
  - 05bb863a、4126f4b2（T7）：工作台改读「快照＋推送」——形态图的开发会话／CLI／业务任务、概览 CLI 数与会话徽标、开发页 CLI 标签的结束与失败都以台账为准；守卫用例把按 CLI lifecycle 分支的存量登记为「只减不增」。
  - 99e93569：回收移交——「不要了」的记录由调和器按 UID 删子对象；失败保留期从失败时刻起算，到期转「不要了」，工作卷写「待回收」。
  - 4d492e70：孤儿回收——09-23 16:48:25Z 第一轮删掉 11 个遗留对象（旧 Runner Secret、改名前的同 Host 预览路由），3 个孤儿 PVC 登记为待回收；每个开发预览主机只剩一条路由。
  - 还没做：额度按台账推导（退役 `task_runtime.admissions` 计数器）、名册 lifecycle 在服务端由记录推导（§11.2）、建与重建移交调和器（等 I25）。
- **第三期（T8，服务槽）**：
  - 4aeaa892：每个服务两条稳定的 `service-slot` 记录，领域条件 `Serving` 表达下线与尚未部署；调和器观测 Deployment。
  - 6d54a25f：槽的旧接口状态在流水线判定就绪之后照台账（副本没全就绪是降级）。
  - 7c3e68d0：形态图槽带的入口与 Deployment 状态照槽记录。
  - 25bf918e：槽的副本由槽记录认领；崩溃重启按 G22 汇总成条件 `CrashLooping`（到期复核）；健康接口、告警巡检与健康卡照槽记录（台账读失败时退回按请求读集群）；视图缺省列出稳定记录；**修复第一期的压缩条件**——只压缩期望已是「不要了」的终态（此前待回收的卷满 7 天会失去对 PVC 的认领，本机最早 09-30 到期）。
  - 下一步：发布页槽卡改读记录（去掉 5 秒轮询，RFC-021 的保留计时作为条件 `RetentionDeadline` 进记录）、构建与迁移 Job 记录、统一预检接入发布／重新部署／切流。
- **待作者裁定**：I24（推送流的复核授权）、I25（调和器建带凭据的 Pod 与 Secret 时凭据放在哪）。
...
- **注意**：
  - 降级分支（服务槽副本没全就绪、崩溃重启）只有模块用例；实机要对共享集群的槽做运维重启才能看到，这一步被权限拦下，没做。
  - 浏览器登录已过期，没有替作者重新登录；工作台的推送效果由组件用例驱动假推送流核对。
  - 参考面板的「接收事件」用例（`referencePanel.test.tsx`）在整层跑时偶发失败一次，单独 5 次与重跑都通过，与 RFC-025 无关。
