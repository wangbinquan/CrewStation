...
接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-i25c-20260924`（66c76d8b，含并行会话的 3a7f4cb5；cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:app-access-20260924`（并行会话 3a7f4cb5 上线），平台路由清单已按 de9dcb5b 应用。
...
- **第四期（T12，数据资源）**：
  ...
  - 还没做：建库建角色与轮换（等 I28）；本机没有进行中的访问绑定，绑定的投影与删角色只有用例；数据面上的两组孤儿库与角色（`cs_cluster_business`、`cs_cluster_business_dev`）留给 T14。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前四步已做（a30d1846、0cfa6cba、6185e4b4、66c76d8b，只剩失败会话重建），I28 待实施。
