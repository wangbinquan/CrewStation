...
## RFC-025 统一资源管理中心：第二期完成（创建移交除外）、第三期服务槽与路由进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§5。本机控制面眼下是 `cs-control-plane:rc025-t9d-20260924`（cb13edb7），工作台是 `cs-console:rc025-p3i-20260924`（26033e7c）。
...
- **第三期后半（T9，路由）**：
  ...
  - 还没做：说明页与去掉 `allowEmptyServices`（等 I26）、同 Host 唯一、开发预览路由进 `route`、放行表定时全量核对。
