...
基线三件套已回填 v0.3.5。最终完整 check＋覆盖率通过：1864 pass／5 skip／0 fail，10304 assertions；历史 Agent 流测试已通过受控复现修正等待条件，测试库磁盘不足导致的无效轮次另行记录。新增代码防护为 2495／2526 行（98.77%），所有 251 个变更生产文件被用例加载；366 个共享生产／测试候选文件哈希与通过门禁的快照一致。console build 通过，正在精确提交并核对发布 SHA 的 CI。独立详细证据见 `proposal/rfc/RFC-011-role-based-home/acceptance.md`。
...
作者已批准“完整实现 RFC 并提交上库”；范围包括 CrewStation 项目资源和内置服务的盘点、重启、副本调整和删除。
生产首笔本地提交 `b7fb3e52b585bafc08bac8dd16d15a57007eee5b`：136 精确路径，包含 cluster-management L6、六页签管理台、UID 用途归属、持久快照/操作、发布槽覆盖、开发工作区保卷重启、CLI/Agent 和业务任务领域动作、档位测试终止及对应回归。
补正提交 `73ad1a0` 包括真实内置组件目录、操作 UID／阶段筛选和 RFC 证据；共享 project 投影、platform 组合根、app 导航/路由和能力 E2E 随 RFC-011 的 `b3d8d0e` 完整提交。RFC-012 的 11 项交接输出均完整保留。基线三件套已回填至 v0.3.6（R54／D53），整批远端发布与精确 SHA CI 正在收口。

定向回归通过：分类/分页/410/取消、响应丢失与继续核对、发布槽覆盖跨发布、业务持久工作区及子任务幂等、父工作卷/配额、档位测试终态、console 操作交互与焦点返回。
真实数据库投影覆盖 506 条目录的跨页与归档/已释放记录；真实平台装配经 HTTP 受理业务工作区重启/关闭。
最终前后端镜像已在本机就绪；console 为 `rfc010-20260920-2`，generation 89，Pod imageID 已核对。1280/390/320px、中英文、浅深色无整页横向溢出或浏览器异常；实际 Pod 详情／事件／日志均 HTTP 200，列表点击与直接 URL 的详情焦点可见，关闭后返回原资源行，操作 UID／阶段筛选可用。
恢复测试库磁盘后的共享完整 `check + coverage`：1864 pass / 5 skip / 0 fail；新增行覆盖率 98.77%，生产构建通过。没有为纯文档回填重跑整套门禁。

本机 `docker-desktop` 上 `crewstation-control` 尚缺新增读取/patch/Ingress 删除权限；两次自动审批拒绝 apply。
已提出具体权限范围的授权问题，尚未获答；不重试、不绕过。20:03 CST 本机快照为部分清单（278 项、27 个控制器、33 个 Pod，84 个403来源），缺失的 ReplicaSet 读取导致系统 Pod 仍显示归属／用途待核对，不能宣称全量清单或真实运维动作已经验收。
后续：精确发布与目标 SHA CI → 得到具体 RBAC 授权后应用并在专用项目验收真实动作。详情和 CM 逐项状态见 `proposal/rfc/RFC-010-cluster-management/acceptance.md`。RFC 保持 **In Progress**。
...
## 2026-09-20 RFC-012：项目算力授权与默认可见性（本地候选）
...
真实 PostgreSQL 后端／契约 46 条与工作台 27 条定向验证通过，生产构建和 1440／390／320px 本地构建浏览器交互通过。与 RFC008 合用共享完整门禁；新增项目算力页 E2E 需要部署新候选，旧共享部署不含该页面，不能视为线上通过。未在本任务暂存、提交、推送、部署；保留共享路由、wiring、i18n、迁移锁与文档中的并行输出。发布和精确 SHA CI 仍待后续，RFC 保持 In Progress。临时证据及共享 gate 状态见 RFC012 plan.md。


RFC012 补充：共享候选 `rfc011-20260920-1` 已由 RFC011 会话部署，项目算力管理页与两个真实查询接口 E2E **1 pass／6 assertions**（未改真实项目配置）。本功能相关定向合计 **96 条通过**。RFC011 请求先提交独占路径避免引用未提交新文件，已协调 RFC010/011 的短 Git 窗口；本批 74 个独占文件，20 个共享文件完整交接，清单和证据见 RFC012 plan.md。不得在共享接线／迁移锁等依赖未收口前单独推送本批。


RFC012 独占路径已本地提交 `5fb7e6c`（74 files，标准 Codex co-author），提交路径和候选哈希逐项核对、index 为空；没有执行 push。20 个剩余共享文件的完整清单在 `proposal/rfc/RFC-012-project-compute-access/plan.md`；已交接 RFC010/011，不能删除或剥离并行 hunk，必须共同收口后再发布。本功能现在已在共享测试环境随 `rfc011-20260920-1` 部署并通过真实页面/API 验收；该部署工作树与远端提交状态是两回事，整仓精确 SHA CI 仍待共享候选完成。
