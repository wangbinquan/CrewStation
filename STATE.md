...
## RFC-025 统一资源管理中心：第二期完成（创建移交除外）、第三期服务槽、路由与限流进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§5。本机控制面眼下是 `cs-control-plane:rc025-t10c-20260924`（de9dcb5b），工作台是 `cs-console:rc025-t10c-20260924`（de9dcb5b），平台路由清单已按 de9dcb5b 应用。
...
- **第三期（T10，限流）**：
  ...
  - 还没做：管理端界面（平台设置里的限流卡、项目管理页的覆盖卡）；T15 校准——平台接口与用户域要登录后才能打到限流，需要和作者一起测；未登录请求的按 IP 限流（Q4 没给取值）。
...
- **待作者裁定**：I24（推送流的复核授权）、I25（调和器建带凭据的 Pod 与 Secret 时凭据放在哪）、I26（说明页的错误体与 RFC-021 不一致；去掉 `allowEmptyServices` 会让暂时没有端点的窗口变成裸 404）。
