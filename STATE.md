...
本轮先推进 T13：集群展开项目拓扑接全平台资源快照与 SSE，按项目过滤；capabilities 开发摘要的阶段、原因与连接事实改读台账，缺记录不回退旧 running。新增回归已先红后绿，本地静态门禁通过，unit 713/0、module 1380/7 skip/0 fail、console 902/0，新增可执行行覆盖 52/52；发布与部署进行中，证据集中在 RFC025 acceptance §12。另修资源维护用例的时钟漂移：该组按数据库时间巡检，却固定在 09-23；现只在该组初始化时对齐数据库时钟，22/22 定向通过，未改生产保留期规则。

作者已明确裁定 I27 选 (a)：归档保留命名空间、额度与网络策略，工作卷和其他资源处理完后由管理员删除。口令轮换如何更新已运行使用者尚无设计，已登记 I31 并询问；其他实现继续。未提交的 `tests/e2e/referenceResources.test.ts` 为接手前已有改动，原样保留，不纳入本轮提交。
...
- **问答裁定**（两轮）：
  - 已有应用照现有设置立即生效。
  - 批准即加为「用户」：成员页新增第四种角色；可见范围收成「项目成员（含用户）」「全部登录用户」两档，原指定用户迁成「用户」。
  - 无权限页在原地址显示，按钮新开工作台申请页。
  - 负责人可设「允许申请／只能授权」，默认允许；只能授权时页上写负责人名字。
  - 三处审批入口：应用展示页、项目概览提醒、管理空间「申请审批」。
  - 接入容器只给管理员与成员。
  - 流程：直接改＋回填，提交、推送并部署本机。
- **改了什么**（3a7f4cb5）：
  - 网关：identity 新端口 `AppAccess`。ForwardAuth 对正式地址逐请求问 project，一次联查项目、可见范围与成员角色。拦下时浏览器得到 403 与「没有项目权限」页，程序调用得到 403 JSON（`details.reason=app-access`），会话不清。
  - 待命版与开发预览的规则不变，「用户」不算在内；正式版本维护中，「用户」与外人一样看维护页。
  - project：新增 `user` 角色，项目里一个动作都没有；可见范围两档，外加 `allowRequests`。
  - 新表 `project.app_access_requests` 与四个接口。迁移 `project/0012`：指定名单迁成成员，删 `user_ids`，加 `allow_requests`。
  - 工作台：成员页加「用户」角色；可见范围弹窗两档外加「申请」一项；应用展示页「使用申请」卡，拒绝在弹窗里写理由；项目概览提醒；管理空间「申请审批」加一栏；申请页 `/apps/:projectId/access`。
  - 回填：Design v0.3.15（§1.1 第 6 条、§4.3、§7.1、§7.3、新 §7.5、D63）；Proposal R49 与 §0.2；Plan T1.4、AT-54、R49 矩阵行；RFC-003 市场可见性与 RFC-009 §3.4 加日期修订；CLAUDE.md；dev-gotchas 记「迁移 SQL 别写别名.列」。
  - 355628fc（上线后实机发现）：管理空间里应用使用申请一栏的翻页条原先在卡片里，挪到卡片外；两栏之间原先贴在一起，现在留出间距。处理结果的提示也挪到空状态之前。
- **门禁**：3a7f4cb5 在 git archive b67e663f 干净导出树加本批文件上跑：arch、lint、两个 typecheck 通过，2949 pass／99 skip／0 fail。CI [35964088577](https://github.com/wangbinquan/CrewStation/actions/runs/35964088577) 六项全部成功。355628fc：工作台层 900 pass，CI [35964716967](https://github.com/wangbinquan/CrewStation/actions/runs/35964716967) 六项全部成功。
- **部署**：
  - 06:22:36Z 迁移 project/0012，只有这一个。
  - 06:22:45–06:23:21Z cs-api、cs-auth、cs-controller、cs-session 换到 `cs-control-plane:app-access-20260924`（3a7f4cb5），console 换到 `cs-console:app-access-20260924`。
  - 随后 crewstation-90 把这四个控制面滚到 `rc025-i25c-20260924`（66c76d8b，含本批）。
  - 06:29:32–06:30:04Z console 换到 `cs-console:app-access-2-20260924`（355628fc）。
- **实机**（开发角色走 OIDC，curl 加无头 Chrome）：
  - 未登录时打开 `demo.cs.localhost/some/page?x=1`，302 到工作台登录页，带完整原地址；登录后跳回原地址，路径和查询串都保留。
  - demo（范围「项目成员」）：dev-member 得到 403「没有项目权限」页，带「申请访问权限」，新开 `/apps/<id>/access`；程序调用得到 403 JSON；dev-developer（开发者成员）与 dev-admin 都是 200。
  - rfc021-verify（全部登录用户）：dev-member 200。
  - reference-api-proxy（接入容器）：dev-member 403，页上只写「请联系项目负责人：CrewStation Admin」，没有申请入口。
  - preview.demo：dev-member 仍是原来的 403「无权访问这个地址」页。
  - 申请链：dev-member 提交得到 201，重复提交 409。dev-admin 在项目概览看到「1 条使用申请待处理」，管理空间「申请审批」里也列出这条；在应用展示页的「使用申请」卡上点「同意」。随后 dev-member 打开 demo 得到 200，`/v1/me` 多了 `role: user`，preview.demo 仍是 403，申请页显示「你已经可以使用这个应用」。
  - 收尾：dev-member 的「用户」成员已移除（开发角色「用户」按设计没有成员关系），demo 对它恢复 403；那条已同意的申请记录保留。
- **影响**：本机 11 个数字人里，10 个是「项目成员」范围，非成员现在打不开它们的正式地址（按裁定立即生效）。rfc021-verify 是「全部登录用户」，不受影响。
- **注意**：e2e 只以一个管理员身份跑（CI 用密码登录，只有引导管理员有口令），所以两个身份的申请链没有写成 e2e，靠本节实机记录与模块、工作台用例覆盖。

## 集群管理详情的操作按钮：同一行顶端对齐，删除类按钮用红框（2026-09-24）

作者：「集群管理界面的右侧详情框，重启、调整副本等四个按钮的排版都错开了，太丑了，你自己看看，把排版优化下」。

- **原因**：四个动作两两一行，不可用的原因写在各自按钮下，外层按格子垂直居中；有原因的格子更高，同一行的按钮就上下错开（改前实量 buildkitd：「重启」比「调整副本」低 11px，第二行差 9px）。英文下「Restore release replicas」184px，格子只有 169px（格子写了 `min-width: 0`），按钮压到右边的「Delete / stop」上。
- **问答裁定**：在「两列网格＋原因合并」「竖排＋原因在右」「收进『操作』菜单」「只做顶端对齐」四种里选了只做顶端对齐：保持两两一行，原因仍写在各自按钮下。收尾：提交、推送并部署本机。
- **改了什么**（01c9e79c）：
  - `.actions` 改为顶端对齐。
  - `.action` 去掉 `min-width: 0`，格子不窄于按钮；中文下宽度不变。
  - 「删除／结束」和资源中心记录的「释放」「删除工作卷」按全站按钮规范用红框（提问时写明了，作者没有异议）。
  - 「资源中心记录」的操作用同一套样式，一并对齐。
- **用例**：组件用例锁住样式（顶端对齐、格子不写 `min-width`）和按钮样式，两条断言在旧代码上都失败。e2e `clusterLayout` 新增一条实量：打开平台组件工作负载的详情，中英文下同一行按钮的顶边相差不超过 1px，按钮不超出格子。
- **门禁**（`git archive` 干净导出树＋本批 6 个文件）：arch、lint、两个 typecheck 通过；测试 2892 pass／99 skip／0 fail（2991 个用例，514 个文件）。CI [35958053642](https://github.com/wangbinquan/CrewStation/actions/runs/35958053642) 六项全部成功。
- **部署与实机**：
...
## RFC-025 统一资源管理中心：第二期完成（工作区、执行环境与档位测试的创建已移交，重建待移交）、第三期服务槽与构建的创建已移交、路由与限流进行中、第四期命名空间与网络策略上线、数据资源进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-t8b-20260924`（9505f329，含并行会话的 e4f15449；cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:app-access-2-20260924`（并行会话上线），平台路由清单已按 de9dcb5b 应用。

- **第二期（T6、T7，任务类容器）**：
  - 8df032a0、99e93569、4d492e70：期望与领域条件投影进台账；回收移交调和器（按 UID 删子对象，失败保留期从失败时刻起算、到期回收、工作卷写「待回收」）；孤儿回收。
  - 05bb863a、4126f4b2（T7）：工作台改读「快照＋推送」。
  - 84e1fc50：额度经台账受理——按阶段数、结束中仍占，恢复与失败后重建重新受理；`task_runtime.admissions` 只剩行锁。
  - 7551a493：CLI 名册的结束与失败在服务端照台账推导，每项带 `phase`（§11.2）。
  - a30d1846（I25 第一步）：开发会话、业务任务的工作区改由调和器照记录建出。期望不含凭据；调和器建 PVC、不可变的 Runner Secret（`<Pod 名>-runner-<第几次启动>`）、只 `envFrom` 它的 Pod 与开发预览，建 Secret 之前经端口向 task-runtime 要内容（Runner 令牌这时签发、只存哈希），Pod 建出后交回实例；观测到过又没了的不补建，建不成的写 `Created` 为假带原因。迁移 `task_runtime/0010`；`CS_WORKLOAD_CREATION=owner` 回退为 task-runtime 自己建。实机：rfc006-verify 以服务身份调 `/chat`，受理后 2.7 秒建出、6.5 秒 Runner 连上，Pod 规格里没有明文环境变量；子任务的执行 Pod 因节点 CPU 请求满了（差 5m）调度不上，Agent 输出没有走到（acceptance §3.7）。
  - 0cfa6cba（I25 第二步）：CLI、headless Agent 与业务子任务的执行环境也由调和器建出。建之前照观测缓存核对父工作区的 Pod 与卷还是受理时那两个实例（没了或换了交 task-runtime 判失败），Runner Secret 沿用 `<Pod 名>-runner`，交回 Pod 与 Secret 两个实例；过了宽限仍在排队的由对账判失败。受理意图的摘要加了与键先后无关的写法（jsonb 会重排档位对象的键，旧摘要让清理认不出资源中心建的 Pod），比对时新旧都认。工作卷观测到了随即唤醒工作区（不再等 2 秒）。实机：rfc006-verify 以服务身份跑一次子任务（业务任务指定 150m 套餐），工作区 Runner 连上后 0.4 秒建出执行 Pod、3 秒后它的 Runner 连上，46 秒输出「收到」；关闭后执行环境与工作区都正常收尾（acceptance §3.8）。
  - 6185e4b4（I25 第三步）：开发会话检出用的 Git 凭据改为每次启动一个 Secret（`<Pod 名>-checkout-<第几次启动>`），作为工作区记录的子对象由调和器在 Pod 之前建，令牌建的时候才签；不再写按服务共用的 `git-checkout-*`。没有实机走过（开会话要登录），子任务回归照常（acceptance §3.9）；下次本机开开发会话时应看到这个 Secret，建不出来可设 `CS_WORKLOAD_CREATION=owner` 回退。
  - 66c76d8b（I25 第四步）：档位测试的 Pod 由调和器建（临时目录、不等卷），Runner 令牌不再以明文环境变量写进系统命名空间的 Pod 规格；等 Runner 的轮询不把「还没建出来」当成容器没起来。档位测试本身没有实机走过（管理员触发，要登录），子任务回归照常（acceptance §3.10）。
  - 还没做：失败会话重建的创建移交（替换旧 Pod、等子执行回收、失败补偿，与逐条调和对不上，单独设计）。
- **第三期（T8，服务槽与构建）**：4aeaa892、6d54a25f、7c3e68d0、25bf918e（槽记录、副本认领、崩溃重启、健康照槽记录）；2b61586f（保留计时 `RetentionDeadline`，槽卡随推送流重读）；b2e19fdc、c2be2128（统一预检接入重新部署、流水线部署、切流与发布受理）；3c5315e0（构建与迁移 Job 进台账）；26033e7c（概览摘要随推送流重读）。
  - 42fcec04（按 I25 同一裁定）：服务槽的 Deployment、Service 与这一次部署的环境 Secret（`<服务>-<物理槽>-env-<第几次部署>`）改由调和器建出。部署、重新部署只把期望随槽状态写进台账（不含配置与密钥），环境在建 Secret 时经端口向 release 要（实际用上的配置版本记回发布）；Deployment 只 `envFrom` 这个 Secret、注解写渲染它的期望版本，被人改了就改回，新版本铺完后删旧环境。流水线照槽记录判铺开（观测到的 Deployment 得是渲染最新期望的那个，Deployment 子对象新增 `appliedGeneration`）；建不成由调和器报给 release、槽记为失败，流水线据此判发布失败；下线由调和器先删 Deployment 再删环境、Service 保留；集群管理的扩缩、恢复、重启、删除改为在受理的事务里改期望（C6），完成照槽记录判；槽的 Service 与环境 Secret 在清单里不给直接删；重新部署的集群预检经 cluster-control 的 `dryRunSlot`。上线前部署的 20 个槽是旧形状，照旧由 release 管，下次部署时换新形状。实机：上线后 20 个槽的 Deployment 一个没动；新形状只用服务端 dry-run 核对过（套在 rfc006-verify 现有 Deployment 上被接受、明文环境变量没了），真正建一次要发布或重新部署（要登录），没有做（acceptance §4）。
  - 9505f329：构建、迁移 Job 同样由调和器建出——流水线只写期望，凭据 Secret（构建的只读 Git 令牌，不再写 `git-cred-<服务>`；迁移的槽环境）建的时候才要，Job 结束或流水线放弃即删，Job 交给 TTL、建过不补建；流水线照 Job 记录判结果（说法照旧），Job 一直没建出来按截止时间加 5 分钟判超时。运维开关 `CS_RELEASE_CREATION=owner` 同时回退槽与 Job。实机只有服务端 dry-run（acceptance §4）。**下次本机有人发布或重新部署时**：看槽记录与 Job 记录的 `spec` 里有 `slot`／`job`、项目命名空间里出现 `<服务>-<物理槽>-env-N` 与 `<Job 名>-env`（Job 结束后没了）、Deployment 规格里没有明文环境变量；不对就设 `CS_RELEASE_CREATION=owner` 回退。
- **第三期后半（T9，路由）**：
  - 3ddccf23：服务路由投影成 55 条 `route` 记录（每 5 分钟补投影）。
  - aed62f71：身份索引墓碑保留 7 天、每小时清理（上线即删 189 行）；补投影、清理这类定时作业收成 `periodicJob`。
...
- **第三期（T10，限流）**：
  ...
  - de9dcb5b：用户域、服务域与平台接口的路由挂上限流；资源推送流单独一条、不挂并发上限；工作台读请求按 `Retry-After` 自动重读；接入约定写明 429。服务域实测：130 个突发里 100 个 200、30 个 429（`Retry-After: 1`）。
  - 5fb115ba：管理端「平台设置」里的网关限流卡、项目管理页的限流卡（单独设置与撤销）。
  - 还没做：T15 校准——平台接口与用户域要登录后才能打到限流，需要和作者一起测；未登录请求的按 IP 限流（Q4 没给取值）。
- **第四期（T11，命名空间、额度与网络策略）**：
  - 6db2d958：provisioning 给每个项目写 `namespace`（Namespace＋额度）与 `network-policy-set` 两条记录，开通链写期望后等两条都运行中再建仓，启动重下发只写期望；调和器照记录渲染（网络策略按名字取模板），被改或被删就补回，从不删。上线前后 73 个线上对象逐个对照不变；实机把额度改大、删掉一条网络策略，都在 1 秒内改回、补回。
  - 调和器渲染的种类（路由、中间件、命名空间、额度、网络策略）观测到变化就直接核对认领它的记录——额度与命名空间没有 `generation`，改了台账不记变更。
  - 没有实机走过的：新建项目时的「等运行中」（要登录管理端建项目），命名空间标签被改后改回（只有模块用例）。
- **第四期（T12，数据资源）**：
  - 5e49a657：data 把生产库、开发库写成 28 条 `database` 记录（子对象是库与同名运行角色，连接串不进台账），访问绑定写成 `data-binding` 记录（挂在会话下，等批准排队、生效报 Granted）；新模块 `data-control`（L2）尾随台账变更加每 30 秒全量，观测 `cs_` 前缀的库与角色（OID 当 UID）。上线后 28 条全部运行中。
  - 5e088e66：「不要了」的访问绑定，还在的临时角色由 `data-control` 删（先核对 OID，只删临时角色形状的名字）；data 自己的删除暂时保留。
  - ff561324（顺手修）：超时的终端探测命令退出码一律为空，整层跑模块级用例时的偶发失败；随下次任务镜像生效。
  - 0b0c43db（I28 第一步）：生产库、开发库改由 data-control 建——口令它生成、加密存 `data_control.credentials`，data 开通时只写期望、等记录就绪，渲染连接串时经端口要口令；已存着连接串的旧库照旧（本机 28 个都是，上线后调和器一个也没建）。`CS_DATA_PROVISIONING=data` 回退。新项目开通要登录，没有实机走过，由真实 PostgreSQL 上的贯通用例核对（acceptance §8）。它的 CI 暴露了一个真竞态：按记录核对读到旧版本 Job 会把已记下的 Finished 改回假（Job 随后被 TTL 删掉就再改不回来），7fef93f0 修了（Finished 为真之后不再改），07:30 随 784ab558 上线。
  - 784ab558（I28 第二步）：访问绑定的临时角色也由 data-control 建（批准时只置为生效、等它建好再答复；渲染时经端口要口令），开发模式的绑定不再存开发库连接串的副本。本机没有进行中的绑定，没有实机走过，由贯通用例核对（acceptance §8）。7fef93f0 的竞态修复随这次上线。
  - 还没做：口令轮换；本机没有进行中的访问绑定，绑定的投影与删角色只有用例；数据面上的两组孤儿库与角色（`cs_cluster_business`、`cs_cluster_business_dev`）留给 T14。
- **第五期（T13，集群管理）**：903167c7——删除待回收的工作卷由资源中心自己受理（只给管理员、只对待回收的卷，期望改为「不要了」，调和器删 PVC；权限不足一律 403）。a89589ab（按 I29 裁定）——清单与历史仍以快照为底，台账认领的行叠加所属标准记录（阶段、原因、可做操作、是否由资源中心维护）；路由、限流与前缀剥离中间件、网络策略、命名空间与额度的「删除」禁用并写明会被补回，工作卷的 PVC 指向「待回收的工作卷」，删除前的检查按实时对象再核对（台账读不到就不给做）；「待回收的工作卷」排在「存储与配置」之后。顺带：标准输出里没有执行者的操作如实写「这类资源暂不支持在这里操作」（工作区的释放、重试此前显示可做、受理才 412）。实机：网络类 237 个对象里项目命名空间的全部被认领，没叠加的 33 个都是平台组件。本机 3 个孤儿工作卷在「待回收」，删不删由作者定；它们的记录没有项目 ID（收编时没按命名空间找项目）；951f4159 修了以后收编的（随 a30d1846 的镜像上线），已有的 3 条仍写「—」。还没做：拓扑读全平台视图、`capabilities` 摘要改读视图。
- **第六期（T14，收编）**：ed208846——服务槽的 Service 与内部 API 的前缀剥离中间件挂到已有记录下（槽下线时 Service 照旧保留，不算结束中）；孤儿回收扩到没人认领也没人引用的中间件，实机删掉两个代理改名后留下的旧中间件。151b0267——tsk_… 旧身份写成记录的别名（6 条）。还没被认领的只剩服务的 Git 凭据 Secret：开发会话检出用的新会话不再写（6185e4b4），已有的 `git-checkout-*` 与构建用的 `git-cred-*` 待收编。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前四步已做（a30d1846、0cfa6cba、6185e4b4、66c76d8b，只剩失败会话重建），同一裁定下服务槽与构建、迁移 Job 也已移交（42fcec04、9505f329），I28 前两步已做（0b0c43db、784ab558，只剩口令轮换）。
- **待作者裁定**：I24（推送流的复核授权）、I27（项目归档后命名空间记录与命名空间怎样收尾；本期归档不释放、不删）。
- **保留期满（09-24 实测，acceptance §3.3）**：三个 09-21 失败的旧会话 04:18:58 由维护作业改成「不要了」，调和器随即删掉 Pod、预览 Service 与路由，三个工作卷进「待回收」，04:20:59 task-runtime 记为已释放；「待回收的工作卷」现有 6 个（3 个孤儿、3 个保留期满），删不删由作者定。**到点要看**：rfc022-verify 的两个失败会话 09-26 06:35Z 到期。
- **注意**：
  - 降级、崩溃重启、路由被人改动后改回，只有模块用例；实机要对共享集群的槽或路由动手，没有做（运维重启被权限拦下过）。
  - 浏览器登录已过期，没有替作者重新登录；工作台的推送效果由组件用例驱动假推送流核对。
  - 本机节点的 CPU 请求已到 88%（可分配 10 核里 8.855 核）：样例 `/chat` 用默认套餐（1 核）建业务任务，子任务的执行 Pod 差 5m 调度不上；实机核对改用 150m 的套餐。要让 `/chat` 本身跑通得先空出 CPU，释放哪些会话或缩哪些槽由作者定。
  - 整层跑时偶发失败过、重跑都通过的用例：`referencePanel.test.tsx`、`runtimes/task/tests/runnerLifecycle.test.ts`、`overviewSlotLifecycle.test.tsx`，与 RFC-025 无关。
  - 集群管理里台账维护的对象（路由、中间件、命名空间与额度、网络策略，a89589ab；资源中心建的槽的 Service 与环境 Secret，42fcec04）不给直接删；槽的运维经台账（42fcec04），工作区、执行环境的运维仍走 RFC-010 的领域操作。
  - `useRecordRefresh` 没有节流：记录一变就重读页面数据。空写循环期间开着的概览页会跟着频繁重读；修复后只随真实变化，节流作为纵深防御留待下一次改工作台时加。

## 项目管理只列数字人，接入容器相关页面归能力接入，开通失败拆成两张卡（2026-09-24）

作者：「现在在系统管理的项目管理和能力接入列表里都放了能力接入类型的项目，你现在需要在项目管理列表里不显示能力接入类型的项目才对」；随后补充「项目管理里不要有那个新建接入容器的按钮」。

- **原因**：项目管理页的 `ProjectDirectory` 按 `DigitalWorker＋APIProxy＋EventProducer` 三类读取（RFC-003 第三十四批写的是「承接全部类型」）。接入容器目录复用同一组件，只读两类，所以接入项目两处都有。
- **改前实测**（dev-admin，无头 Chrome）：
  - 项目管理 14 行里有 3 个接入项目：GitLab 事件生产者、参考 API 代理、RFC-003 管理接入验收。页上有「项目类型」筛选、「类型」列和「新建接入容器」。
  - 接入项目侧栏的返回、顶栏「平台管理」、接入项目资源配置页的返回，都落到项目管理。
- **问答裁定**：
  - 接入容器相关的页面全部归「能力接入」：返回去向和左栏高亮都改，数字人不变。09-21「返回固定到项目管理」对接入项目作废。
  - 项目管理的类型筛选和类型列都去掉。
  - 总览「开通失败」拆成两张待办卡。
  - 流程：直接改＋回填，提交推送并部署。
- **改了什么**（2aaf6e46）：
  - 项目管理只按 `DigitalWorker` 读，去掉类型筛选、类型列与「新建接入容器」，旧链接的 `kind` 忽略。能力接入的接入容器目录保留类型筛选与类型列。
  - 接入项目侧栏改为「← 返回接入容器」。它和下面三处都回 `/admin/capabilities?tab=integrations`：项目不存在时的返回、项目里顶栏的「平台管理」、接入项目资源配置页的返回。新建页还没有项目，顶栏照旧去管理总览。
  - 左栏当前项改由 `currentAdminPage`（`shared/admin/adminNavigation.ts`）统一计算。原因：路由自己按前缀判定当前时，会强行写 `aria-current`，调用方盖不掉（记进 dev-gotchas）。
    - 新建接入容器、接入项目的开通页与资源配置页，路径虽在 `/admin/projects` 下，也高亮「能力接入」。
    - 项目种类读到之前不标当前项，只替管理员读项目（`useIntegrationPage`）。
  - 总览拆成「数字人开通失败」「接入容器开通失败」两张卡，各读各的。入口分别去项目管理与能力接入，都带开通状态＝失败。待办格子原本就是三列。
  - `INTEGRATION_KINDS` 移到 `shared/admin/integrationKinds.ts`，加 `isIntegrationKind`。
- **回填**：RFC-003 design §2.7 加 2026-09-24 修订说明。
- **用例**：
  - 新增 `adminIntegrationScope`（7 条）。
  - 改 `adminDirectory`（11 条）：夹具的项目数可调，只列数字人时也能翻页。
  - 改 `adminProjectSpace`、`adminNavigation`。
  - 旧代码上红 15 条：老文件 10 条；新文件 5 条（先给旧代码补一个空的 `currentAdminPage` 才能加载）。另外 3 条旧代码上本来就绿，锁的是保留行为和「只替管理员读项目」。
- **门禁**：
  - 「ed208846＋本批」的干净导出树上 `bun run check` 通过：2857 pass／98 skip／0 fail。
  - 提交前 HEAD 前进到 19485d54（crewstation-3a 的 slug 保留名、I30 登记），两笔都不碰本批文件。在 19485d54 的导出树上又跑过 `check:static` 与整层工作台（889 pass）。
- **提交与 CI**：2aaf6e46，[CI 35943890055](https://github.com/wangbinquan/CrewStation/actions/runs/35943890055) 六项成功，含新增代码防护与 e2e。
- **部署**：
  - 01:42:19Z 只滚 console：`cs-console:rc025-t10d-20260924` → `cs-console:admin-dw-only-20260924`，01:42:50Z 就绪，重启 0 次，没有迁移。
  - 镜像从 `git archive 2aaf6e46` 构建，含 main 上到 19485d54 的全部内容，所以也带上了 crewstation-3a 73f6751c 的 slug 提示文案；包是 `index-K_gVCTpT.js`。
  - 同一时段 crewstation-90 只滚了控制面（`cs-control-plane:rc025-t14b-20260924`）。
- **实机**（dev-admin，无头 Chrome，只读）：
  - 项目管理 11 行全是数字人，接口里的数字人正好 11 个。没有类型筛选，表头是「项目｜负责人｜开通状态｜管理操作」，新建入口只剩「新建数字人」。
  - 能力接入列出 3 个接入项目，类型筛选（APIProxy／EventProducer）、类型列和「新建接入容器」都在。
  - 接入项目「GitLab 事件生产者」：侧栏是「← 返回接入容器」，点了落在 `/admin/capabilities?tab=integrations`，左栏是「能力接入」，列表里有它；项目里顶栏「平台管理」也指向那里。
  - 接入项目的资源配置页：左栏「能力接入」，返回是「返回接入容器」。「演示数字人」的资源配置页仍是「项目管理」与「返回项目管理」。
  - 新建接入容器页：左栏「能力接入」。
  - 总览三张待办卡：「待审批 API 申请」「数字人开通失败」（→ `/admin/projects?state=failed`）「接入容器开通失败」（→ `/admin/capabilities?tab=integrations&state=failed`）。
  - 390、1280、1440 宽都没有横向溢出，控制台无报错。
- **没做**：管理总览页头的「新建数字人」「新建接入容器」两个按钮没动，作者只点名了项目管理。

## 部署与运行形态图的刷新：切回、进入立即补读，观测时间到秒（2026-09-23）

作者问：「部署与运行形态的图，是不是没有自动更新，必须刷新页面才会刷新？」

- **诊断**（无头 Chrome 只读实测，加 Traefik 访问日志）：
  - 图在前台时确实每 15 秒重读一次，但读的是集群管理每 30 秒采集一次的快照，所以变化最长约 45 秒才上图。
  - 缺陷：全局 `staleTime` 30 秒挡掉了「回到前台补读」和进入页面时的重读，页签在后台期间跳过的定时重读也不补。实测切走 18 秒再切回，又等了 10 秒才重读。
  - 作者那次：
    - 13:47:13、13:47:29 在开发页结束了两个 CLI。
    - 形态页签 13:47:26 读过一次后转入后台，13:47:41 那次重读被跳过。
    - 13:47:46 手动刷新，才拿到 13:47:39 的快照。
  - 「观测于」只显示到分钟；悬停提示写「每 15 秒换一份快照」，与实际不符。
- **裁定**：
  - 「你盯着，等他做完你再做」：等 crewstation-90 的 RFC-025 T7 上线后再修。T7 让开发会话、CLI、业务任务改读台账和推送，提交 05bb863a，15:40Z 部署。
  - 观测时间精确到秒。
  - 直接改＋回填，部署本机。
  - 「重读时刻对齐服务端采集」作者没选，没做。
- **改了什么**：
  - `useApiQuery`：
    - 设了定时重读的查询，新鲜期缩到自己的重读周期（只缩短、不延长 QueryClient 的缺省值）。进入页面或切回标签页时，数据旧过一个周期就立即重读。
    - `refetchOnWindowFocus` 只在当前确实在定时重读时默认打开。
    - 与 `usePollingRefetch` 的「后台暂停、回前台补查」一致，工作台约 30 处同类查询一并生效。
    - 测试用的 QueryClient（新鲜期 0、切回重读）下行为不变。
  - `formatDateTime`／`useDateText` 加 `second` 精度，`TopologyStamp` 改用它，中文显示为「观测于 2026年9月23日 21:55:09」。其他页面的时间格式不变。
  - 悬停提示改为「集群快照每 30 秒采集一次，页面每 15 秒读取；节点位置按 UID 固定」。这句集群管理拓扑和项目形态图共用，对两处都成立。
- **回填**：RFC-019 design §6 加 2026-09-23 修订说明。
- **用例**：
  - 新增 `apiQueryPolling` 3 条，用生产配置的 QueryClient，拨系统时钟模拟数据已放多久，切走、切回用 `focusManager.setFocused`：
    - 切走 18 秒再切回立即补读，不满一个周期不补读；
    - 进入页面时，缓存旧过一个周期就重读；
    - 不例行重读、显式关掉补读、无限新鲜期这三种都不补读。
  - `topologyDiagram` 加两条断言：观测时间到秒、悬停提示的实际节奏。
  - 旧代码上红 3 条。
  - 第一版用例改了 `document.visibilityState`，又把焦点复位成「未定」。满负载时后面 release 系页面的用例偶发多红；改用 `setFocused` 并在结束时回到前台之后，交替对照两轮都是全绿。
- **门禁**：「99e93569＋本批」的干净导出树上 `bun run check` 通过：2707 pass／97 skip／0 fail。
- **提交与 CI**：eba454be，[CI 35889636345](https://github.com/wangbinquan/CrewStation/actions/runs/35889636345) 六项成功，含新增代码防护与 e2e。
- **部署**：
  - 16:42:12Z 只滚 console：`cs-console:rc025-t7b-20260923` → `cs-console:topo-refresh-20260923`。镜像从 `git archive eba454be` 构建，已包含 crewstation-90 的 T7 第二步（4126f4b2）；包是 `index-DB8sGXSZ.js`。
  - 16:42:43Z 就绪，重启 0 次。滚之前已通知 crewstation-90 与 crewstation-55。
- **实机**（无头 Chrome，dev-developer，演示数字人，只读）：
  - 观测时间「观测于 2026年9月24日 00:42:56」，新快照到达后变为「00:43:25」；页面仍每 15 秒重读一次。
  - 上次读取后 2 秒切走、18 秒后切回，0.03 秒后重读（修复前要再等 10 秒）。
  - 从概览离开到「发布与上线」停 20 秒，再回「运行与诊断」，点进后 0.04 秒重读。
  - 悬停提示为「快照完整 · 集群快照每 30 秒采集一次，页面每 15 秒读取；节点位置按 UID 固定」。
  - 1440、390 宽无横向溢出，控制台无报错。
  - 缓存不满一个周期时（概览卡 5 秒前刚重读过）按设计直接用缓存，15 秒后才重读。
- **回填**：RFC-019 acceptance 加 §5；dev-gotchas 加两条（全局新鲜期挡掉「回前台补读」；`focusManager` 单例的用例卫生）。
- **没做／留给作者**：
  - 重读时刻对齐服务端采集：作者没选，服务槽那半边仍最长约 45 秒才上图。
  - 进入页面时若缓存不满一个周期，第一次重读从挂载起算，显示的数据最旧约两个周期。按数据年龄提前第一次重读，要让周期随每次渲染变化，React Query 会因此反复重启定时器，这次没做。

## 宽屏只有内容区滚动：顶栏与左栏不动，窄屏保持整页滚动（2026-09-23）

作者：「你看下所有页面，还有哪些页面是有整页滚动条的，会带着左侧栏一起动的，都改成右侧独立滚动条」。

- **盘点**（dev-admin，演示数字人／参考 API 代理，无头 Chrome 逐页、逐页签量文档多出的高度，1440×900／1280×720／1920×1080）：根因只在外壳——`.shell` 只有 `min-height: 100vh`，内容一长整个文档在滚，左栏与顶栏跟着走，所以凡是内容超过一屏的页面都会这样。
  - 实测超出的页面：网关 4089／4904／2510，能力接入·API 开放策略 2168／2958／1988，集群管理·资源清单 2122／2410／1927，项目管理 689／869／509，管理总览 614／816／434，项目设置·项目信息 609／789／429，发布与上线 589／769／409，项目开发列表 586／785／406，集群管理·拓扑 584／687／361，算力档位 532／746／333，事件来源 442，任务容器规格模板 440，用户与权限 349，项目资源配置 334，接入容器发布 355，历史对话会话 147，能力市场 88，概览 59（仅 1280×720）。
  - 1280×720 下其余管理页都多出 43px：管理左栏约 711px，本身就比顶栏以下的 668px 高。
  - 当时不滚的只有开发页、调用链（bd 刚改）、设置其他组、运行与诊断其他页签，而且只是因为数据少。
- **问答裁定**：窄屏（≤800px，左栏变成内容上方的横条）保持整页滚动；没有左栏的页面（市场、项目列表、新建项目）一起改；流程「直接改＋回填，提交推送并部署」。
- **改了什么**（0316d5ab）：
  - 宽屏外壳 `height: 100dvh`，`main` 是唯一的纵向滚动区，左栏 `overflow-y: auto` 自己滚。
  - 窄屏外壳照旧随内容长高，`main` 改为 `overflow: visible`、`min-width: 0`：整页滚动时吸顶元素吸在窗口上。事先实量了 390／320 下 38 个页面，没有一页靠 `main` 横向滚动。
  - 路由 `scrollToTopSelectors` 指向 `main`（`data-scroll-restoration-id="cs-main"`）：切页回顶、返回恢复，`resetScroll: false`（调用链选中、集群切视图）不动。
  - 删掉算力档位编辑页改写 `main` overflow 的覆盖（集群拓扑详情的同类覆盖由 6b 的 b324ee04 删除）。
  - Chrome 的 sticky 按滚动容器的内容框吸附，`main` 的 24px 上内边距让保存栏上方露出一条滚过去的内容。外壳给出 `--cs-main-sticky-top`（宽屏 −24px、开发页 −8px、窄屏 0），保存栏改用它；实量宽屏贴住内容区顶边 52px，390 宽吸在窗口顶 0px。
- **用例**：
  - `shellScroll.test.tsx` 4 条（样式链、禁止页面改写 main 的 overflow、吸顶变量、切页回顶／返回恢复／resetScroll: false），改前全红。
  - e2e `shellScroll` 4 条（往内容区塞 3000px 占位，量外壳本身，不依赖环境数据），对线上旧包 4 条全红，换包后全绿。
  - `projectWorkspaceIa` 两处「整页不滚动」改为「文档＋main」量法；`clusterLayout`（6b）与 `traceChains`（bd）已各自改好。
- **回填**：RFC-003 design §6 同日修订说明；开发规则 §7「宽屏只有内容区滚动」；dev-gotchas「宽屏滚的是内容区 main，不是窗口」一条。
- **并行协调**：6b（集群管理铺满一屏）、bd（调用链两栏铺满）与本批同时改滚动，事先约定外壳归我、各页归各自；Topology 那条 `:global(main)` 覆盖与 clusterLayout 的量法由 6b 在 b324ee04 里改。
- **门禁与 CI**：
  - 提交前在「edb5d4a2＋本批」干净导出上：`check:static` 通过，用例 2613 pass／94 skip，0 条用例失败。另有三个独立项目找不到 `hono`（导出树的已知坑），补装依赖后重跑 88 pass。
  - 基线前进到 8df032a0（含 6b 的 b324ee04）后在「新 HEAD＋本批」上重跑：`check:static` 通过，工作台层 844 pass／0 fail。
  - 0316d5ab 的 [CI 35876824671](https://github.com/wangbinquan/CrewStation/actions/runs/35876824671) 六项全部成功（含 `gate` 与 `e2e`）。
- **部署与实机**：
  - `cs-console:shell-scroll-20260923`（`git archive 0316d5ab`＝当时的 origin/main，含 b324ee04、4a1fd27e、0436ce71）导入节点，14:57:30Z 只滚了 console。
...
作者：「集群管理界面，现在右侧详情栏已经把整个页面都搞出来滚动条了……你把状态卡片也挪到总揽页面顶端去，让集群管理只显示拓扑和资源清单，并且占满页面不要出现涉及整个页面的滚动条」。

- **改前实量**（作者的 Chrome，1728×873，dev-admin）：
  - 拓扑按宽度铺满后，图本身 862px 高，叠上 170px 的指标条，整页 1441px；点节点后详情抢焦点，又把整页往下带了一截。
  - 资源清单 43 行时整页 3433px。详情排在清单下方，点一行要滚到 2560px 才看得到。
- **问答裁定**（一轮）：
  - 资源清单的详情放在右侧，同拓扑。
  - 项目「运行与诊断 → 部署与运行形态」用的是同一个工作区，一起改。
  - 流程：直接改＋回填，推送并部署本机。
  - 提问前写明、作者没有反对的做法：
    - 指标条整条原样搬过去，含采集状态与折叠明细。
    - 宽屏（≥1100px）一屏；图比可用高度高时在图框里滚，字号不变（整张缩进一屏字只剩约 55%）。
    - 窄屏详情排在下面，照常滚动，同调用链的裁定。
- **改法**（全部在 `apps/console`，无契约、无迁移）：
  - **管理总览**：
    - 由 `app/admin/AdminOverview.tsx` 装配（features 之间不互相引用）；`AdminOverviewPage` 加 `status` 插槽，`adminOverviewRoute` 从 features/admin 挪到 `app/router/adminGlobalRoutes.ts`。
    - `ClusterOverviewStrip` 自己读摘要与容量；摘要与集群页不带筛选时共用一个查询键。标题「集群状态」，计数格改为 `Link`。
    - 回执先过 `ClusterSummarySchema`／`ClusterCapacitySchema`，缺字段按读取失败显示。第一版没有这层检查：用例的通用假接口回 `{items: []}`，整个总览页在 `.sources` 上崩掉。
  - **集群管理**：`ClusterPage` 只剩两个页签；摘要读取失败写在页签上方。
  - **高度**：
    - 共享 `shared/lib/useViewportFill`，从调用链 f0a8b052 那份挪来（与 crewstation-bd 约定）；变量 `--trace-fill` 改名 `--viewport-fill`。
    - 多观察主区内容的尺寸：外壳定高后主区自己不变，上方多出一条提示只改变内容的高度。
  - **形态图工作区**：
    - 宽屏是纵向弹性列，高度取上面的变量；图框 `overflow: auto`，占满图例以上的高度；详情与图同高，自己滚。
    - 删掉 sticky、`100dvh - 32px` 和 `:global(main):has(...)`。
    - 详情栏按选中项加 key：换选一项时新详情从顶上看起，不接着上一份的滚动位置。
  - **资源清单**：
    - 结构是 `.inventory` → `.split`（清单｜380px 详情）。清单卡 `listCard` 定高，只有 `.rows` 滚。
    - `DataTable` 加 `stickyHeader`：表格自己不成滚动区，表头贴住外层滚动区的顶边；合并边框用内阴影补下沿线。
    - 筛选项最小宽度 150 → 130px：1280 宽下七项排成一行，表格区 180 → 250px。
- **用例**：
  - **新增 `viewportFill.test.tsx`**：hook 的量法、观察对象与卸载，`stickyHeader`，三处样式链，只剩一份 hook。
  - **改写与新增**：
    - `clusterManagement`：首条改写，新增清单两栏一条。
    - `clusterMetrics`：首条改为总览状态条。
    - `topologyDiagram`：新增两栏一条。
    - `traces`：改变量名。
  - **变异验证**：在干净导出里做了 7 处变异，每处都有用例变红——恢复 `:has`、去掉 hook、详情不进右栏、总览不带状态条、表格回到自滚、只观察 main、清单不定高。
  - **e2e `clusterLayout.test.ts` 重写**：整页溢出按「文档＋main」两种都算，覆盖五项：
    - 1440×900／1280×800 拓扑一屏，详情在旁；
    - 清单表头吸顶、详情在右、换视图不动；
    - 1024 宽照常滚，换视图不跳；
    - 总览状态条在待办之前，格子进对应视图；
    - 项目形态页一屏。
  - **对线上旧包**：5 红 1 绿（1024 那条本来就没变）。
- **本地门禁与核对**（「HEAD＋本批」干净导出）：check:static 通过，console 层 0 fail。
- **实机核对**（CDP 只换 `/assets/*`，新旧两种外壳各一遍）：
  - 1728×873、1440×900 下，拓扑、拓扑＋详情、清单、清单＋详情、项目形态页，文档与 main 都不滚。
  - 1280×720 旧外壳下文档多出 43px，来自管理左栏比窗口高；55 让左栏独立滚动后消失，新外壳实量为 0。
  - 新 clusterLayout 6/6，topology 7 pass／1 skip，clusterMetrics 4/4。
- **回填**：
  - RFC-010 proposal §3、RFC-015 proposal §2.1、RFC-019 proposal §4 各加「2026-09-23 修订二」。
  - RFC-019 design §5 加同日再修订，撤掉 sticky 那条的做法。
  - RFC-003 proposal §4 加总览「集群状态」一条。
- **并行协调**：
  - crewstation-55（所有页面改右侧独立滚动）负责外壳；`Topology.module.css` 那条 `:has` 由我删，e2e `clusterLayout` 由我改成两种外壳都成立的量法。
  - crewstation-bd 同意把 hook 挪到 shared。
- **提交、CI、部署与实机**：见下一次 STATE 更新。

## RFC-025 统一资源管理中心：第一期（基础）上线，第二期进行中（2026-09-23）

作者「批准并实施」（427a2520）后按计划六期推进。第一期（T2–T5）只建底座：台账还没有所属模块写入，调和器只观测、不改集群。

- **代码**：c9499ef6（`modules/resources` 台账 L1、`packages/resource-runtime`、`modules/cluster-control` L2、契约与 api-client、组合根）、107883d7（收编空跑三处误判修正、T2 实测补记）。变更日志的序号在提交时按提交顺序盖章（延迟约束触发器＋咨询锁），推送流按序号尾随不会漏掉晚提交的早序号。
- **门禁与 CI**：干净导出树上 check:static 通过，unit 553、module 1249（12 跳过）、console 828；改动行覆盖 99.0%。CI 35870169467 六项成功；35872803892 第一次 e2e 因 CI 机器的无头 Chrome 没起来一条未跑，重跑六项成功。
- **部署**：14:01:50 迁移 Job 只执行 resources 的两个迁移；cs-controller、cs-api 先后换到 `rc025-p1-20260923`（571ea0e2），14:33 再换到 `rc025-p1b-20260923`（107883d7），每次 0 次重启；console 未动。
- **实机**：视图与推送流的授权、拒绝与游标分支逐项核对（acceptance.md §1.3）；推送流经网关快照 3 毫秒到达、心跳 15 秒准时。收编空跑修正后 `adoptable 26、orphan 5、retained 4、platform 7、unclassified 0`。
- **一次险情**：第一次空跑把 cs-demo 正在运行的开发会话的工作卷判成了孤儿——旧 PVC 的任务标签是 RFC-013 之前的 `tsk_…`，没先经身份目录解析。已修；第六期正式收编与孤儿回收沿用「先解析旧 ID」这条规则。
- **T2 实测**：Traefik 限流 429 带 `Retry-After`，`inFlightReq` 不带；计数按路由各一份；按请求头分桶时缺头的请求共用一个空键桶（限流只能挂在 ForwardAuth 之后）；SSE 经网关不缓冲、静默 200 秒不断（设计 §7.3、§8）。
- **待作者裁定**：I24（平台没有成员变化的领域事件，推送流沿用会话流「发事件前复核授权」的先例）。
- **下一步**：第二期第一步——task-runtime 在环境落库的同一事务里把期望与领域条件投影进台账（保存点隔离，投影失败只告警），定期补投影；cluster-control 尾随台账变更、按记录从观测缓存补观测。随后 T7 工作台改读「快照＋推送」。
- **注意**：本机不要用 `bun run test:cover` 预检改动行覆盖，它会连共用集群跑 e2e（dev-gotchas）；用三层各自 `--cover` 再 `test:patch --tiers unit,module,console`。

## 开发页释放入口：页头「释放会话」，会话面板「当前会话」置顶（2026-09-23）

作者先问「创建第一个开发 CLI 前总会先建一个 task 容器、CLI 全关了它还在，它是做什么的」。答复：那是开发会话的父 Pod，保管工作卷、常驻 TaskRunner，编辑器、终端、预览、发布前检查都靠它；按 G19／D50，空闲只提醒，只有手动释放才回收。接着问「在哪释放」，看到入口后说：「太隐蔽了吧，并且那个按钮为什么在中间而不是旁边」「把当前会话卡放在最上面，把日志卡放在最下面」。

问答裁定：页头加「释放会话」；卡片里的按钮放底部操作条靠左（沿用 09-23 按钮统一）；流程「直接改＋回填，提交推送并部署本机」。

- **为什么在中间**：按钮放在 `Card` 的 `footer`（灰色说明区）里，外面一层限宽 420px、`align-items: flex-end`，面板宽时按钮停在 420px 处。
- **改了什么**：
  - 页头操作区最后加红字红框的「释放会话」，用紧凑档，和同一排按钮一致。只对会话创建者与负责人、且会话没有失败时显示。点了打开会话面板，并在「当前会话」卡里直接展开确认，确认面板取得焦点；工作区检查、不干净时输入 `discard` 的弹窗都照旧。确认已展开时再点，会重新检查。
  - 「会话与环境」面板的顺序：当前会话 → 连接与恢复 → 历史对话会话 → 最近日志。最近日志是最后一项，由它长到面板底边。
  - `SessionCard`：释放按钮进 `actions`（底部操作条，靠左）。`footer` 只留一句「结束开发并释放环境：确认后销毁容器与工作卷，未提交或未推送的代码会丢失。」，不能释放的人连这句也不显示。
...
- **行为**（`features/dev-session/components/session/SessionCover.tsx`）：
  - 连接第一次就绪之前，开发页内容区只有一块居中的加载层，页头、CLI 区、工具面板与底部状态条都不渲染。加载层有三种：
    - 进页四步清单：读取会话 → 连接页面通道 → 等待开发环境响应 → 恢复个人布局与 CLI 标签。已完成的打勾带用时，当前一步带计时（`model/connection/entryProgress.ts`；时刻来自 `TaskStreamSocket` 新记的 `openedAt`／`runnerAt`）。页面通道失败过或等满 10 秒给「重新连接页面」；当前一步等满 1 分钟给「仍然打开工作区」（第一步不给）。
    - 开始开发或重建：RFC-022 的五段步骤条从 CLI 区中间移到整页中间；启动失败时保卷恢复卡紧跟在下面，等待连接失败不再单给一个打开会话面板的「重试」。
    - 进来时就「开发环境已停止」「开发环境需要更新」：整页状态卡，原因与任务号 `role=alert`，恢复卡在下面；「正在释放」也是状态卡。恢复卡始终是加载层的第二个子节点，三种形态来回切换时不重挂，已提交的恢复请求与回执都还在。
  - 个人布局、CLI 名册与算力档位在加载层后面照常先读，连接就绪且三者读完才一次揭开（`NativeWorkspace` 的 `gate`）。窄屏判定在揭开前量的是加载层（`useToolPanel` 的 `rootKey`），布局恢复不会在揭开后再跳一次；从动态通知跳来的定位等揭开后再聚焦。
  - 揭开之后同一次进页不再盖回：断线、失败与在页面里发起的重建照旧是工作区加提示，草稿与 CLI 画面不动。
  - 代价（作者选项里写明）：进来时就失败的会话，「数据访问」「可使用资源」等面板要等环境恢复后才看得到。
  - 实机验收脚本的 `settle` 多等 `main [data-page-loading]` 撤掉。
- **回填**：RFC-003 development-workspace §2、RFC-022 proposal D3 与 design §8.3、RFC-008 design §2，各加 2026-09-23 修订；dev-gotchas 补 CDP 拦截的 `urlPattern` 会命中页面文档一条。
- **用例**：新增 `sessionLoading` 7 条（揭开时机、读会话、通道失败与重连、环境不响应与 1 分钟出口、揭开后不盖回、揭开前窄屏判定、出口阈值）、`entryProgress` 2 条、`taskStreamReplay` 时刻 1 条；改写 `sessionStartup` 2 条、`rebuildSession` 5 条、`failedDevSession` 1 条（「首屏写明任务号与原因」的约束保留）。改写与新增的用例在旧代码上 9 条红。
- **提交前的实机核对**（不动共享部署：从「HEAD＋本批」导出树构建，无头 Chrome 只换 `/assets/*`，dev-developer，演示项目）：
