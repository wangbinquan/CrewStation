...
- **待作者裁定**：I24（推送流的复核授权）、I25（调和器建带凭据的 Pod 与 Secret 时凭据放在哪）、I26（说明页的错误体与 RFC-021 不一致；去掉 `allowEmptyServices` 会让暂时没有端点的窗口变成裸 404）、I27（项目归档后命名空间记录与命名空间怎样收尾；本期归档不释放、不删）、I28（数据面的调和器建角色时口令与连接串放在哪；T12 先做投影、观测与删除）。
