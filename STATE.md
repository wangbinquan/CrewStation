...
接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面眼下是 `cs-control-plane:rc025-t14b-20260924`（151b0267，cs-controller 1 个副本），工作台由 crewstation-87 换到 `cs-console:admin-dw-only-20260924`（2aaf6e46），平台路由清单已按 de9dcb5b 应用。
...
- **第五期（T13，集群管理）**：903167c7——删除待回收的工作卷由资源中心自己受理（只给管理员、只对待回收的卷，期望改为「不要了」，调和器删 PVC；权限不足一律 403）。本机 3 个孤儿工作卷在「待回收」，删不删由作者定；集群管理的界面、清单来源与台账维护对象的运维语义等 I29。
...
- **待作者裁定**：I24（推送流的复核授权）、I25（调和器建带凭据的 Pod 与 Secret 时凭据放在哪）、I26（说明页的错误体与 RFC-021 不一致；去掉 `allowEmptyServices` 会让暂时没有端点的窗口变成裸 404）、I27（项目归档后命名空间记录与命名空间怎样收尾；本期归档不释放、不删）、I28（数据面的调和器建角色时口令与连接串放在哪；T12 先做投影、观测与删除）、I29（集群管理改读台账：清单来源、台账维护的对象还给不给「删除」、「待回收的工作卷」放哪；后端的删除待回收工作卷先做）。
