...
接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-i25b-20260924`（0cfa6cba，cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:cluster-actions-2-20260924`（d93c06ba），平台路由清单已按 de9dcb5b 应用。
...
- **第二期（T6、T7，任务类容器）**：
  ...
  - 还没做：失败后的重建与档位测试的创建移交；检出用的 Git 凭据 Secret 改为渲染时向 scm 要。
...
- **第六期（T14，收编）**：ed208846——服务槽的 Service 与内部 API 的前缀剥离中间件挂到已有记录下（槽下线时 Service 照旧保留，不算结束中）；孤儿回收扩到没人认领也没人引用的中间件，实机删掉两个代理改名后留下的旧中间件。151b0267——tsk_… 旧身份写成记录的别名（6 条）。还没被认领的只剩服务的 Git 凭据 Secret（随 I25）。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前两步已做（a30d1846、0cfa6cba），I28 待实施。
