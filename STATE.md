...
## 并行接力：RFC-010 集群资源管理方案（2026-09-20）

作者要求看到 Kubernetes 工作负载、Pod 等资源的数量、运行状态、所属项目与 Pod 类型，并展示系统内置工作负载。
本轮明确选择：**首版包括重启、调整副本与删除；只覆盖 CrewStation 项目和平台内置服务**。
已完成 `proposal/rfc/RFC-010-cluster-management/` 三件套、源码与本机只读盘点、ADR-0006（提议）和本会话交互预览；RFC 为 Draft，待整体批准。

关键缺口：旧系统组件标签不完整；开发工作区与 CLI 共用 workload 标签；需要 owner UID＋任务用途批量解析；
现有普通 Pod 删除会把任务判失败，正常环境重启需领域扩展；发布会重新应用 Manifest 副本，运维覆盖需明确定义。
建议按项目／平台系统分别归属，Pod 用途独立于 Kubernetes 控制器类型；运行时副本覆盖保留至手动恢复，核心组件只开放受支持重启／扩缩。
副本持续性与核心删除限制已作为选项询问作者，尚未收到裁定；不能把方案文字当实施批准。

预览已验证筛选、系统内置资源、用途／故障原因、非法副本校验、模拟重启／扩缩／删除与操作记录；
1024px、390px 及 320px 六视图无整页横向溢出，浏览器未记录错误。预览为演示数据，不连接真实集群。
生产代码、集群写操作与数据迁移均未执行；完整门禁和 CM-01…CM-25 真实验收留到批准实现后。
调研期间并行 RFC-008／009 完成各自发布，门禁体系仍有共享暂存内容；本会话未运行 staging／commit／push，也未改动它们的内容。
