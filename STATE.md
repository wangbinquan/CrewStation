...
- **第二期（T6、T7，任务类容器）**：
  ...
  - 还没做：建与重建移交调和器（等 I25）。
- **第三期（T8，服务槽与构建）**：4aeaa892、6d54a25f、7c3e68d0、25bf918e（槽记录、副本认领、崩溃重启、健康照槽记录）；2b61586f（保留计时 `RetentionDeadline`，槽卡随推送流重读）；b2e19fdc、c2be2128（统一预检接入重新部署、流水线部署、切流与发布受理）；3c5315e0（构建与迁移 Job 进台账）；26033e7c（概览摘要随推送流重读）。槽与 Job 的建、改仍由 release 执行（等 I25）。
- **第三期后半（T9，路由）**：
