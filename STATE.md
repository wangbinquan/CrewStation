...
- **第四期（T12，数据资源）**：
  ...
  - 还没做：访问绑定的临时角色经 data-control 建、口令轮换；本机没有进行中的访问绑定，绑定的投影与删角色只有用例；数据面上的两组孤儿库与角色（`cs_cluster_business`、`cs_cluster_business_dev`）留给 T14。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前四步已做（a30d1846、0cfa6cba、6185e4b4、66c76d8b，只剩失败会话重建），I28 第一步已做（0b0c43db，只剩临时角色与轮换）。
