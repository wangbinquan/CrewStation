...
## RFC-025 统一资源管理中心：第二期完成（工作区与执行环境的创建已移交，重建与档位测试待移交）、第三期服务槽、路由与限流进行中、第四期命名空间与网络策略上线、数据资源进行中（2026-09-24）

接 09-23「第一期（基础）上线」一节。逐步的门禁、CI、部署与实机记录见 `proposal/rfc/RFC-025-resource-center/acceptance.md` §3–§11。本机控制面（cs-api、cs-auth、cs-controller、cs-session）眼下是 `cs-control-plane:rc025-i25d-20260924`（6185e4b4，cs-controller 1 个副本；cs-events 与两个 MCP 仍是 `iface-20260923`），工作台是 `cs-console:cluster-actions-2-20260924`（d93c06ba），平台路由清单已按 de9dcb5b 应用。
...
- **作者已裁定（09-24）**：I25＋I28 渲染时向所属模块要凭据（台账与库备份里没有凭据；data-control 生成口令、加密存自己的表）；I26 沿用 RFC-021 的错误体 `not-deployed`＋`details`，说明页由 cs-api 按台账渲染、ForwardAuth 的未部署分支退役、`allowEmptyServices` 保留兜底；I29 三个 (a)（已做，a89589ab）。I26 已做（b35ea86d）；I25 前三步已做（a30d1846、0cfa6cba、6185e4b4），I28 待实施。
