...
共享 project 投影、platform 组合根、app 导航/路由和能力 E2E 已完整交接 RFC-011 提交；RFC-012 的 11 项交接输出均随本提交完整保留。未单独 push。
...
已部署版本在 1280/390/320px、中英文、浅深色无整页横向溢出或浏览器异常；最新零值/计数/焦点修正尚需新镜像对齐。
RFC-011 正在运行唯一共享 `check + coverage`，本会话不重复启动。

本机 `docker-desktop` 上 `crewstation-control` 尚缺新增读取/patch/Ingress 删除权限；两次自动审批拒绝 apply。
已提出具体权限范围的授权问题，尚未获答；不重试、不绕过。本机读取为部分清单（一次快照 274 项、26 个控制器、33 个 Pod，84 个403来源），不能宣称全量清单或真实运维动作已经验收。
后续：完整门禁/构建 → 精确发布与目标 SHA CI → 得到具体 RBAC 授权后应用并在专用项目验收真实动作。详情和 CM 逐项状态见 `proposal/rfc/RFC-010-cluster-management/acceptance.md`。RFC 保持 **In Progress**。
