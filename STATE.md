...
作者要求依据原始理念全面重设计第一版原型的功能组织、UX 与使用逻辑。已对照两篇理念文章、基线与 RFC-001／002，检查本机界面与源码，完成 `proposal/rfc/RFC-003-workbench-ux-redesign/`：三件套、16 项可追溯审查、交互附件与其验证记录。

方案：六个项目入口（概览、开发、发布与上线、运行与诊断、能力接入、项目设置）；开发默认 Agent 与预览同屏；统一两种源码来源的发布、试用、上线、回退；平台新建与接入职责收回管理空间。保留现有能力，列明工作区预检、成员定位、真实 API 试调、有界项目摘要等接口缺口。

**发现的实际断点**：开发页让用户去发布页切流，但真正按钮在概览；接入容器在管理空间展示却要回租户列表创建；Swagger 只看文档、试调需用户去终端；未推送清单在释放响应后才呈现。具体源码行号与实跑范围见 RFC 的 `audit.md`。

交互附件已在浏览器验证消息、表单错误、两空间申请反馈、版本切换与回退、日志上下文、草稿保留及 1024／390／320 宽度。数据与执行全部为演示，**生产代码尚未修改，不代表真实接口验收通过**。按开发规则 §5.3，下一步先取得 RFC 具体方案批准，再执行 `plan.md` 的 T3–T12；不要把作者对浏览器操作的授权当成对尚未评审 RFC 的批准。
