...
## RFC-014 用户与权限、认证 UX 实现中（2026-09-21）

作者已明确授权完整实现、验证与提交上库。正式页面已实现紧凑用户目录／单人角色编辑、409 基准重读与重新确认、认证标签路由、四组接入配置及逐字段校验、独立连接诊断、身份字段与项目覆盖选择。复用公共布局，新增原生选择行和复制控件；保留权限边界、原始错误、空密钥和空字段集合语义。

最终定向回归 44 pass／0 fail、206 assertions，最新 console 类型检查与 production build 通过；最终英文 320px 用户目录／角色面板及认证页五档宽度×明暗主题复查通过。此前真实提供方测试 POST 200（10–12ms）与原值 PATCH 200（11–17ms），没有改角色、密钥、策略或转发规则；真实与模拟证据边界见 `proposal/rfc/RFC-014-identity-admin-ux/acceptance.md`。

RFC-013 已修复共享阻断。联合候选完整门禁为 1937 pass／5 skip／0 fail、12165 assertions；379 个生产文件的变更行覆盖 2887／2965（97.4%），没有违规。609 个 RFC-013 文件哈希与通过门禁的快照一致，RFC-014 源码没有在门禁后变化。受限沙箱曾造成监听与数据库失败的无效轮次已单独留档，不冒记绿色。

作者已明确「授权两个 RFC 一并上库」，本任务接手联合 650 个路径，保留共享文件中的全部并行输出。提交前 main 与 origin/main 同为 `f94f0f3`、index 为空；正在完成精确提交、推送及 hosted CI。本次不部署本机，RFC-013 的实际升级与运行态验收尚未完成。
