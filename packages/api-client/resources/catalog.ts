import type { ComputeProfileDto, ComputeProfileSummaryDto, ProjectTemplateDto, ServicePlanDto, TaskProfileDto } from '@crewstation/contracts';
...
export interface CatalogResource {
  ...
  /** GET /v1/catalog/compute-profiles?full=true：含驱动与模型，仅管理员；非管理员拿到的仍是租户投影。 */
  listComputeProfilesFull(): Promise<ItemsPage<ComputeProfileDto>>;
  /** PUT /v1/catalog/compute-profiles（按 name 新增或覆盖） */
...
export function catalogResource(transport: Transport): CatalogResource {
  return {
    ...
    listComputeProfilesFull: () => transport.request<ItemsPage<ComputeProfileDto>>('GET', '/v1/catalog/compute-profiles', { query: { full: 'true' } }),
