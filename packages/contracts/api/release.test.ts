...
test('槽的计时与下线字段可选：旧响应照常解析，新字段按形状校验；发布状态多了已下线', async () => {
  ...
  const retention = { kind: 'pending', since: '2026-09-23T01:00:00.000Z', deadline: '2026-10-07T01:00:00.000Z', postponable: false, postponements: 0, periodHours: 336 };
  ...
  // 能不能推迟由服务端算好（2026-09-23 裁定），计时里必须带上。
  const { postponable: _postponable, ...withoutPostponable } = retention;
  expect(SlotDtoSchema.safeParse({ ...base, retention: withoutPostponable }).success).toBe(false);
