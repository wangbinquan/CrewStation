 ...
 * orphan：所属对象已不在（或已结束），收编时按孤儿规则回收（PVC 只进待回收）；retained：失败保留期内；unclassified：认不出归属。
 */
export const AdoptionVerdictSchema = z.enum(['owned', 'adoptable', 'orphan', 'retained', 'unclassified']);
