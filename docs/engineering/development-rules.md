...
- **别人的在制品会把本地全量门禁刷成红的**，而 CI 检查的是**提交树**、看不到那些未提交改动——这时「本地红」既不说明你有问题，也不保证你没问题。按本次改动的文件单独跑一遍才作数：`bunx eslint <你改的目录／文件…>`。2026-09-21 实撞：RFC-016 删掉一个函数后只重跑了 typecheck，本机全量 lint 的输出被并行会话的四条报错淹没，那条「`PreviewState` 未使用」就这么推了上去，`static` 当场红。
...
- **宽屏只有内容区滚动**（2026-09-23 作者裁定）：外壳在 ≥801px 时正好一屏高，顶栏与左栏不动，`main` 是唯一的纵向滚动区；≤800px 仍是整页滚动。页面样式不得改写 `main` 的 `overflow`（`apps/console/src/tests/shellScroll.test.tsx` 阻断），也不要按 `window.scrollY` 排版；吸顶要贴住内容区顶边用 `top: var(--cs-main-sticky-top)`；页签内的选中与切换不该回顶时导航传 `resetScroll: false`。页面打开与换页后外壳把焦点放到内容区（只改地址参数时不动），页面要聚焦自己的控件就在渲染时聚焦内容区里的元素，外壳不会抢。e2e 量「整页多出的高度」取文档溢出加 `main` 溢出，量滚动位置取 `scrollY + main.scrollTop`，新旧两种外壳下都成立。
- **表单与确认用弹窗，不在页内展开**（2026-09-23 作者裁定）：点了才出现的表单用 `shared/ui/dialog/FormDialog`；带核对材料或输入的确认，以及放弃／切换／离开类提示（含 `UnsavedChangesGuard`）用 `ConfirmationDialog`。它们都搭在 `Dialog` 底座上：页面内的模态 `<dialog>`，经应用根上的 `DialogHost` 渲染（不嵌在打开它的表单或隐藏面板里），标题栏右上角 ✕、Esc 关闭、点遮罩不关，打开时焦点进弹窗、关闭后回到打开它的控件，进行中锁住关闭。**关窗不丢草稿**：草稿放在打开者那一层（弹窗只是视图），再打开恢复；底部最右的「清空」回到初始值、弹窗不关；只有离开页面才丢，离开确认写明是哪几份草稿。仍留在行内的只有两类：一行式的是／否确认（`InlineConfirm`），以及工作区干净时释放开发会话的确认（`ConfirmationPanel`）。列表里逐项编辑的弹窗用 `shared/lib/useDraftTarget`：表单组件按 `sequence` 常驻挂载、弹窗只在 `open` 时画，再点同一项恢复（没改过就按这次的最新值重起），有未保存输入时换对象先确认。页面把一片内容 `hidden` 但仍挂载时（身份守卫、页签后面的另一组），用 `DialogVisibility` 包住，里面的弹窗随之不画；只有离开确认标 `persistent` 照样显示。弹窗里复位区域按钮档位（`ButtonSizeContext` 会穿过 portal），操作条一律标准档。用例里有弹窗开着时只点得到最上层弹窗（`tests/interactiveScope.ts`，`renderApp` 与 `renderElement` 的点击都已如此）。
