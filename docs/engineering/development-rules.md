...
- **宽屏只有内容区滚动**（2026-09-23 作者裁定）：外壳在 ≥801px 时正好一屏高，顶栏与左栏不动，`main` 是唯一的纵向滚动区；≤800px 仍是整页滚动。页面样式不得改写 `main` 的 `overflow`（`apps/console/src/tests/shellScroll.test.tsx` 阻断），也不要按 `window.scrollY` 排版；吸顶要贴住内容区顶边用 `top: var(--cs-main-sticky-top)`；页签内的选中与切换不该回顶时导航传 `resetScroll: false`。e2e 量「整页多出的高度」取文档溢出加 `main` 溢出，量滚动位置取 `scrollY + main.scrollTop`，新旧两种外壳下都成立。
