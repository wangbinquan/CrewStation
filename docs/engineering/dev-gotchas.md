...
### 「与上一版相同就不写」要按库里读回来的样子比：顺序不同、字段没存下都会每次判成变化

2026-09-24 实撞（RFC-025 资源台账）：`commitRecord` 用 `jsonHash` 比较新旧两版，相同就不写。两处让它每次都判成不同，进而自己跟自己转圈：

- 子对象从库里按（种类，名字）读回，合并却按期望里的顺序排，数组一比就不同。开发工作区记录有了四个子对象之后（09-23 16:22），调和器每核对一次就整组重写、追加一行变更，这行变更又触发下一次核对——每条记录每秒空写 4–8 次，台账变更一小时约 11 万行，还经推送流扇出到开着的工作台。
- 观测里新加的字段（`generation`）没写进 `observed` 列：读回来少一个字段，下一次同样的观测又算「变了」。

判据：`resources.changes` 按分钟计数远高于对象实际的变化频率；同一条记录隔一秒取两次快照，内容完全相同而 `version` 在涨。
做法：比较时不看数组先后（按键排好再比）；新加的字段先确认存得下、读得回；「写进去、读回来、再同样写一次应当不写」要有用例（`modules/resources/tests/ledgerWrites.test.ts`）。
...
### 「刚做过」不能只记在组件内存里：切页就丢，而全局缓存 30 秒内切回读的是旧名册

2026-09-23 实撞：关掉在运行的 CLI 后切出开发页再切回，它以「不可用」回到原处，还能再结束一次。三处叠加：

- 「本页刚关掉」只放在 `NativeWorkspace` 的 `useRef` 里，组件卸载就没了；
- 结束是异步的：`POST …/stop` 204 之后约 1–2 秒进程才报 ended（实机 12:08:47.686Z 布局落库、12:08:48.495Z 才 ended）；
- `apps/console/src/shared/api/queryClient.ts` 的全局 `staleTime` 是 30 秒，重新挂载时名册直接用缓存（仍是「运行中」），对账据此把它放回。

判据与做法：用户动作的结论要么持久化（这里是个人布局的 `hiddenTerminalIds`，8e5b022d），要么由服务端给出可判定的状态（RFC-025 的 `stopping`）；不要让「页面里记着」参与状态推导。实机复现时用脚本「结束 → 标签消失后立刻切页 → 0.4 秒后切回」，比手点稳定。

### 串行的写队列要给单个请求设上限：一个挂住的请求会挡住之后的全部保存
...
### 两条节奏都能发现变化时，用例只能断言结果，不能断言是哪一条发现的

2026-09-24 RFC-025 的数据面观测（`data-control`）有两条节奏：尾随台账变更（每秒）与定期全量（每 30 秒）；每次核对都取一份新快照。
第一版模块用例假设「库被人删了只有全量那一轮发现得了」，于是等全量那一轮「写了观测就记一行」的日志——5 次里 1 次卡满 5 秒超时：
前一步刚写下的「运行中」本身是一条台账变更，尾随那一拍正好在用例删库之后取快照，先把「消失」写了，全量那一轮没有新写入，也就不记日志。
这种「谁先到都对」的机制，模块用例只看台账里的结果（阶段、子对象的观测）；要核对某一条节奏自己的行为（记不记日志、一批取几份快照），
在工作器层用假台账、把另一条节奏的间隔拉到极长单独测（`data-control/workers/dataPlaneObserver.test.ts`）。
这类偶发要用重跑抓：`for i in $(seq 1 12); do bun test <文件>; done`，别只跑一次绿了就提交。

### 「在期限内反复观察」的循环要先观察、再看期限

2026-09-24 CI 上 `cluster-management` 的「执行、观察、日志」用例偶发得到 `needs-attention`（期望 `succeeded`）：用例把观察期限设成 100 毫秒、用真实时钟，
实现是 `while (已过时间 < 期限) { 观察… }`——负载高时，从记下「开始观察」到进入循环之间就过了 100 毫秒，于是一次都没观察就判「观察期限内尚未收敛」。
本机连跑都绿，只在 CI 的整层里出现。改成「先观察一次，没收敛再看期限」，并加了一条期限为 0 的用例（旧实现必失败）。
同类写法（等待、轮询、租约续约）都照这个顺序：至少做一次，再判超时。
...
### 推送前看一眼 `origin/main..HEAD`：本地 main 上可能有别人刚提交、还没推的提交

2026-09-23 实撞（437bffe）：用私有索引提交时，父提交是本地 HEAD；另一个会话刚在本地提交了 cf5f325、还没推送，`git push` 把它连同自己的一起推了上去。推送之前 `git log --oneline origin/main..HEAD` 应当只列出自己的提交；有别人的，先问对方是现在推还是等。
