...
- [I31. RFC-025 数据口令轮换时已运行使用者怎样换用新口令](#i31-rfc-025-数据口令轮换时已运行使用者怎样换用新口令)

- [I32. RFC-025 同 Host 唯一与内部 API 前缀分流](#i32-rfc-025-同-host-唯一与内部-api-前缀分流)
- [I33. RFC-025 未登录请求按 IP 限流的初值](#i33-rfc-025-未登录请求按-ip-限流的初值)
...
**作者已裁定（2026-09-27）**：选 (a)——归档后保留命名空间、额度与网络策略，调和器继续维护；后续由管理员在工作卷和其他资源全部处理完后删除命名空间。不得因归档自动删卷或删除诊断现场。
...
## I31. RFC-025 数据口令轮换时已运行使用者怎样换用新口令

**现状（2026-09-27 接手核对）**：RFC-025 design §6.6 与 T12 要求轮换，但 data 公开 API（`modules/data/api/moduleApi.ts`）没有轮换入口；data-control 只提供创建时保存的口令（`application/provisionDatabase.ts` 的 `credentialOf`），凭据仓储只有 get 与 putIfAbsent。I28 解决的是生成和保存归属，没有规定轮换的受理权限、旧容器如何得到新值、失败时是否保留旧凭据。服务槽和任务在启动时注入连接串，数据库改口令不会自动刷新它们的环境。

**待作者裁定**：已异步询问。(a) 管理员手动轮换，只在没有运行中的使用者时允许，下一次启动注入新值（建议）；(b) 管理员手动轮换并自动重建全部受影响的槽和任务（会中断运行）；(c) 本 RFC 暂不提供轮换入口，明确登记后续。裁定前不增加自动轮换或自动重启行为。

## I32. RFC-025 同 Host 唯一与内部 API 前缀分流

**现状（2026-09-27）**：RFC-025 design §7.1 要求「一个 Host 一条生效路由」，按上级阶段选一条、其余摘除。但 `modules/gateway/domain/routePlan.ts:45` 为每个 API 代理声明同一个 `platformApiHost` 配不同 `/api/<proxy>` 前缀；本机也实查到 `demo-internal-api` 使用 `Host(api.svc.cs.internal) && PathPrefix(/api/demo)`。这些入口按路径分流，不是同一个入口的重复实例。

**为什么是问题**：机械按 Host 选一条会关闭其他代理的 API 入口；合并成一条 IngressRoute 又会改变当前每条记录拥有一个子对象的归属设计。不能为完成「唯一」而静默收缩已有能力。

**待作者裁定**：已异步询问。(a) 唯一键为规范化的 Host＋路径前缀；同一前缀的重复候选才按上级阶段取舍，无前缀开发预览仍按 Host 唯一（建议）。(b) 维持一个 Host 一个 IngressRoute，将多个路径规则合并到一个 Host 级记录中，需要补跨项目路由归属与清理设计。裁定前不摘除合法的不同前缀入口。

## I33. RFC-025 未登录请求按 IP 限流的初值

**现状（2026-09-27）**：design §7.3 要求未登录请求按 IP 限流；Q4 只给已认证三类流量的默认值，`/auth` 目前没有该中间件。T15 已完成现有三类单身份突发实测（acceptance §13），未把这些实测当成登录入口容量证明。

**待作者裁定**：已异步询问。(a) 首版对 `/auth` 按客户端 IP 平均 20/s、突发 40，健康检查不纳入（建议）；(b) 本 RFC 明确延后未登录限流；(c) 作者提供其他取值。多级反向代理取客户端 IP 必须沿用受信入口配置，不能接受任意客户端自报的身份头。Q4 的已认证现值另按提案要求报作者最终确认，裁定前保留当前配置。
