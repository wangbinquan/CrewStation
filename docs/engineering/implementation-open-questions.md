...
**裁定后的实施（2026-09-24）**：第一步是生产库、开发库。`data-control` 新增 `data_control.credentials`（按记录 ID 存角色名与口令密文）；`data` 开通时只写期望（记录标明由 data-control 建），等记录就绪；`data-control` 发现库或角色不在，就取已存口令（没有才生成、先存再建）建运行角色与库；`data` 渲染连接串时经端口 `credentialOf` 要口令、自己拼（0b0c43db，已部署；RFC-025 设计 §6.6 补记、验收 §8）。已存着连接串的旧库照旧（本机 28 个都是）。运维开关 `CS_DATA_PROVISIONING=data` 回退。后续：访问绑定的临时角色（诊断只读、生产变更）经 data-control 建；口令轮换。
