// cs-session 进程入口：读取配置、连接数据库与集群、装配组合根、挑选本进程的入口。业务逻辑一律在 modules/*。
...
const server = serve(app, { port: portFrom(process.env, name, 8083), websocket: platform.api.websocket as never });
logger.info('listening', { port: server.port, role: 'session', routers: (platform.api.routers.session).length, background: background.length });
