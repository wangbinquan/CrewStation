...
test('可见范围与展示资料各一个弹窗：关窗草稿都留着，离开共用一次确认；保存一张不会清除另一份草稿', async () => {
  ...
  await editSetting(page, '可见范围', 'authenticated'); await editSetting(page, '应用用途', '尚未保存的用途'); await closeSetting(page);
  expect(document.querySelectorAll('dialog').length).toBe(0);
  await page.click('高级'); expect(page.search().tab).toBe('visibility'); expect(page.text()).toContain('应用展示资料、可见范围有未保存的输入');
  ...
  await openSetting(page, '修改可见范围'); expect(settingsField('可见范围').value).toBe('authenticated');
...
test('身份或任一设置读取失败都会暂停写入，两份草稿都留着、打开弹窗可查看；恢复读取不自动保存', async () => {
  const f = visibilitySettingsFixture(); page = await renderApp(visibilitySettingsRoute);
  await editSetting(page, '可见范围', 'authenticated'); await editSetting(page, '应用用途', '不能丢失的用途'); await closeSetting(page);
  for (const failure of ['app-visibility', 'app-presentation', 'me'] as const) {
    ...
    if (failure === 'me') expect(settingsButton('修改展示资料')).toBeUndefined();
      ...
      await openSetting(page, '修改可见范围'); expect(settingsField('可见范围').value).toBe('authenticated'); expect(settingsButton('保存可见范围').disabled).toBe(true);
      await act(async () => { settingsForm('可见范围').dispatchEvent(new Event('submit', { bubbles: true, cancelable: true })); }); await page.settle();
...
test('取消只收起所属弹窗、草稿留着；清空只把这一份回到已保存的值，不碰另一份草稿', async () => {
  const f = visibilitySettingsFixture(); page = await renderApp(visibilitySettingsRoute);
  await editSetting(page, '可见范围', 'authenticated'); await editSetting(page, '应用用途', '独立草稿');
  await clickSetting(page, '取消'); expect(document.querySelectorAll('dialog').length).toBe(0); expect(f.writes).toHaveLength(0);
  await openSetting(page, '修改可见范围'); expect(settingsField('可见范围').value).toBe('authenticated');
  await clickSetting(page, '清空'); expect(settingsField('可见范围').value).toBe('members'); expect(document.querySelectorAll('dialog[open]').length).toBe(1); await closeSetting(page);
...
test('负责人身份变化后草稿仍可打开检查、保存暂停，恢复后继续；初始只读用户不出现编辑入口', async () => {
  const f = visibilitySettingsFixture(); page = await renderApp(visibilitySettingsRoute);
  await editSetting(page, '应用用途', '角色变化前的草稿'); await editSetting(page, '可见范围', 'authenticated'); await closeSetting(page);
...
test('返回浏览器历史前可保留草稿，确认离开后才导航；范围与申请开关在同一份草稿里', async () => {
  ...
  await editSetting(page, '可见范围', 'authenticated'); await editSetting(page, '申请', 'closed');
  await page.back(); expect(page.path()).toContain('/settings'); await clickSetting(page, '继续编辑');
  expect(settingsField('可见范围').value).toBe('authenticated'); expect(settingsField('申请').value).toBe('closed');
...
test('应用展示是展示资料、可见范围与使用申请三张卡：「检查保存后的效果」已删除，页面上没有查找框，也不请求检查接口', async () => {
  ...
  // 2026-09-23 作者裁定删除这张卡与后端检查接口（RFC-009 proposal §3.4 同日再修订）；2026-09-24 负责人多了「使用申请」卡。
  expect([...document.querySelectorAll('main section > header > h2')].map((node) => node.textContent)).toEqual(['应用展示资料', '可见范围', '使用申请']);
  expect(paths).toContain('/v1/app-access-requests');
...
test('应用设置首屏只显示摘要，两个编辑入口各开一个弹窗，焦点进第一个输入，取消后回到入口', async () => {
  ...
  expect(document.activeElement === settingsField('可见范围')).toBe(true);
});
