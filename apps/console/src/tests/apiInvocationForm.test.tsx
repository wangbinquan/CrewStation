...
test('详情表单展示约束和全部字段错误，真实请求按固定会话发送并呈现 HTTP 错误与截断', async () => {
  ...
  await invocationClick(page, '发送请求');
  expect(f.calls).toHaveLength(1); expect(f.calls[0]!.input).toEqual({ expectedTaskId: invocationTaskId, operationKey: invocationOperation.key, pathParameters: { id: 'a / b' }, query: { tag: ['one', 'two'] }, headers: { 'content-type': 'application/json' }, body: '{"title":"文档"}' });
...
test('失败、收起、取消切操作均保留输入；确认切换只丢弃当前表单', async () => {
  ...
  const next = () => [...document.querySelectorAll('tr')].find((row) => row.textContent?.includes('crm:GET:/ping'))!;
...
test('会话替换后不会把旧输入自动发到新会话，失败重读保留，再明确绑定才允许发送', async () => {
  ...
  f.state.taskId = TaskIdSchema.parse(`tsk_${'e'.repeat(32)}`); await refreshInvocationQueries(page);
...
test('读取失败、失去授权与错会话响应不会显示成功或清空输入', async () => {
  ...
  f.pending.handle = async (input) => Response.json(invocationResponse(input, { taskId: `tsk_${'e'.repeat(32)}` })); await invocationClick(page, '发送请求');
