...
test('概览的下一步横幅只从有效的槽与发布事实推导：待验证版本不同于正式版本才提示等待验证并直达该发布', async () => {
  ...
  f.item.slots.value[1] = { ...slot('preview', 'v1.0.0', prodRelease, 'trial.test') }; await page.reread();
  expect(page.text()).not.toContain('等待验证');
  // 尚无正式版本但待验证就绪：提示首次上线，而不是把待命槽当作已上线。
  f.item.slots.value = [{ name: 'prod', active: true, state: 'empty', replicas: 0, readyReplicas: 0, host: 'formal.test' }, slot('preview', 'v1.0.1', previewRelease, 'trial.test')]; await page.reread();
...
test('最新发布失败或进行中时横幅优先说明该发布；测试者不显示下一步', async () => {
  ...
  f.item.releases.value = [release('building')]; await page.reread();
  expect(page.text()).toContain('v1.0.2 正在发布');
  f.item.role = 'tester'; f.admin = false; await page.reread();
