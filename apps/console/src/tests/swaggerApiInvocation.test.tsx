...
test('实际 Swagger 的 Try it out 与 Execute 走固定会话 HTTP，原服务没有浏览器请求', async () => {
  const f = apiInvocationFixture(); page = await renderApp(`${invocationRoute}&proxy=crm`); await expand();
  ...
  expect(f.calls[0]!.input).toMatchObject({ expectedTaskId: invocationTaskId, operationKey: invocationOperation.key, pathParameters: { id: 'a / b' }, query: { search: '中文' }, body: '{"title":"保留正文"}' });
...
test('Swagger 输入先绑定会话；会话变化或读取失败后不发送，明确重绑才可继续', async () => {
  ...
  f.state.taskId = TaskIdSchema.parse(`tsk_${'e'.repeat(32)}`); await refreshInvocationQueries(page); await invocationClick(page, 'Execute', block);
...
test('切换代理或重新加载文档前确认，取消保留；文档更新和读取失败不会卸载草稿', async () => {
  ...
  await act(async () => { picker.value = 'billing'; picker.dispatchEvent(new Event('change', { bubbles: true })); }); await page.settle();
  expect(page.text()).toContain('丢弃 crm 的 Swagger 输入'); await invocationClick(page, '保留当前输入'); expect(picker.value).toBe('crm'); expect(field().value).toBe('kept');
...
test('Swagger 在途修改保留新输入，另一个 Execute 不会并发发送，截断与来源在当前操作明确展示', async () => {
  ...
  page = await renderApp(`${invocationRoute}&proxy=crm`); await expand(); const post = document.querySelector('.opblock-post')!;
