...
export const ADMIN_ID = '01a0bf5d-8f4b-799e-8662-91273789253a';
export const TASK_ID = '01a0bf5d-8f4b-7558-86ec-159088a0105c';
const fixtureIds = new Map<string, string>();
export const profileIdOf = (key: string): string => { if (/^[0-9a-f-]{36}$/.test(key)) return key; if (!fixtureIds.has(key)) fixtureIds.set(key, Bun.randomUUIDv7()); return fixtureIds.get(key)!; };
export const testIdOf = (n: number) => profileIdOf(`test:${n}`);
...
export function profileTest(overrides: Loose<ProfileTestDto> = {}): ProfileTestDto {
  return ProfileTestDtoSchema.parse({
    testId: testIdOf(1), revision: 1, contentHash: 'c0ffee00c0ffee00c0ffee', trigger: 'save', state: 'passed', outcome: 'passed',
    stages: [
      ...
      { id: 'step:claude-settings', kind: 'step', stepId: '01a0bf5d-8f4b-7e58-8d8a-6b38a70679a1', name: 'Claude settings.json', state: 'succeeded', durationMs: 5 },
    ...
    ...overrides, profile: profileIdOf(String(overrides.profile ?? "claude-daily")),
  });
}
...
export function profileDetail(overrides: DetailOverrides = {}): ComputeProfileDetailDto {
  ...
  return ComputeProfileDetailDtoSchema.parse({
    id: profileIdOf(String(rest.name ?? 'claude-daily')), name: 'claude-daily', protocol: 'claude-code', description: '日常开发', enabled: true, isDefault: false, revision: 1,
    ...
    content: {
      ...
      steps: [{ kind: 'file', stepId: '01a0bf5d-8f4b-7e58-8d8a-6b38a70679a1', name: 'Claude settings.json', pathTemplate: SETTINGS_PATH, contentTemplate: '{"env":{"ANTHROPIC_AUTH_TOKEN":"{{secrets.ANTHROPIC_AUTH_TOKEN}}"}}', format: 'json', mode: 0o600, existing: 'require-same' }],
      vars: {}, secrets: [{ id: '01a0bf5d-8f4b-794b-85ed-6ff36d0b859a', name: 'ANTHROPIC_AUTH_TOKEN' }], configFile: { kind: 'claude-settings', pathTemplate: SETTINGS_PATH },
      ...content,
    },
    contentHash: 'c0ffee00c0ffee00c0ffee', credentials: [{ id: '01a0bf5d-8f4b-794b-85ed-6ff36d0b859a', name: 'ANTHROPIC_AUTH_TOKEN', set: true }], referencedBy: [], createdBy: ADMIN_ID, createdAt: AT,
    ...rest,
  });
}
...
export function terminalProfile(overrides: DetailOverrides = {}): ComputeProfileDetailDto {
  return profileDetail({
    ...
    ...overrides,
    content: {
      image: 'registry.cs.local/runtimes/aider:1', launch: { protocol: 'terminal', binaryPath: '/opt/aider/bin/aider', extraArgs: [], isSandbox: false }, steps: [], secrets: [], configFile: { kind: 'none' },
...
export function computeBackend(initial: readonly ComputeProfileDetailDto[] = [profileDetail({ isDefault: true })]) {
  ...
  const replace = (next: ComputeProfileDetailDto) => { state.profiles = state.profiles.map((p) => (p.id === next.id ? next : p)); return next; };
  const create = (body: Record<string, unknown>) => {
    ...
    const created = profileDetail({ name, description: body.description, defaultVisible: body.defaultVisible ?? true, protocol: content.launch.protocol, image: content.image, binaryPath: content.launch.binaryPath, model: content.launch.model,
      availability: { state: 'testing', available: false, reason: '测试中' }, latestTest: queuedTest(profileIdOf(name), 1), content, credentials: content.secrets.map((secret) => ({ ...secret, set: true })) });
  ...
  const save = (profile: ComputeProfileDetailDto, body: Record<string, unknown>) => {
    ...
    const executed = changed ? { content, revision, availability: { state: 'testing' as const, available: false, reason: '测试中' }, latestTest: queuedTest(profile.id, revision) } : {};
  ...
  const profileRoute = (method: string, sub: string, query: string, body: Record<string, unknown>, profile: ComputeProfileDetailDto): Response => {
    ...
    if (sub === 'default') { state.profiles = state.profiles.map((p) => ({ ...p, isDefault: p.id === profile.id, defaultVisible: p.id === profile.id ? true : p.defaultVisible })); return json(state.profiles.find((p) => p.id === profile.id)); }
    if (sub === 'copy') { const copy = { ...profile, id: Bun.randomUUIDv7(), name: String(body.name), isDefault: false, revision: 1 }; state.profiles.push(copy); return json(copy, 201); }
  ...
  const route = (method: string, path: string, query: string, body: Record<string, unknown>): Response => {
    ...
    if (path === '/v1/catalog/task-profiles') return json({ items: [{ id: '01a0bf5d-8f4b-74c3-80c6-46c408c84194', name: 'cli-large', cpu: '2', memory: '4Gi', storage: '2Gi', description: '' }] });
    ...
    const profile = state.profiles.find((p) => p.id === decodeURIComponent(match[1]!));
