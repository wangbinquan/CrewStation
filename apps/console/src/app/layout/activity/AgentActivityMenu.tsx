...
export function AgentActivityMenu(): ReactElement {
  ...
  return <div className={styles.root} ref={root}>
    <Button variant="ghost" aria-expanded={open} onClick={() => open ? close() : setOpen(true)}>{t('activity.title')}{counts.pending ? <b className={styles.warning}>{t('activity.pendingCount', { count: counts.pending })}</b> : null}{counts.completions ? <b className={styles.success}>{t('activity.completedCount', { count: counts.completions })}</b> : null}</Button>
