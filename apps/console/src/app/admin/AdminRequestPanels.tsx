...
export function AdminRequestPanels({ search }: { readonly search: RequestSearch }) {
  ...
  return <>
    ...
    <AccessRequestReview scope={{ ...(search.projectId ? { projectId: search.projectId } : {}), state: search.state, ...(search.accessCursor ? { cursor: search.accessCursor } : {}) }}
      title={t('admin.requests.app')} empty={t('admin.requests.appEmpty')} onPage={(accessCursor) => void navigate({ to: '/admin/requests', search: { ...search, accessCursor } })} />
  </>;
}
