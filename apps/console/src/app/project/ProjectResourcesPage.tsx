...
import { ProjectSettingsSection } from '../../features/projects';
import { useT } from '../../shared/lib/useT';
import { PROJECT_PATHS } from '../../shared/project/projectPaths';
import { useProjectScope } from '../../shared/project/ProjectScope';
import { parseResourceSearch, RESOURCE_SECTIONS } from '../../shared/project/resourceSearch';
...
export function ProjectResourcesPage() {
  ...
  const search = parseResourceSearch(useSearch({ strict: false })), section = search.section ?? 'api';
  ...
  return <div className={styles.page}>
    ...
    <SectionNavigation label={t('resources.groups')} value={section} items={RESOURCE_SECTIONS.map((value) => ({ value, label: t(`resources.section.${value}`), description: t(`resources.hint.${value}`) }))} onChange={(value) => change({ section: value as ResourceSection })}>
      <Stack key={`${projectId}:${section}`}>
        ...
        {section === 'data' ? <Link to={PROJECT_PATHS[space].development} params={{ projectId }} search={{ view: 'data' }}>{t('resources.openData')}</Link> : null}
        {section === 'project' ? <ProjectSettingsSection section="repository" /> : null}
        ...
        {section === 'api' || section === 'events' ? <details><summary>{t('resources.technicalSummary')}</summary><CapabilitiesPage embedded section={section} /></details> : <CapabilitiesPage embedded section={section} topic={search.topic} />}
      </Stack>
    </SectionNavigation>
  </div>;
}
