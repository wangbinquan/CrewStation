...
export function ProjectSettingsPage() {
  ...
  const tab = search.tab === 'visibility' && space === 'admin' ? 'members' : search.tab ?? 'members';
  const change = (next: SettingsSearch) => { void navigate({ to: PROJECT_PATHS[space].settings, params: { projectId }, search: next }); };
  return <div className={styles.page}>
    <PageHeader title={t('nav.settings')} />
    <Tabs label={t('nav.settings')} value={tab} items={tabs.map((value) => ({ value, label: t(`settings.tab.${value}`) }))} onChange={(value) => change({ tab: value as SettingsTab })}>
