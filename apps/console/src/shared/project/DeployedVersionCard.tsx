...
export interface DeployedVersionCardProps {
  ...
  /** 底部操作条最左的主动作（上线／回退、重新部署），蓝底。 */
  readonly primary?: ReactNode;
  /** 排在访问入口之后的其他动作（进入维护、推迟、下线……），危险动作放最后。 */
 ...
 * 标签作标题，下一行提交与副本，再下是地址；按钮全在底部操作条（2026-09-23 裁定：对象卡片的动作放底部）：
 * 主动作 → 访问入口（只在实际就绪时出现）→ 其他动作。
 */
export function DeployedVersionCard({ role, slot, known, onSelect, health, sha = 'full', primary, actions, maintenance }: DeployedVersionCardProps): ReactElement {
  ...
  const live = identity && !offline && !empty, open = live && slotCanOpen(slot) ? <ExternalButtonLink href={`//${slot!.host}`}>{t(`slot.open.${role}`)}</ExternalButtonLink> : null;
  return <Card compact title={t(`slot.${role}`)} extra={badges} actions={<>{primary}{open}{actions}</>}>
    ...
    {offline ? <div className={styles.version}>
      ...
      <p className={styles.host}>{slot!.host}{slotCanOpen(slot) ? null : <span className={styles.note}> · {t('slot.notReady')}</span>}</p>
