export type DevelopmentView = 'cli' | 'preview' | 'split' | 'code' | 'diff' | 'changes' | 'conversation';
...
export function parseDevelopmentSearch(search: Record<string, unknown>): DevelopmentSearch {
  const view = typeof search.view === 'string' && ['cli', 'preview', 'split', 'code', 'diff', 'changes', 'conversation'].includes(search.view) ? search.view as DevelopmentView : undefined;
