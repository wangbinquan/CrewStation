 ...
 * 弹窗只在 `open` 时画出来：取消、✕、Esc 只收起；再点同一个对象恢复上次输入（没改过就按这次传入的最新值重起一份）；
 * 有未保存输入时改去编辑别的对象先确认（`next`），「继续编辑」回到原来那份；「清空」让同一个对象重挂一份表单；
 * 保存成功才丢草稿。`same` 判断两次点的是不是同一个对象。
 */
export function useDraftTarget<T>(same: (current: T, next: T) => boolean) {
  ...
  return {
    ...
    select: (value: T) => { if (target && dirty && same(target.value, value)) setOpen(true); else if (dirty) setNext({ value }); else apply({ value }); },
