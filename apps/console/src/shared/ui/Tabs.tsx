...
export function Tabs({ label, items, value, onChange, children }: TabsProps): ReactElement {
  const id = useId();
  const list = useRef<HTMLDivElement>(null);
  const index = Math.max(0, items.findIndex((item) => item.value === value));
  return <div className={styles.tabs}>
    <div className={styles.list} role="tablist" aria-label={label} ref={list} onKeyDown={(event) => {
