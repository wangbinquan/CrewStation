  ...
  return <div className={styles.stack}>
    <Card compact stacked title={t('logs.alerts.title')} footer={t('logs.alerts.recent')}>
      ...
      {shown.length ? <DataTable columns={[t('logs.alerts.type'), t('logs.alerts.state'), t('logs.alerts.detail'), t('logs.alerts.firedAt')]}>
        {shown.map((row) => <tr key={row.id}><td><Button size="small" aria-pressed={row.id === search.alertId} onClick={() => change({ ...search, tab: 'alerts', alertId: row.id })}>{t(`logs.alerts.type.${row.type}`)}</Button></td><td><Badge tone={row.state === 'firing' ? 'danger' : 'neutral'}>{t(`logs.alerts.state.${row.state}`)}</Badge></td><td className={styles.detail}>{row.detail}</td><td>{date(row.firedAt)}</td></tr>)}
      </DataTable> : null}
    </Card>
    {search.alertId ? <Card compact title={t('logs.alerts.selected')} extra={<Button variant="ghost" onClick={() => change({ ...search, alertId: undefined })}>{t('logs.alerts.closeDetail')}</Button>}
      actions={selected?.slot ? <Button onClick={() => onLogs(selected.slot!)}>{t(`logs.alerts.logs.${selected.slot}`)}</Button> : undefined}>
