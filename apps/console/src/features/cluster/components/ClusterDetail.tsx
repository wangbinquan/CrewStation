...
export function ClusterDetail({ resourceId, snapshotId, close, select, onOperation }: { resourceId: string; snapshotId?: string; close: () => void; select: (row: ClusterResource) => void; onOperation: (id: string) => void }) {
  ...
  return <section ref={panel} tabIndex={-1} aria-label={t('cluster.detail')}><Card title={r?.name ?? t('cluster.detail')} extra={<Button onClick={close}>{t('cluster.close')}</Button>} className={styles.detail} stacked><QueryStatus isPending={detail.isPending} error={detail.error} />{r ? <>
    ...
    <Tabs label={t('cluster.detail')} value={tab} onChange={setTab} items={['overview', 'related', 'containers', 'events', ...(r.kind === 'Pod' ? ['logs'] : [])].map((value) => ({ value, label: t(`cluster.detail.${value}`) }))}>
      {tab === 'overview' ? <div className={styles.stack}><dl className={styles.facts}>{Object.entries({ UID: r.uid, [t('cluster.purpose')]: t(`cluster.purpose.${r.purpose}`), [t('cluster.status')]: r.phase, [t('cluster.ready')]: r.ready ? t('cluster.yes') : t('cluster.no'), [t('cluster.reason')]: r.reason || '—', ...r.facts, ...(r.taskId ? { taskId: r.taskId } : {}), ...(r.parentTaskId ? { parentTaskId: r.parentTaskId } : {}), ...(r.agentId ? { agentId: r.agentId } : {}), ...(r.releaseId ? { releaseId: r.releaseId } : {}), ...(r.profile ? { [t('cluster.profile')]: `${r.profile}${r.profileRevision === undefined ? '' : ` @${r.profileRevision}`}` } : {}), [t('cluster.observed')]: new Date(r.observedAt).toLocaleString() }).map(([key, value]) => <Pair key={key} label={key} value={value} />)}</dl><ClusterActionPanel row={r} onOperation={onOperation} /></div> : null}
      ...
      {tab === 'containers' ? <div className={styles.stack}>{r.containers.map((c) => <Card key={c.name} title={`${c.init ? 'init · ' : ''}${c.name}`} compact><dl className={styles.facts}><Pair label={t('cluster.image')} value={c.image} /><Pair label={t('cluster.status')} value={`${c.state} · ${c.reason ?? ''} ${c.exitCode ?? ''}`} /><Pair label="requests" value={JSON.stringify(c.requests)} /><Pair label="limits" value={JSON.stringify(c.limits)} /><Pair label={t('cluster.ports')} value={c.ports.join(', ') || '—'} /><Pair label={t('cluster.reason')} value={c.message ?? '—'} /></dl></Card>)}</div> : null}
