...
export function ClusterNodes({ selectPod }: { selectPod: (id: string) => void }) {
  ...
  return <Card title={t('cluster.tab.nodes')} stacked><p className={styles.caption}>{t('cluster.metrics.nodesReadOnly')}</p><QueryStatus isPending={query.isPending} error={query.error} />
    {query.error?.status === 410 ? <Button onClick={() => { setCursor(0); setObservationId(undefined); }}>{t('cluster.newSnapshot')}</Button> : null}
    <DataTable className={styles.table} columns={[t('cluster.metrics.node'), t('cluster.status'), 'Pod', 'CPU', t('cluster.metrics.memory'), t('cluster.metrics.disk'), t('cluster.metrics.network')]}>{query.data?.items.map((n) => <tr key={n.uid}><td><Button variant="ghost" onClick={() => setSelected(n.resourceId)}>{n.name}</Button><small>{n.roles.join(', ')} · {n.version}</small></td><td>{n.ready ? t('cluster.yes') : t('cluster.no')}{n.unschedulable ? <small>{t('cluster.metrics.cordoned')}</small> : null}</td><td>{n.podCount} / {n.allocatable.pods ?? '—'}</td><td><MetricValue metric={n.metrics.cpu} compact /><small>{t('cluster.metrics.requests')} {amount(n.demand.requests.cpu ?? '0', 'cores')} / {amount(n.allocatable.cpu, 'cores')}</small></td><td><MetricValue metric={n.metrics.memory} compact /><small>{t('cluster.metrics.requests')} {amount(n.demand.requests.memory ?? '0')} / {amount(n.allocatable.memory)}</small></td><td><MetricValue metric={n.metrics.fsUsed} compact /><small>{t('cluster.metrics.available')} {amount(n.metrics.fsAvailable?.value)}</small></td><td>↓ <MetricValue metric={n.metrics.networkRx} compact /><br />↑ <MetricValue metric={n.metrics.networkTx} compact /></td></tr>)}</DataTable>
...
function NodeDetails({ node, close, selectPod }: { node: ClusterNode; close: () => void; selectPod: (id: string) => void }) {
  ...
  return <Card title={node.name} extra={<Button onClick={close}>{t('cluster.close')}</Button>} stacked><p className={styles.code}>UID: {node.uid}</p><ResourceBudgetTable demand={node.demand} capacity={node.capacity} allocatable={node.allocatable} />
    ...
    <DataTable className={styles.budgetTable} columns={[t('cluster.metrics.interface'), t('cluster.metric.networkRx'), t('cluster.metric.networkTx'), t('cluster.metric.networkRxErrors'), t('cluster.metric.networkTxErrors')]}>{Object.entries(node.interfaces).map(([name, metrics]) => <tr key={name}><td><Button variant="ghost" onClick={() => { setNetwork(name); setDevice(''); }}>{name}</Button></td>{(['networkRx', 'networkTx', 'networkRxErrors', 'networkTxErrors'] as const).map((m) => <td key={m}><MetricValue metric={metrics[m]} compact /></td>)}</tr>)}</DataTable>
    <DataTable className={styles.budgetTable} columns={[t('cluster.metrics.device'), t('cluster.metric.diskRead'), t('cluster.metric.diskWrite'), t('cluster.metric.diskReadOps'), t('cluster.metric.diskWriteOps')]}>{Object.entries(node.devices).map(([name, metrics]) => <tr key={name}><td><Button variant="ghost" onClick={() => { setDevice(name); setNetwork(''); }}>{name}</Button></td>{(['diskRead', 'diskWrite', 'diskReadOps', 'diskWriteOps'] as const).map((m) => <td key={m}><MetricValue metric={metrics[m]} compact /></td>)}</tr>)}</DataTable><p className={styles.caption}>{t('cluster.metrics.deviceHint')}</p>
    <details><summary>{t('cluster.metrics.managedPods', { count: node.managedPodIds.length })}</summary><div className={styles.controls}>{node.managedPodIds.map((id) => <Button key={id} variant="ghost" onClick={() => selectPod(id)}>{node.managedPods?.find((p) => p.resourceId === id)?.name ?? id}</Button>)}</div></details>
    {device || network ? <p>{device || network} <Button onClick={() => { setDevice(''); setNetwork(''); }}>{t('cluster.metrics.nodeHistory')}</Button></p> : null}<ClusterHistory key={`${device}/${network}`} scope="node" resourceId={node.resourceId} device={device || undefined} networkInterface={network || undefined} />
  </Card>;
}
