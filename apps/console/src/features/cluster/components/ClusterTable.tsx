...
export function ClusterTable({ rows, select }: { rows: ClusterResource[]; select: (row: ClusterResource) => void }) {
  const t = useT();
  return <DataTable className={styles.table} columns={[t('cluster.resource'), t('cluster.owner'), t('cluster.purpose'), t('cluster.status'), t('cluster.ready'), t('cluster.location')]}>
    {rows.map((r) => <tr key={r.uid}><td><Button variant="ghost" data-cluster-resource={r.resourceId} onClick={() => select(r)}>{r.name}</Button><small>{r.kind}</small></td><td><Ownership row={r} /></td><td>{t(`cluster.purpose.${r.purpose}`)}{r.slotRole ? <small>{r.physicalSlot} · {r.slotRole}</small> : null}</td><td><Badge tone={r.abnormal ? 'danger' : r.deletingAt ? 'warning' : 'neutral'}>{r.deletingAt ? t('cluster.terminating') : r.phase}</Badge>{r.reason ? <small className={styles.reason} title={r.reason}>{r.reason}</small> : null}</td><td>{r.kind === 'Pod' ? r.ready ? t('cluster.yes') : t('cluster.no') : r.desired === undefined ? '—' : `${r.readyReplicas ?? 0}/${r.desired}`}{r.restarts > 0 ? <small>{t('cluster.restarts', { count: r.restarts })}</small> : null}</td><td>{r.namespace || '—'}{r.node ? <small>{r.node}</small> : null}</td></tr>)}
  </DataTable>;
}
