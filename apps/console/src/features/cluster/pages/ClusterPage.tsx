...
export function ClusterPage() {
  ...
  const rows = useApiQuery(queryKeys.cluster('resources', resourceQuery), () => api.cluster.resources(resourceQuery), { enabled: !!snapshotId && !['operations', 'nodes', 'history', 'topology'].includes(search.tab ?? ''), keepPrevious: (previous) => sameApartFromSnapshot(previous, resourceQuery) });
  ...
  return <><PageHeader title={t('cluster.title')} description={t('cluster.description')} actions={<Button onClick={() => refresh.mutate(undefined)} disabled={refresh.isPending}>{t('cluster.refresh')}</Button>} />
    <Stack><ClusterCapacityPanel /><QueryStatus isPending={summary.isPending} error={summary.error ?? refresh.error} />
      ...
      {!['nodes', 'topology'].includes(search.tab ?? '') ? <Card compact><ClusterFilters search={search} summary={s} change={change} /></Card> : null}
      <Tabs label={t('cluster.resources')} value={search.tab ?? 'workloads'} onChange={(tab) => change({ tab: tab as ClusterSearch['tab'], kind: undefined, purpose: undefined, status: undefined })} items={['topology', 'workloads', 'pods', 'network', 'storage', 'nodes', 'history', 'namespaces', 'operations'].map((value) => ({ value, label: t(`cluster.tab.${value}`) }))}>
        {search.tab === 'topology' ? <ClusterTopology search={search} change={change} go={go} summary={s} snapshotId={snapshotId} /> : search.tab === 'nodes' ? <ClusterNodes selectPod={(resourceId) => go({ ...search, resourceId })} /> : search.tab === 'history' ? <ClusterHistoryBrowser key={filter.projectId ?? 'all'} projectId={filter.projectId} /> : search.tab === 'operations' ? <ClusterOperations search={search} change={change} /> : <Card title={t('cluster.resourceCount', { count: rows.data?.total === 0 && !rows.data.complete ? '—' : rows.data?.total ?? '—' })} compact stacked>
