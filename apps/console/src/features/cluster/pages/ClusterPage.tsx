...
export function ClusterPage() {
  ...
  const rows = useApiQuery(queryKeys.cluster('resources', { ...filter, snapshotId }), () => api.cluster.resources({ ...filter, snapshotId }), { enabled: !!snapshotId && search.tab !== 'operations' });
  ...
  return <><PageHeader title={t('cluster.title')} description={t('cluster.description')} actions={<Button onClick={() => refresh.mutate(undefined)} disabled={refresh.isPending}>{t('cluster.refresh')}</Button>} />
    <Stack><QueryStatus isPending={summary.isPending} error={summary.error ?? refresh.error} />
      ...
      <Card compact><ClusterFilters search={search} summary={s} change={change} /></Card>
      <Tabs label={t('cluster.resources')} value={search.tab ?? 'workloads'} onChange={(tab) => change({ tab: tab as ClusterSearch['tab'], kind: undefined, purpose: undefined, status: undefined })} items={['workloads', 'pods', 'network', 'storage', 'namespaces', 'operations'].map((value) => ({ value, label: t(`cluster.tab.${value}`) }))}>
        {search.tab === 'operations' ? <ClusterOperations projectId={search.projectId} operationId={search.operationId} /> : <Card title={t('cluster.resourceCount', { count: rows.data?.total === 0 && !rows.data.complete ? '—' : rows.data?.total ?? '—' })} compact stacked>
          ...
          {rows.data ? <ClusterTable rows={rows.data.items} select={(row) => go({ ...search, resourceId: row.resourceId })} /> : null}
