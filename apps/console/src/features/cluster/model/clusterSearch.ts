...
export type ClusterSearch = Partial<ClusterFilter> & { tab?: 'workloads' | 'pods' | 'network' | 'storage' | 'namespaces' | 'operations' | 'nodes' | 'history'; resourceId?: string; operationId?: string; operationUid?: string; operationPhase?: ClusterOperationQuery['phase'] };
export function parseClusterSearch(raw: Record<string, unknown>): ClusterSearch {
  ...
  const tab = ['workloads', 'pods', 'network', 'storage', 'namespaces', 'operations', 'nodes', 'history'].includes(String(raw.tab)) ? raw.tab as ClusterSearch['tab'] : 'workloads';
...
export function clusterFilter(search: ClusterSearch): ClusterFilter { const { tab, resourceId: _resourceId, operationId: _operationId, ...filters } = search; return ClusterFilterSchema.parse({ ...filters, view: ['operations', 'nodes', 'history'].includes(tab ?? '') ? undefined : tab ?? 'workloads' }); }
