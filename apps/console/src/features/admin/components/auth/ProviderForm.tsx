...
function useProviderDraft(initial: OidcProviderDto | undefined) {
  ...
  const incomplete = value.slug === '' || value.displayName === '' || value.issuerUrl === '' || value.clientId === '' || value.scopes === '' || (initial === undefined && value.clientSecret === '');
...
function IdentityFields({ draft, editing }: { readonly draft: Draft; readonly editing: boolean }): ReactElement {
  ...
  return (
    <>
      <AdminField label={t('admin.auth.slug')} value={draft.value.slug} onChange={draft.set('slug')} hint={t('admin.auth.slugHint')} placeholder="corp-sso" disabled={editing} />
      <AdminField label={t('admin.auth.displayName')} value={draft.value.displayName} onChange={draft.set('displayName')} hint={t('admin.auth.displayNameHint')} />
      <AdminField label={t('admin.auth.issuerUrl')} value={draft.value.issuerUrl} onChange={draft.set('issuerUrl')} hint={t('admin.auth.issuerUrlHint')} placeholder="https://idp.corp.com" />
      <AdminField label={t('admin.auth.clientId')} value={draft.value.clientId} onChange={draft.set('clientId')} />
      <AdminField label={t('admin.auth.clientSecret')} value={draft.value.clientSecret} onChange={draft.set('clientSecret')} hint={editing ? t('admin.auth.clientSecretKeep') : t('admin.auth.clientSecretHint')} />
      <AdminField label={t('admin.auth.scopes')} value={draft.value.scopes} onChange={draft.set('scopes')} hint={t('admin.auth.scopesHint')} />
    </>
  );
}
...
function EndpointFields({ draft }: { readonly draft: Draft }): ReactElement {
  ...
  return (
    <>
      <AdminField label={t('admin.auth.authorizationEndpoint')} value={draft.value.authorizationEndpoint} onChange={draft.set('authorizationEndpoint')} hint={t('admin.auth.manualHint')} placeholder="https://idp.corp.com/oauth/authorize" />
      <AdminField label={t('admin.auth.tokenEndpoint')} value={draft.value.tokenEndpoint} onChange={draft.set('tokenEndpoint')} placeholder="https://idp.corp.com/oauth/token" />
      <AdminField label={t('admin.auth.userinfoEndpoint')} value={draft.value.userinfoEndpoint} onChange={draft.set('userinfoEndpoint')} placeholder="https://idp.corp.com/api/user" />
      <AdminField label={t('admin.auth.jwksUri')} value={draft.value.jwksUri} onChange={draft.set('jwksUri')} placeholder="https://idp.corp.com/jwks.json" />
      <AdminField
        label={t('admin.auth.userinfoStyle')}
        value={draft.value.userinfoRequestStyle}
  ...
  return (
    <>
      <AdminField
        ...
        value={draft.value.provisioning}
      ...
      />
      <AdminField label={t('admin.auth.allowedDomains')} value={draft.value.allowedDomains} onChange={draft.set('allowedDomains')} disabled={draft.value.provisioning !== 'allowlist'} hint={t('admin.auth.allowedDomainsHint')} placeholder="@corp.com, @sub.corp.com" />
      <AdminField
        label={t('admin.auth.trustEmail')}
        value={draft.value.trustEmailVerified}
      ...
      />
      <AdminField label={t('admin.auth.usernameClaim')} value={draft.value.usernameClaim} onChange={draft.set('usernameClaim')} hint={t('admin.auth.usernameClaimHint')} placeholder="preferred_username" />
      <AdminField label={t('admin.auth.gitNameClaim')} value={draft.value.gitNameClaim} onChange={draft.set('gitNameClaim')} hint={t('admin.auth.gitNameClaimHint')} placeholder="git_name" />
      <AdminField label={t('admin.auth.emailClaim')} value={draft.value.emailClaim} onChange={draft.set('emailClaim')} hint={t('admin.auth.emailClaimHint')} placeholder="email" />
      <AdminField label={t('admin.auth.subjectClaim')} value={draft.value.subjectClaim} onChange={draft.set('subjectClaim')} hint={t('admin.auth.subjectClaimHint')} placeholder="sub" />
      <AdminField label={t('admin.auth.claimMappings')} value={draft.value.claimMappings} onChange={draft.set('claimMappings')} hint={t('admin.auth.claimMappingsHint')} placeholder="employee-no=empNo, department=deptName" />
      <AdminField
 ...
 */
export function ProviderForm({ initial, busy, error, onSubmit }: ProviderFormProps): ReactElement {
  const t = useT();
  ...
  return (
    <AdminForm
      ...
      onSubmit={() => onSubmit(draft.toRequest())}
    >
