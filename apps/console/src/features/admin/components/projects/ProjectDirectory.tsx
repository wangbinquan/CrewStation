...
export function ProjectDirectory({ search, apply, integration = false }: {
  ...
  const { query, refresh, busy, userId } = useAdminRead(queryKeys.adminProjectPage(request), async () => {
  ...
  return <Card compact title={t(integration ? 'admin.integrations.title' : 'admin.directory.title')} footer={t('admin.directory.hint')}
    extra={<div className={styles.actions}><Button disabled={busy} onClick={() => void refresh()}>{t('admin.directory.refresh')}</Button>
    ...
    <ProjectDirectoryFilters key={JSON.stringify(search)} search={search} items={items} userId={userId} integration={integration} busy={busy} apply={apply} />
    ...
    {items.length ? <ProjectDirectoryTable items={items} available={settled && !busy} /> : null}
    <div className={styles.toolbar}><span className={styles.muted}>{settled ? t('admin.directory.count', { count: items.length }) : t('admin.directory.countUnknown')}</span>
      <div className={styles.actions}>{search.cursor ? <Button disabled={busy} onClick={() => apply({ ...search, cursor: undefined })}>{t('admin.directory.first')}</Button> : null}
        <Button disabled={busy || !!query.error || !query.data?.nextCursor} onClick={() => apply({ ...search, cursor: query.data?.nextCursor })}>{t('admin.directory.next')}</Button></div></div>
  </Card>;
}
