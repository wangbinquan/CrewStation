...
export function AdminProjectTodos() {
  ...
  const { query, refresh, busy } = useAdminRead(queryKeys.adminProjectPage(request), async () => {
  ...
  const items = [401, 403, 404].includes(query.error?.status ?? 0) ? [] : query.data?.items ?? [], current = !busy && !query.error;
  return <AdminTodoCard title={t('admin.todo.projects')} refreshLabel={t('admin.todo.refresh.projects')} pending={query.isPending} error={query.error} busy={busy}
