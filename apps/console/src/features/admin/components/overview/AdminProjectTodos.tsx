...
export function AdminProjectTodos() {
  ...
  return <AdminTodoCard title={t('admin.todo.projects')} pending={query.isPending} error={query.error}
    count={query.data ? items.length : undefined} more={!!query.data?.nextCursor}
    open={<ButtonLink to="/admin/projects" search={{ state: 'failed' }}>{t('admin.todo.openProjects')}</ButtonLink>}>
