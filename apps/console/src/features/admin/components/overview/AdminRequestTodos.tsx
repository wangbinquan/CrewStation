...
export function AdminRequestTodos() {
  ...
  const { query, refresh, refreshing } = useAdminRead(queryKeys.accessRequestPage(request), async () => {
  ...
  return <AdminTodoCard title={t('admin.todo.api')} refreshLabel={t('admin.todo.refresh.api')} pending={query.isPending} error={query.error} busy={refreshing}
    count={query.data ? items.length : undefined} more={!!query.data?.nextCursor} checkedAt={query.dataUpdatedAt} refresh={refresh}
    open={<ButtonLink size="small" to="/admin/requests" search={{ state: 'pending' }}>{t('admin.todo.openRequests')}</ButtonLink>}>
