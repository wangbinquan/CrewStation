...
export function ResourceCatalogSection({ kind }: { readonly kind: ResourceCatalogKind }) {
  ...
  return <Card compact title={t(`admin.${prefix}.title`)} footer={t(`admin.${prefix}.hint`)} extra={<Button variant="primary" disabled={busy || frozen} onClick={() => { writer.resetFeedback(); editor.requestLoad(); }}>{t('admin.resource.new', { kind: t(`admin.resource.${kind}`) })}</Button>}>
