...
export function AdminOverviewPage(): ReactElement {
  ...
  return (
    <AdminSection title={t('nav.admin.overview')} description={t('admin.overview.hint')}>
      <ActionRow><ButtonLink to="/admin/projects/new" search={{ scope: 'digital-worker' }}>{t('projects.wizard.title.digital-worker')}</ButtonLink><ButtonLink to="/admin/projects/new" search={{ scope: 'integration' }}>{t('projects.wizard.title.integration')}</ButtonLink></ActionRow>
      <AdminTodos />
      <AdminOverviewCards />
    </AdminSection>
  );
