...
export function ApiInvocationResult({ outcome }: { readonly outcome: ApiInvocationOutcome }) {
  ...
  return <Card compact title={t('catalog.invoke.result')}>
    <div className={styles.summary}><Badge tone={result.status >= 400 ? 'warning' : 'info'}>HTTP {result.status}</Badge><span>{t('catalog.invoke.duration', { ms: result.durationMs })}</span><code>{outcome.request.operationKey}</code></div>
