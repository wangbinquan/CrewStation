 ...
 * 下线删掉工作负载、不可恢复，只能从发布记录重新部署，所以要行内确认。
 */
export function StandbyActions({ slot, lifecycle, disabled }: StandbyActionsProps): ReactElement {
  ...
  return <ActionRow>
    {period ? <Button size="small" disabled={busy} onClick={() => void lifecycle.postpone(slot)}>
      {lifecycle.pending === 'postpone' ? t('release.lifecycle.postponing') : t(`release.lifecycle.postpone.${period.unit}`, { count: period.count })}
    </Button> : null}
    <InlineConfirm variant="ghost" label={t('release.lifecycle.offline')} busy={busy} {...(lifecycle.pending === 'offline' ? { busyLabel: t('release.lifecycle.offlining') } : {})}
...
/**
 * 待验证卡上负责人的动作区：有版本时是推迟与下线；已下线且那个版本还能重新部署时，给「重新部署」的快捷入口。
 * 不是负责人或管理员（`lifecycle` 为空）时什么都不画。
 */
export function standbyLifecycle(slot: SlotDto | undefined, releases: readonly ReleaseDto[], lifecycle: SlotLifecycle | undefined, disabled: boolean, onRedeploy: (releaseId: string) => void): ReactNode {
  if (!lifecycle || !slot) return undefined;
  if (slot.releaseId) return <StandbyActions slot={slot} lifecycle={lifecycle} disabled={disabled} />;
  const gone = slot.offline ? releases.find((release) => release.id === slot.offline!.releaseId) : undefined;
  return gone?.redeployable ? <RedeployShortcut release={gone} disabled={disabled || !!lifecycle.pending} onRedeploy={onRedeploy} /> : undefined;
}
...
function RedeployShortcut({ release, disabled, onRedeploy }: { readonly release: ReleaseDto; readonly disabled: boolean; readonly onRedeploy: (releaseId: string) => void }): ReactElement {
  const t = useT();
  return <ActionRow><Button size="small" disabled={disabled} onClick={() => onRedeploy(release.id)}>{t('release.redeploy.action', { tag: release.tag })}</Button></ActionRow>;
}
