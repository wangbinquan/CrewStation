...
import type { MaintenanceUser } from '@crewstation/contracts';
...
import { MAINTENANCE_SWITCHES, maintenanceRequest, validateMaintenanceDraft } from '../model/maintenanceDraft';
import type { MaintenanceDraftErrors } from '../model/maintenanceDraft';
...
/**
 * 进入或调整维护的弹窗（RFC-021 M4、M6、M7、M13、B1）：三个开关默认全开，原因必填，预计恢复时间与临时放行的人选填。
 * 草稿在页面上（useMaintenanceEditor），关窗不丢；别人先改过维护时服务端 409，提示后保留输入。
 */
export function MaintenanceDialog({ projectId, serviceId, editor, actions, onSaved, onSettled }: MaintenanceDialogProps): ReactElement | null {
  const t = useT(), id = useId(), reasonInput = useRef<HTMLTextAreaElement>(null), endInput = useRef<HTMLInputElement>(null);
  const [errors, setErrors] = useState<MaintenanceDraftErrors>({}), [busy, setBusy] = useState(false), [error, setError] = useState<string>();
  const state = editor.editor;
  if (!state) return null;
  const { draft } = state, updating = state.mode === 'update';
  const update = (patch: Parameters<MaintenanceEditor['update']>[0]) => { editor.update(patch); setError(undefined); };
  ...
  const submit = async () => {
    ...
    try { await api.services.setMaintenance(serviceId, maintenanceRequest(draft, state.revision)); editor.saved(); onSaved(t(updating ? 'release.maintenance.updated' : 'release.maintenance.entered')); }
  ...
  return <FormDialog title={t(updating ? 'release.maintenance.updateTitle' : 'release.maintenance.enterTitle')} submitLabel={t(updating ? 'release.maintenance.submitUpdate' : 'release.maintenance.submitEnter')}
    busyLabel={t('release.maintenance.saving')} busy={busy} error={error} dirty={editor.dirty} onSubmit={() => void submit()} onClose={editor.close}
    onClear={() => { editor.clear(); setErrors({}); setError(undefined); }}>
