...
/** 唯一发布准备：来源、只读检查、明确发布；202 交给实际 releaseId 详情继续观察。每一步的按钮在底部一行：主按钮最左，「关闭」无边框排最后。 */
export function PublishForm({ serviceId, projectId, source, canPublish, actions, onSource, onClose, onAccepted }: PublishFormProps) {
  ...
  const close = <Button variant="ghost" disabled={p.busy} onClick={onClose}>{t('release.prepare.close')}</Button>;
  return <Card stacked compact title={t('release.prepare.title')}>
    ...
    {p.step === 0 ? <PublishSourceFields preparation={p} onSource={onSource} close={close} /> : <>
      ...
      {p.step === 1 ? <ActionRow><Button variant="primary" disabled={p.busy || !canPublish || p.stale} onClick={() => p.setStep(2)}>{t('release.prepare.reviewVersion')}</Button><Button disabled={p.busy} onClick={() => p.setStep(0)}>{t('release.prepare.back')}</Button>{close}</ActionRow> : <form ref={form} noValidate aria-label={t('release.prepare.title')} onSubmit={(event) => { event.preventDefault(); void p.submit(); }}><Stack>
