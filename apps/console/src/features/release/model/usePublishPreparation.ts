...
export function usePublishPreparation(projectId: string, serviceId: string, source: PublishSource, canPublish: boolean, onAccepted: (release: ReleaseDto) => void) {
  ...
  const check = async () => {
    if (lock.current || !canPublish) return; lock.current = true; setChecking(true); setError(undefined); setFailedPaths([]); setSnapshot(undefined); setStep(0);
    try {
    ...
    } catch (cause) { setError(errorMessage(cause)); } finally { lock.current = false; setChecking(false); }
  };
  const submit = async () => {
    ...
    lock.current = true; setError(undefined); setErrors({});
    try {
      ...
      accepted.current = true; setComplete(true); if (mounted.current) onAccepted(parsed.data);
  ...
  return { source, branches, workspace, tags, selected, branch, version, message, setVersion, setMessage, setBranch, step: current ? step : 0, setStep, snapshot: current, resetCheck, check, submit, busy, checking, error, errors, setErrors, failedPaths, canPublish, stale,
    dirty: !complete && (branch !== '' || version !== 'patch' || message !== ''), accepted,
