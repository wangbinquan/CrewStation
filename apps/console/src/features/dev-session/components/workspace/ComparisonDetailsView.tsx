...
export function ComparisonDetailsView({ projectId, comparisonId, comparison }: { readonly projectId: string; readonly comparisonId: string; readonly comparison: VersionComparisonDto }): ReactElement {
  const t = useT();
  ...
  const unavailable = selection.tab === 'uncommitted' ? (comparison.workspace.status === 'unavailable' ? comparison.workspace.reason : undefined)
    ...
    : !('ahead' in comparison.commits) ? (comparison.commits.status === 'unavailable' ? comparison.commits.reason : t(`devSession.compare.relation.${comparison.commits.status}`)) : undefined;
  ...
  return <Tabs label={t('devSession.compare.details')} items={tabs.map((tab) => ({ value: tab, label: t(`devSession.compare.tab.${tab}`) }))} value={selection.tab} onChange={(tab) => setSelection({ tab: tab as ComparisonDetailQuery['tab'], limit: 50 })}>
    ...
    {data && !data.patch ? <>
      {selection.tab === 'ahead' || selection.tab === 'behind' ? <DataTable columns={['SHA', t('devSession.compare.subject')]}>
        ...
        {data.files.map((file) => <tr key={file.path}><td><Button variant="ghost" onClick={() => setSelection({ ...selection, path: file.path })}>{file.originalPath ? `${file.originalPath} → ` : ''}{file.path}</Button></td><td>{file.status}{file.untracked ? ` · ${t('devSession.compare.untracked')}` : ''}</td><td>{file.binary ? t('devSession.compare.binary') : `+${file.additions ?? '?'} −${file.deletions ?? '?'}`}</td></tr>)}
      </DataTable>}
