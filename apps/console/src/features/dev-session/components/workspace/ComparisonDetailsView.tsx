...
export function ComparisonDetailsView({ projectId, comparisonId, comparison, onOpenFile }: { readonly projectId: string; readonly comparisonId: string; readonly comparison: VersionComparisonDto; readonly onOpenFile?: (path: string) => void }): ReactElement {
  const t = useT();
  const [selected, setSelected] = useState<{ comparisonId: string; query: ComparisonDetailQuery }>({ comparisonId, query: { tab: 'ahead' in comparison.commits ? 'ahead' : 'uncommitted', limit: 50 } });
  const selection: ComparisonDetailQuery = selected.comparisonId === comparisonId ? selected.query : { tab: selected.query.tab, path: selected.query.path, limit: 50 };
  const setSelection = (query: ComparisonDetailQuery) => setSelected({ comparisonId, query });
  ...
  return <Tabs label={t('devSession.compare.details')} items={tabs.map((tab) => ({ value: tab, label: t(`devSession.compare.${preview && tab !== 'uncommitted' ? 'previewTab' : 'tab'}.${tab}`) }))} value={selection.tab} onChange={(tab) => setSelection({ tab: tab as ComparisonDetailQuery['tab'], limit: 50 })}>
    ...
    {selection.path ? <Button variant="ghost" onClick={() => setSelection({ tab: selection.tab, limit: 50 })}>{t('devSession.compare.backFiles')}</Button> : null}
    {data?.patch ? <>
    ...
    {data && !data.patch ? <>
      {selection.tab === 'ahead' || selection.tab === 'behind' ? <DataTable columns={['SHA', t('devSession.compare.subject')]}>
        ...
        {data.files.map((file) => <tr key={file.path}><td><Button variant="ghost" className={styles.filePath} onClick={() => setSelection({ ...selection, path: file.path })}>{file.originalPath ? `${file.originalPath} → ` : ''}{file.path}</Button>
