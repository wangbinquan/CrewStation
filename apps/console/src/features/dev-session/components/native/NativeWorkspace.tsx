...
export function NativeWorkspace({ taskId, userId, channel, stream, canDevelop, onActivity, preview, editor, changes }: {
  ...
  const screen = <SplitGrid mode={tab.layout} ratios={tab.ratios} onResize={(ratios) => store.update((value) => updateWorkspaceTab(value, tab.id, (current) => ({ ...current, ratios })))} separatorLabel={(axis, index) => t(`devSession.native.resize.${axis}`, { index })}
    items={visible.map((terminalId) => ({ id: terminalId, content: <NativeTerminalCard terminalId={terminalId} terminal={roster?.find((item) => item.terminalId === terminalId)} layout={layout} store={store} channel={channel} stream={stream} onStop={(id) => native.stop.mutate(id)} onActivity={onActivity} canDevelop={canDevelop} /> }))} />;
  return <section className={styles.workspace}>
    ...
    <Tabs label={t('devSession.native.tabs')} value={layout.view === 'cli' ? layout.activeTabId : layout.view} items={[...layout.tabs.map((tab) => ({ value: tab.id, label: `${tab.name} · ${tab.paneOrder.length}` })), ...(['preview', 'code', 'changes'] as const).map((value) => ({ value, label: t(`devSession.native.view.${value}`) }))]}
