...
export function DevelopmentPreview({ preview, previewHost, connected }: { readonly preview: PreviewHandle; readonly previewHost: string; readonly connected: boolean }): ReactElement {
  const t = useT();
  const [generation, setGeneration] = useState(0);
  const url = preview.confirmed ? previewUrl(previewHost, preview.status.state) : undefined;
  return <section className={styles.preview}>
    <header><strong>{t('devSession.native.developmentPreview')}</strong><span>{t(!connected || !preview.confirmed ? 'devSession.native.lifecycle.unknown' : `devSession.previewState.${preview.status.state}`)}</span>
      <Button variant="ghost" onClick={() => { setGeneration((value) => value + 1); preview.refresh(); }}>{t('devSession.preview.refresh')}</Button><Button variant="ghost" disabled={!connected || preview.busy} onClick={preview.restart}>{t('devSession.preview.restart')}</Button>
