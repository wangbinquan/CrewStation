...
export function useWorkspaceLayout(taskId: string, userId: string, initialName: string) {
  ...
  const store = useMemo(() => {
    ...
    const created = new WorkspaceLayoutStore({ get: () => api.devSession.getWorkspaceLayout(taskId), save: (input) => api.devSession.saveWorkspaceLayout(taskId, input) }, initialWorkspaceLayout(initialName), (layout) => normalizeGroups(layout, newDraftResourceId));
