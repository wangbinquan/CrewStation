...
const EMPTY: StartDraft = { compute: '', prompt: '', open: false, busy: false };
...
export function useHistoricalStart(mutation: DevAgentsHandle['start'], onStarted: (agentId: string, showResult: boolean) => void) {
  ...
  const edit = (change: Partial<Pick<StartDraft, 'compute' | 'prompt'>>) => {
  ...
  const start = () => {
    ...
    void mutation.mutateAsync({ compute: sent.compute === '' ? { kind: 'default' } : { kind: 'profile', profileId: sent.compute }, prompt: sent.prompt.trim() }).then(
      (agent) => {
  ...
  return { ...draft, edit, setOpen, start, dirty: draft.compute !== '' || draft.prompt !== '' };
}
