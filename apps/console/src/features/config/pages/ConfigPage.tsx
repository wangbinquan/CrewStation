  ...
  return (
    <Stack>
      ...
      <Tabs label={t('config.title')} value={env} items={(['development', 'production'] as const).map((value) => ({ value, label: t(`config.env.${value}`) }))} onChange={(value) => onEnvironmentChange(value === 'production' ? 'production' : 'development')}>
        {(['development', 'production'] as const).map((group) => <div key={`${projectId}:${group}`} hidden={env !== group}><DialogVisibility hidden={env !== group}><Stack><ConfigEnvPanel projectId={projectId} env={group} onDirtyChange={dirtyChanged} />{group === 'production' && env === 'production' ? <ProductionConfigImpact /> : null}</Stack></DialogVisibility></div>)}
      </Tabs>
