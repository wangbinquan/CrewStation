...
function useSummaryRefresh(userId: string | undefined, readIdentity: () => Promise<{ data?: { id: string }; error: unknown }>,
  ...
  usePollingRefetch(refresh, 30_000, enabled); return refresh;
}
...
export function useProjectSummaries(search: ProjectListSearch) {
  ...
  const refresh = useSummaryRefresh(me.data?.id, me.refetch, query.refetch, me.isSuccess && !me.error);
  return { me, query, refresh };
}
