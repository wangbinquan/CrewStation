...
/** 同一成员清单只执行一项变更；清单每 30 秒在原位重读，读取失败时保留表单，恢复后继续。例行重读不算「不可用」，否则按钮每 30 秒变灰一次。 */
export function useMemberManagement(projectId: string, canManage: boolean) {
  const members = useApiQuery(queryKeys.members(projectId), () => api.projects.listMembers(projectId), AUTO_REFRESH);
  ...
  const saveMember = async (input: SetMemberRequest) => {
    if (lock.current || unavailable || !canManage) return undefined;
  ...
  const removeMember = async (member: MemberDto) => {
    if (lock.current || unavailable || !canManage) return;
