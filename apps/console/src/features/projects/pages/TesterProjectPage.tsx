...
export function TesterProjectPage() {
  const t = useT(), { projectId } = useProjectScope(), { me, query, refresh } = useProjectSummary(projectId);
  ...
  return <>
    <PageHeader title={item?.project.name ?? t('projects.preview.title')} actions={<Button disabled={me.isFetching || query.isFetching} onClick={() => void refresh()}>{t('projects.preview.refresh')}</Button>} />
    ...
    {item ? <Card compact title={t('projects.summary.preview')}>
      <DeploymentFact item={item} name="preview" canOpen={!me.isFetching && !query.isFetching && summaryIsFresh(item)} />
