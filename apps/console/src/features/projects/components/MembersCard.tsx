...
export function MembersCard({ projectId, ownership }: MembersCardProps) {
  ...
  return <Card stacked compact title={t('projects.members.title')} extra={canManage ? <Button variant="primary" disabled={blocked} onClick={() => editor.select(null)}>{t('projects.members.add')}</Button> : undefined}>
    ...
    {editor.next ? <ConfirmationDialog question={t('projects.members.discardQuestion')} confirmLabel={t('projects.members.discard')} cancelLabel={t('ui.draft.stay')} busy={pending} focus="cancel" onConfirm={editor.confirm} onCancel={editor.keep} /> : null}
    {editor.member !== undefined ? <MemberForm key={editor.sequence} open={editor.open} initialMember={editor.member ?? undefined} projectId={projectId} isAdmin={isAdmin} canManage={canManage} members={items} pending={pending} disabled={unavailable || ownership.unavailable || Boolean(editor.next)} onSave={async (input) => { const result = await management.saveMember(input); if (result) editor.close(); return result; }} onDirtyChange={editor.dirtyChanged} onClose={editor.hide} onClear={editor.clear} /> : null}
  </Card>;
}
