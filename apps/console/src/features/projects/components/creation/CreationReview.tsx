...
export function CreationReview({ draft, catalog }: { draft: CreationDraft; catalog: CreationCatalog }) {
  const t = useT(), owner = catalog.users.find((user) => user.id === draft.ownerUserId), plan = catalog.plans.find((item) => item.id === draft.plan);
  const template = catalog.templates.find((item) => item.id === draft.template && item.kind === draft.kind);
  return <>
    <DefinitionList layout="grid" items={[
      ...
      { label: t('projects.create.kind'), value: t(`projects.kind.${draft.kind}`) }, { label: t('projects.create.template'), value: template?.name ?? draft.template },
