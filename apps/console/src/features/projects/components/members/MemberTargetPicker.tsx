...
export function MemberTargetPicker(props: PickerProps) {
  const { user, rawId, disabled, error, onSelect, onRawId, isAdmin, projectId } = props;
  const t = useT(), id = useId(), [mode, setMode] = useState<'lookup' | 'id' | 'directory'>('lookup');
  ...
  return <div className={styles.picker}>
    <div className={styles.actions}>
      {(['lookup', 'id', ...(isAdmin ? ['directory'] as const : [])] as const).map((value) => <Button key={value} variant={mode === value ? 'secondary' : 'ghost'} aria-pressed={mode === value} disabled={disabled} onClick={() => { setMode(value); onRawId(''); }}>{t(`projects.members.mode.${value}`)}</Button>)}
    </div>
    {mode === 'lookup' ? <MemberLookup projectId={projectId} onSelect={onSelect} actionKey="projects.members.select" disabled={disabled} selectionError={error} /> : null}
