...
export function ProjectSummaryActions({ item, space, available }: { readonly item: ProjectSummaryDetail; readonly space: ProjectSpace; readonly available: boolean }) {
  ...
  return <div className={styles.actions}>
    {action?.type === 'provision' ? <Link data-primary-project-action className={styles.primary} to="/admin/projects/$projectId/provisioning" params={params}>{t(action.label)}</Link> : null}
    ...
    {available && item.role !== 'admin' && (item.project.state === 'failed' || item.project.state === 'provisioning') ? <span className={styles.muted}>{t('projects.summary.contactAdmin')}</span> : null}
  </div>;
}
