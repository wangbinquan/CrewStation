...
export function ProjectListFilters({ search, items, userId, apply }: {
  readonly search: ProjectListSearch; readonly items: readonly ProjectSummary[]; readonly userId?: string;
  ...
  return <form className={styles.filters} onSubmit={(event) => { event.preventDefault(); apply({ q, state, ownerUserId: owner }); }}>
    ...
    <Button type="submit">{t('projects.summary.searchSubmit')}</Button><Button onClick={() => apply({ q: '' })}>{t('projects.summary.clear')}</Button>
  </form>;
}
