...
- 平台部署会注入 `CS_PLATFORM_API_URL`，本代理通过服务域的 `/internal/egress/http` 使用管理员出站白名单。请把上游域名加入本代理项目的规则，或在申请获批后重试；拒绝与撤销立即生效，被阻记录属于代理项目。无需放开项目命名空间到外部的直接网络。旧平台须先升级出站接口，再正常发布新版代理；没有注入平台地址的独立本地运行仍可直接请求上游。
- 平台 HTTP 出站请求体最多 1 MiB，响应最多 4 MiB，上游期限最长 8 秒；不跟随重定向，不自动重试。超时可能发生在业务操作已执行之后，请先核对结果。
