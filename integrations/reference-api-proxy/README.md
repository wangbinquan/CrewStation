...
- **本机 kind 集群上，项目命名空间到 `host.docker.internal:8929` 是不通的**（控制面所在的 `crewstation-system` 通）。所以本代理部署上去后 `/v4/...` 会按超时回 `504`，直到出站策略放行该地址，或把 `GITLAB_BASE_URL` 改成集群内可达的地址。属于出站白名单的事，不在本项目里解决。
