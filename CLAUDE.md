...
RFC-010 adds `/admin/cluster` and the L6 cluster-management module for managed-resource inventory, exact Pod-purpose classification and durable lifecycle operations. Its implementation and automated checks are separate from local RBAC approval and real mutation acceptance; see `proposal/rfc/RFC-010-cluster-management/acceptance.md` for the current boundary.
